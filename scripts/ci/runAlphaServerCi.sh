...
deploy_alpha_artifact() {
  ...
  if [[ -n "$prev_head" ]]; then
    if git diff --name-only "$prev_head" HEAD | grep -q "^packages/nolo-ci/"; then
      echo "🔁 nolo-ci core logic changed; restarting nolo-ci service..."
      NOLO_REPO_DIR="$REPO_DIR" \
      NOLO_BUN_BIN="$BUN_BIN" \
      NOLO_PM2_BIN="$PM2_BIN" \
      bash ./scripts/ops/manageNoloCiPm2.sh restart || true
    fi
  fi
}
...
deploy_main_remote() {
  ...
  remote_artifact="$(main_remote_artifact_path)"
  ssh -i "$key_path" -o IdentitiesOnly=yes "${PRODUCTION_USER}@${PRODUCTION_HOST}" \
    "BUILD_SHA='${BUILD_SHA}' REMOTE_ARTIFACT='${remote_artifact}' PRODUCTION_REPO_DIR='${PRODUCTION_REPO_DIR}' PRODUCTION_PM2_BIN='${PRODUCTION_PM2_BIN}' PRODUCTION_PM2_HOME='${PRODUCTION_PM2_HOME}' bash -s" <<'REMOTE'
set -Eeuo pipefail
...
build_alpha_web() {
  NODE_ENV=production NOLO_BUILD_SHA="$BUILD_SHA" "$BUN_BIN" run build
}
...
main_web_release() {
  ...
  timed_phase "build-web" build_release_web
...
alpha_deploy() {
  ...
  timed_phase "build-web" build_alpha_web
