...
export async function ensureDemoUsersOnBases(args: {
  ...
  for (const baseUrl of bases) {
    try {
      ...
      // A down/unreachable LOCAL dev origin must not abort the whole sync when
      // a remote base (e.g. the user's authoritative server) is reachable.
      // Remote/required bases still surface their error to the caller.
      if (isLocalBaseUrl(baseUrl) && isConnectionFailure(error)) {
        continue;
      }
      throw error;
    }
  ...
  if (results.length === 0) {
    throw new Error("No reachable server base found for demo-user sync.");
  }

  return results;
}

/** True for network/connectivity failures (not auth/validation errors). */
function isConnectionFailure(error: unknown): boolean {
  const msg = toErrorMessage(error);
  return /(couldn't connect|unable to connect|failed to connect|connect refused|unreachable|failed to fetch|network|ECONN|ENOTFOUND|port unreachable)/i.test(
    msg
  );
}
