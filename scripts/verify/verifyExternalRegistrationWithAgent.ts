...
export function shouldRetryForegroundAfterStaleBackgroundFailure(snapshot: {
  ...
  const assistantCount =
    typeof checkpoint?.traceSummary?.assistantCount === "number"
      ? checkpoint.traceSummary.assistantCount
      : -1;
  const toolCount =
    typeof checkpoint?.traceSummary?.toolCount === "number"
      ? checkpoint.traceSummary.toolCount
      : -1;
  const executionMode =
