...
$minimumInstallerBytes = if ($env:NOLO_DESKTOP_MIN_INSTALLER_BYTES) {
  ...
  # The current clean Windows smoke installer is ~48.6 MB. Keep a meaningful
  # truncation guard while leaving headroom for normal compression drift.
  45000000 # 45 MB default
}
...
function Assert-InstalledDesktopSecurityBoundary {
  ...
  $devToolGuarded = [int]$devToolResponse.StatusCode -eq 403 -and $devToolBody -match "Forbidden: dev tool auth required"
  $devToolUnavailable = [int]$devToolResponse.StatusCode -eq 404
  if (-not ($devToolGuarded -or $devToolUnavailable)) {
    throw "Expected /api/read-file to be guarded or unavailable in installed desktop production runtime."
  }
}

function Assert-InstalledDesktopRouteMap {
  param([Parameter(Mandatory = $true)][string]$BaseUrl)

  $routeResponse = Invoke-WebRequest -Uri "$BaseUrl/api/desktop/clipboard" -Method Options -TimeoutSec 5 -UseBasicParsing -SkipHttpErrorCheck
  $allowedMethods = [string]$routeResponse.Headers["Access-Control-Allow-Methods"]
  if ([int]$routeResponse.StatusCode -ne 204 -or $allowedMethods -notmatch "POST") {
    throw "Expected the installed desktop clipboard route to expose POST via OPTIONS; got status $($routeResponse.StatusCode), methods '$allowedMethods'."
  }
}

function Wait-ForDesktopWindowStartup {
  param(
    [Parameter(Mandatory = $true)][string]$PrimaryLogPath,
    [Parameter(Mandatory = $true)][string]$FallbackLogPath,
  ...
  for ($i = 0; $i -lt 80; $i++) {
    $log = Read-SmokeLog -PrimaryPath $PrimaryLogPath -FallbackPath $FallbackLogPath
    if ($log -match "\[desktop\] BrowserWindow created" -and $log -match $expectedPublicDirPattern) {
      return
    }
    Start-Sleep -Milliseconds 500
  }
