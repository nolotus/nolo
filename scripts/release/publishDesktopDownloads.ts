...
async function verifyPublishedUrl(url: string, minBytes: number) {
  // R2 eventual consistency: a freshly PUT object may HEAD as 0 bytes or
  // 404 for the first second or two before it is read-consistent across
  // the edge. Retry with backoff instead of failing on the first probe.
  const maxAttempts = 5;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (response.ok) {
      ...
      lastError = new Error(
        `Published artifact is too small: ${url} (${contentLength} bytes < ${minBytes} bytes)`,
      );
    } else {
      lastError = new Error(`HEAD ${url} failed with ${response.status}`);
    }
