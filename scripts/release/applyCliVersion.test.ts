...
import { isVersionGreater, resolveCliVersionAction } from "./applyCliVersion";
...
describe("resolveCliVersionAction", () => {
  it("equal version is a no-op (idempotent rerun)", () => {
    expect(resolveCliVersionAction("0.6.1-alpha.1", "0.6.1-alpha.1")).toBe("noop");
  });

  it("higher candidate is a bump", () => {
    expect(resolveCliVersionAction("0.6.1-alpha.2", "0.6.1-alpha.1")).toBe("bump");
  });

  it("lower candidate is a downgrade", () => {
    expect(resolveCliVersionAction("0.6.1-alpha.1", "0.6.1-alpha.2")).toBe("downgrade");
  });
});