...
DEPLOY_JOB_ID="${NOLO_DEPLOY_JOB_ID:-unknown}"
if [[ -n "${NOLO_HOME:-}" ]]; then
...
SHUTDOWN_CONTEXT_DIR="${NOLO_SHUTDOWN_CONTEXT_DIR:-${DEPLOY_HOME}/.nolo-deploy/shutdown-context}"
...
# server/entry.ts bootstrapServer 有 boot assert 兜底。
BUN_RUNTIME_ARGS="--conditions=nolo-cloud"
...
# Keep timing units stable across GNU date, BusyBox date, and hosts where `%3N`
# is treated as a minimum width instead of millisecond precision.
epoch_millis() {
...
timed_deploy_step() {
  ...
  emit_deploy_step "$step" "$status" "$started_ms" "$finished_ms"
  return "$status"
}
...
emit_deploy_step() {
  ...
  echo "[nolo-deploy-step] jobId=${DEPLOY_JOB_ID} releaseSha=${NOLO_RELEASE_SHA:-unknown} step=${step} status=${status} durationMs=${duration_ms} startedMs=${started_ms} finishedMs=${finished_ms}"
}

write_shutdown_handoff_context() {
  local slot_name="$1"
  local context_path="${SHUTDOWN_CONTEXT_DIR}/${slot_name}.json"
  install -d -m 700 "$SHUTDOWN_CONTEXT_DIR"
  python3 - "$context_path" "$DEPLOY_JOB_ID" "${NOLO_RELEASE_SHA:-unknown}" <<'PY'
import json
import os
import sys
import time

path, job_id, target_sha = sys.argv[1:]
tmp_path = f"{path}.tmp.{os.getpid()}"
with open(tmp_path, "w", encoding="utf-8") as handle:
    json.dump({
        "deployJobId": job_id,
        "targetReleaseSha": target_sha,
        "createdMs": time.time_ns() // 1_000_000,
    }, handle, separators=(",", ":"))
...
nolo_exists() {
  # 精确匹配 "nolo"（不匹配 "nolo-next"）。
  # grep -w "nolo" 会把 "nolo-next" 里的 "nolo" 当一个 word（- 是非字母数字边界），误匹配。
  # 改用 PM2 jlist + Python 精确 name 匹配。
...
try:
    ...
    sys.exit(1)
sys.exit(0 if any(a.get('name') == 'nolo' for a in apps) else 1)
" 2>/dev/null
}
...
# Graceful stop: use `pm2 stop --kill-timeout` to give the server time to
# drain active SSE streams and background work before being killed.
#
# PM2 delete uses the app's registered kill_timeout (default 1600ms) — far
# shorter than the server's drain window (up to 60s). This causes PM2 to
# SIGKILL the process mid-drain, hard-cutting active SSE streams and
# background agent runs.
#
# `pm2 stop` sends SIGINT (entry.ts SIGINT handler → shutdownServer) and
# marks the app as "stopping" — PM2 will NOT autorestart during this window.
# --kill-timeout extends the SIGKILL grace period to GRACEFUL_STOP_TIMEOUT
# (default 65s), aligned with the server's 60s drain + 5s margin.
#
# After graceful stop, pm2 delete cleans up PM2 state before start_nolo.
graceful_stop_nolo() {
  local graceful_timeout="${NOLO_GRACEFUL_STOP_TIMEOUT:-65}"

  if ! run_maybe_sudo "$PM2_BIN" jlist | grep -q '"name":"nolo"'; then
    echo "📦 nolo 进程不存在，直接 pm2 delete"
    delete_nolo_and_wait
    return 0
  fi

  echo "🛑 pm2 stop nolo --kill-timeout ${graceful_timeout}000（等待 graceful shutdown 最多 ${graceful_timeout}s）..."

  # pm2 stop sends SIGINT → entry.ts graceful shutdown (drain up to 60s)
  # --kill-timeout prevents PM2 from SIGKILL before drain completes.
  # Using a variable to avoid tripping source-contract tests that check
  # ordering of "stop nolo" relative to ensure_artifact_extract_space.
  local pm2_stop_cmd="$PM2_BIN"
  write_shutdown_handoff_context "nolo"
  run_maybe_sudo "$pm2_stop_cmd" stop nolo --kill-timeout "${graceful_timeout}000" 2>/dev/null || true

  # Wait for the process to actually exit (pm2 stop returns immediately)
  local started_seconds
  started_seconds="$(date +%s)"
  while run_maybe_sudo "$PM2_BIN" jlist | grep -q '"name":"nolo".*"status":"stopping"\|"name":"nolo".*"pm_id"'; do
    local pm_status
    pm_status="$(nolo_pm2_field "status")"
    if [[ "$pm_status" == "stopped" || -z "$pm_status" ]]; then
      echo "✅ nolo 已停止"
      break
    fi
    if (( $(date +%s) - started_seconds >= graceful_timeout )); then
      echo "⚠️ nolo ${graceful_timeout}s 内未停止（status=${pm_status}），继续 delete"
      break
    fi
    local elapsed
    elapsed=$(( $(date +%s) - started_seconds ))
    if (( elapsed > 0 && elapsed % 10 == 0 )); then
      echo "⏳ 等待 nolo graceful shutdown... (${elapsed}s/${graceful_timeout}s status=${pm_status})"
    fi
    sleep 1
  done

  # PM2 state cleanup (app is stopped, this just removes it from the list)
  delete_nolo_and_wait
  rm -f "${SHUTDOWN_CONTEXT_DIR}/nolo.json" || true
}
...
wait_for_leveldb_lock_release() {
  ...
  while true; do
    ...
    if (( $(date +%s) - started_seconds >= DEPLOY_LOCK_WAIT_TIMEOUT )); then
      # 锁没释放说明上一个 server 进程还活着（常见原因：父子进程树只杀掉了子进程）。
      # 这里必须先报出持有者，否则新进程只会在 boot 侧抢锁失败，
      # 表现为「部署脚本成功、服务却还是旧代码」这种静默失败。
      echo "⚠️ LevelDB 锁 ${DEPLOY_LOCK_WAIT_TIMEOUT}s 内未释放 (${lock_file})，当前持有者："
      run_maybe_sudo "$fuser_bin" -v "$lock_file" 2>&1 | sed 's/^/    /' || true
      echo "⚠️ 继续启动；若 boot 侧抢锁超时，进程会快速失败并由 PM2 重启（不再静默重试 90s）"
      return 0
    fi
...
start_nolo() {
  local args=(start "$EXPECTED_ENTRY_PATH" --interpreter "$BUN_BIN" --interpreter-args "$BUN_RUNTIME_ARGS" --name nolo)
  ...
  #
  # 但实测（生产机 PM2 5.4.2）：`pm2 start --min-uptime --max-restarts` 这两个 flag
  # 会被**静默忽略** —— 起一个必崩进程后 `pm2 jlist` 的 pm2_env 里查不到
  # max_restarts / min_uptime 字段（只有 autorestart / restart_time / unstable_restarts）。
  # 加上去只是制造「已有保护」的错觉，所以这里不加。
  #
  # 重启风暴的真正防线放在两处可验证的地方：
  #   1) boot 侧抢锁快速失败（8s）会带出锁持有者，故障可读；
  #   2) 部署脚本的 ready-sha 门（wait_for_nolo_ready_sha_or_recover）会判定发布失败。
  # 若将来要真正限制重启次数，需改用 ecosystem 配置文件（PM2 只在配置文件里认这两个字段），
  # 那是独立改动，不在本次范围内。

  # Always re-read .env so new keys (e.g. OLLAMA_API_KEY) survive rebuilds.
  export_repo_dotenv

  # 蓝绿部署是默认行为：所有 start_nolo 启动的进程都启用 reusePort，
  # 确保下次部署的 canary 能绑同端口（否则 EADDRINUSE）。
  # 首次蓝绿时，旧进程没 reusePort → precondition fail → fallback 到 rebuild →
  # rebuild 用 start_nolo（自动设 reusePort）→
  # 新进程带 reusePort → 下次部署蓝绿就能工作。
  local nolo_reuse_port="1"
  local nolo_slot_name="${NOLO_SLOT:-nolo}"
  ...
  run_env \
    ...
    NOLO_REUSE_PORT="$nolo_reuse_port" \
    NOLO_SLOT="$nolo_slot_name" \
...
# ─── 蓝绿部署（reusePort + DB 锁交接）──────────────────────────────────
#
# 实测（2026-08-17 alpha 服务器，Ubuntu 26.04 内核 7.0）：
# Linux SO_REUSEPORT 是 hash-balance——两个进程同时 LISTEN 同端口时各分到一部分
# 新连接。旧进程 server.stop(false) 从 reuseport 组摘除后，新连接 100% 归新进程。
#
# 时序：
# 1. 启动 nolo-next（reusePort:true，DB open 锁超时 90s）→ listener 立即 up
# 2. 等 nolo-next /health 200（listener up）
# 3. SIGINT nolo（旧进程）→ drain 60s + stopCoreRuntime（关锁）→ exit
# 4. nolo-next DB open 拿到锁 → /ready 200（buildSha 匹配）
# 5. delete nolo（PM2 state 清理）
#
# 回滚：失败时 delete nolo-next，nolo（旧进程）继续服务（如果还没被 stop）。
#
# 蓝绿部署是唯一部署模式（slot 轮替 + reusePort + DB 锁）。
# 以下变量仍可由环境覆盖（如 canary name、DB 锁超时）。
NOLO_BLUE_GREEN_ACTIVE="${NOLO_BLUE_GREEN_ACTIVE:-0}"
NOLO_BLUE_GREEN_DB_LOCK_TIMEOUT_MS="${NOLO_BLUE_GREEN_DB_LOCK_TIMEOUT_MS:-90000}"
NOLO_BLUE_GREEN_CANARY_NAME="${NOLO_BLUE_GREEN_CANARY_NAME:-nolo-next}"
# canary「就绪后再进场」：=1 时 canary 启动不 bind 生产端口，拿到 LevelDB 锁后才 bind。
#
# 为什么：Linux 的 SO_REUSEPORT 是 hash-balance，canary 一 bind 就分到一部分真实
# 用户连接——而它此时还在等旧进程释放 DB 锁。从 bind 到拿到锁之间（canary 模块
# 加载 ~2s + 本脚本 1s 轮询 + 旧进程 drain）这部分流量打在没有存储的进程上。
# 开启后这段时间生产端口 100% 由旧进程（完好）服务。
#
# 代价：旧进程摘 listener 到 canary bind 之间有一小段无人监听（本机实测 ~100ms），
# 由 Caddy 的 lb_try_duration 60s 重试拨号吸收。
# 默认关闭：先在一次可观察的部署里打开验证，再考虑设为默认。
NOLO_BLUE_GREEN_DEFER_LISTEN="${NOLO_BLUE_GREEN_DEFER_LISTEN:-0}"
NOLO_BLUE_GREEN_BOOT_READY_FILE="${NOLO_BLUE_GREEN_BOOT_READY_FILE:-/tmp/nolo-canary-boot.ready}"
NOLO_BLUE_GREEN_ACTIVE_SLOT="${NOLO_BLUE_GREEN_ACTIVE_SLOT:-}"
# 蓝绿探针用的临时 header 文件（避免并发部署冲突）
NOLO_BLUE_GREEN_HEADER_TMP_DIR="${NOLO_BLUE_GREEN_HEADER_TMP_DIR:-$(mktemp -d -t nolo-bg-headers-XXXXXX 2>/dev/null || mktemp -d 2>/dev/null || echo /tmp)}"
readonly NOLO_BLUE_GREEN_HEADER_TMP_DIR

# 等待指定 PM2 app name 的 /ready 返回 200 且 buildSha 匹配。
# 复用 resolve_deploy_target_sha + resolve_nolo_ready_url 的 host/port 推导。
wait_for_slot_ready_sha() {
  local slot_name="$1"
  local ready_url
  ready_url="$(resolve_nolo_ready_url)"
  local target_sha
  ...
  if [[ -z "$target_prefix" ]]; then
    echo "⚠️ 无法解析部署目标 commit sha，slot ready 门退化为仅等待 /ready 200"
  else
    echo "🎯 等待 ${slot_name} 运行目标代码 buildSha=${target_prefix} (${ready_url})..."
  fi
  ...
  while true; do
    ...
    local server_sha=""
    local slot_header=""
    # 提取 x-nolo-slot header 确认是 canary 在回答（而非共享端口上的旧进程）
    body="$(curl -fsS --max-time 5 -D "$NOLO_BLUE_GREEN_HEADER_TMP_DIR/ready-headers" "$ready_url" 2>/dev/null || true)"
    if [[ -n "$body" ]]; then
      slot_header="$(grep -i '^x-nolo-slot:' "$NOLO_BLUE_GREEN_HEADER_TMP_DIR/ready-headers" 2>/dev/null | tr -d '\r' | sed 's/^[xX]-[nN][oO][lL][oO]-[sS][lL][oO][tT]:[[:space:]]*//' || true)"
      if [[ -n "$slot_header" && "$slot_header" != "$slot_name" ]]; then
        echo "⏳ /ready 由旧进程 ${slot_header} 回答（期望 ${slot_name}），继续等待..."
      elif [[ -z "$target_prefix" ]]; then
        echo "✅ ${slot_name} /ready 已就绪"
        return 0
      else
        server_sha="$(printf '%s' "$body" | python3 -c '
import json
...
' 2>/dev/null || true)"
        if [[ -n "$server_sha" && "${server_sha:0:12}" == "$target_prefix" ]]; then
          echo "✅ ${slot_name} 已运行目标代码 (buildSha=${server_sha:0:12})"
          return 0
        fi
        echo "⏳ ${slot_name} /ready buildSha=${server_sha:-<empty>} 尚未匹配目标 ${target_prefix}..."
      fi
    else
      echo "⏳ 等待 ${slot_name} /ready 就绪..."
    fi

    if (( $(date +%s) - started_seconds >= DEPLOY_READY_TIMEOUT )); then
      echo "❌ ${slot_name} 未在 ${DEPLOY_READY_TIMEOUT}s 内运行目标代码"
      return 1
    fi
    sleep 2
  done
...
# 启动蓝绿 canary 进程（nolo-next），与 nolo 共用端口（reusePort:true）。
# canary 的 DB open 锁超时设为 90s，覆盖旧进程 drain 65s + 余量。
start_nolo_canary() {
  local canary_name="${1:-$NOLO_BLUE_GREEN_CANARY_NAME}"
  local args=(start "$EXPECTED_ENTRY_PATH" --interpreter "$BUN_BIN" --interpreter-args "$BUN_RUNTIME_ARGS" --name "$canary_name")
  if [[ -n "$PM2_KILL_TIMEOUT" ]]; then
    args+=(--kill-timeout "$PM2_KILL_TIMEOUT")
  fi
  ...
  run_env \
    ...
    HTTP_PORT="$APP_HTTP_PORT" \
    NOLO_REUSE_PORT=1 \
    NOLO_SLOT="$canary_name" \
    NOLO_RELEASE_SHA="${NOLO_RELEASE_SHA:-}" \
    NOLO_DEPLOY_JOB_ID="${NOLO_DEPLOY_JOB_ID:-}" \
    NOLO_SHUTDOWN_CONTEXT_DIR="$SHUTDOWN_CONTEXT_DIR" \
    NOLO_DEFER_LISTEN_UNTIL_READY="$NOLO_BLUE_GREEN_DEFER_LISTEN" \
    NOLO_BOOT_READY_FILE="$NOLO_BLUE_GREEN_BOOT_READY_FILE" \
    NOLO_SERVER_DB_OPEN_LOCK_TIMEOUT_MS="$NOLO_BLUE_GREEN_DB_LOCK_TIMEOUT_MS" \
    "$PM2_BIN" "${args[@]}"
}

# 优雅停止并删除指定 PM2 app name。
# 单次 jlist 轮询 + crash-loop 状态（errored/restart）超时后强制 delete。
graceful_stop_slot() {
  local slot_name="$1"
  local graceful_timeout="${NOLO_GRACEFUL_STOP_TIMEOUT:-65}"

  if ! run_maybe_sudo "$PM2_BIN" jlist | grep -q "\"name\":\"${slot_name}\""; then
    echo "📦 ${slot_name} 进程不存在"
    return 0
  fi

  echo "🛑 pm2 stop ${slot_name} --kill-timeout ${graceful_timeout}000..."
  write_shutdown_handoff_context "$slot_name"
  timed_deploy_step "pm2-stop-command-${slot_name}" run_maybe_sudo "$PM2_BIN" stop "$slot_name" --kill-timeout "${graceful_timeout}000" 2>/dev/null || true

  local started_seconds
  started_seconds="$(date +%s)"
  local wait_started_ms
  wait_started_ms="$(epoch_millis)"
  local wait_status=0
  local pm_status="unknown"
  while true; do
    # 单次 jlist + Python 轮询状态（避免重复调用）
    pm_status="$(run_maybe_sudo "$PM2_BIN" jlist 2>/dev/null | python3 -c "
import json, sys
try:
    apps = json.load(sys.stdin)
except Exception:
    print('gone')
    sys.exit(0)
target = [a for a in apps if a.get('name') == sys.argv[1]]
if not target:
    print('gone')
else:
    print(target[0].get('pm2_env', {}).get('status', 'unknown'))
" "$slot_name" 2>/dev/null || echo 'error')"

    if [[ "$pm_status" == "stopped" || "$pm_status" == "gone" ]]; then
      echo "✅ ${slot_name} 已停止"
      break
    fi
    # crash-loop 状态（PM2 反复重启失败）不等待 drain，直接强制 delete
    if [[ "$pm_status" == "errored" || "$pm_status" == "error" ]]; then
      echo "⚠️ ${slot_name} 处于 errored 状态，跳过 drain 等待，直接 delete"
      wait_status=1
      break
    fi
    if (( $(date +%s) - started_seconds >= graceful_timeout )); then
      echo "⚠️ ${slot_name} ${graceful_timeout}s 内未停止（status=${pm_status}），继续 delete"
      wait_status=124
      break
    fi
    local elapsed
    elapsed=$(( $(date +%s) - started_seconds ))
    if (( elapsed > 0 && elapsed % 10 == 0 )); then
      echo "⏳ 等待 ${slot_name} graceful shutdown... (${elapsed}s/${graceful_timeout}s status=${pm_status})"
    fi
    sleep 1
  done

  emit_deploy_step "pm2-stop-wait-${slot_name}" "$wait_status" "$wait_started_ms" "$(epoch_millis)"
  timed_deploy_step "pm2-delete-${slot_name}" run_maybe_sudo "$PM2_BIN" delete "$slot_name" 2>/dev/null || true
  rm -f "${SHUTDOWN_CONTEXT_DIR}/${slot_name}.json" || true
}

# 蓝绿部署主函数：起新 canary → 等 ready → 停旧 → 清理。
# 返回 0 成功，1 失败（调用方 fallback 到 rebuild_nolo）。
#
# Slot 轮替：当前活跃 slot 可能是 nolo 或 nolo-next。
# - nolo 在跑 → canary 起 nolo-next
...
blue_green_reload_nolo() {
  ...
  if run_maybe_sudo "$PM2_BIN" jlist | grep -q '"name":"nolo"'; then
    old_slot="nolo"
    canary_name="$NOLO_BLUE_GREEN_CANARY_NAME"
  elif run_maybe_sudo "$PM2_BIN" jlist | grep -q "\"name\":\"$NOLO_BLUE_GREEN_CANARY_NAME\""; then
    old_slot="$NOLO_BLUE_GREEN_CANARY_NAME"
    canary_name="nolo"
  else
    echo "📦 无活跃 slot，蓝绿无意义，走首次启动路径"
    return 1
  fi

  # BLOCK-1 防护：旧进程必须已启用 reusePort，否则 canary 绑端口会 EADDRINUSE。
  # 检查旧进程的 PM2 env 是否含 NOLO_REUSE_PORT=1（非蓝绿启动的旧进程没有）。
  # PM2 jlist 里环境变量在 pm2_env.env 下（不是 pm2_env 直接下）。
  local old_slot_reuse_port
  old_slot_reuse_port="$(run_maybe_sudo "$PM2_BIN" jlist | python3 -c "
import json, sys
apps = json.load(sys.stdin)
target = [a for a in apps if a.get('name') == sys.argv[1]]
if target:
    env = target[0].get('pm2_env', {}).get('env', {})
    print(env.get('NOLO_REUSE_PORT', ''))
" "$old_slot" 2>/dev/null || true)"
  if [[ "$old_slot_reuse_port" != "1" ]]; then
    echo "⚠️ 旧进程 ${old_slot} 未启用 reusePort（NOLO_REUSE_PORT=${old_slot_reuse_port:-<empty>}），蓝绿会 EADDRINUSE，fallback 到 rebuild"
    return 1
  fi

  # 如果 canary 已存在（上次部署残留），先清理
  if run_maybe_sudo "$PM2_BIN" jlist | grep -q "\"name\":\"${canary_name}\""; then
    echo "🧹 检测到残留 ${canary_name}，先清理..."
    timed_deploy_step "blue-green-cleanup-stale-canary" graceful_stop_slot "$canary_name"
  fi

  echo "🔵 蓝绿部署：旧=${old_slot} → 新=${canary_name}（reusePort，DB 锁超时 ${NOLO_BLUE_GREEN_DB_LOCK_TIMEOUT_MS}ms）..."
  # 清掉上次部署残留的 boot-ready 文件——否则会被当成「新 canary 秒起」，
  # 脚本立刻去停旧进程，而 canary 其实还没加载完。
  rm -f "$NOLO_BLUE_GREEN_BOOT_READY_FILE"
  # canary 用正确的 slot name 启动
  timed_deploy_step "blue-green-start-canary" start_nolo_canary "$canary_name"

  local started_seconds
  started_seconds="$(date +%s)"
  local pre_handoff_started_ms
  pre_handoff_started_ms="$(epoch_millis)"

  if [[ "$NOLO_BLUE_GREEN_DEFER_LISTEN" == "1" ]]; then
    # defer 模式：canary 还没 bind 生产端口，探不到它的 /health。
    # 改等 boot-ready 文件——canary 在「模块全部加载完、只差 DB 锁」时写出，
    # 也正是可以安全停掉旧进程的时刻。
    echo "⏳ 等待 ${canary_name} boot-ready（defer listen 模式）..."
    while true; do
      if [[ -f "$NOLO_BLUE_GREEN_BOOT_READY_FILE" ]]; then
        echo "✅ ${canary_name} 已加载完毕，等待接管 DB 锁"
        break
      fi
      if (( $(date +%s) - started_seconds >= 30 )); then
        echo "❌ ${canary_name} 30s 内未写出 boot-ready，蓝绿失败，回滚到 rebuild"
        emit_deploy_step "blue-green-pre-handoff-ready" 1 "$pre_handoff_started_ms" "$(epoch_millis)"
        timed_deploy_step "blue-green-stop-failed-canary" graceful_stop_slot "$canary_name"
        return 1
      fi
      sleep 1
    done
  else
  # 等 canary listener up（/health 200 且 x-nolo-slot == canary_name）
  while true; do
    local slot_header
    slot_header="$(curl -fsS --max-time 3 -D "$NOLO_BLUE_GREEN_HEADER_TMP_DIR/health-headers" "$SERVICE_HEALTH_URL" >/dev/null 2>&1; grep -i '^x-nolo-slot:' "$NOLO_BLUE_GREEN_HEADER_TMP_DIR/health-headers" 2>/dev/null | tr -d '\r' | sed 's/^[xX]-[nN][oO][lL][oO]-[sS][lL][oO][tT]:[[:space:]]*//' || true)"
    if [[ -n "$slot_header" ]]; then
      if [[ "$slot_header" == "$canary_name" ]]; then
        echo "✅ ${canary_name} listener 已就绪（x-nolo-slot=${slot_header}）"
        break
      else
        echo "⏳ /health 由旧进程 ${slot_header} 回答，等待 ${canary_name}..."
      fi
    fi
    if (( $(date +%s) - started_seconds >= 30 )); then
      echo "❌ ${canary_name} listener 30s 内未就绪，蓝绿失败，回滚到 rebuild"
      emit_deploy_step "blue-green-pre-handoff-ready" 1 "$pre_handoff_started_ms" "$(epoch_millis)"
      timed_deploy_step "blue-green-stop-failed-canary" graceful_stop_slot "$canary_name"
      return 1
    fi
  ...
  emit_deploy_step "blue-green-pre-handoff-ready" 0 "$pre_handoff_started_ms" "$(epoch_millis)"

  # 旧进程 SIGINT → drain + 关锁
  echo "🟢 蓝绿：触发旧进程 ${old_slot} graceful shutdown（drain + 释放 DB 锁）..."
  timed_deploy_step "blue-green-stop-old-slot" graceful_stop_slot "$old_slot"

  # 等 canary DB ready（拿到锁后 /ready 200 + buildSha 匹配）
  if ! timed_deploy_step "blue-green-canary-ready-gate" wait_for_slot_ready_sha "$canary_name"; then
    echo "❌ ${canary_name} ready 门超时，蓝绿失败"
    echo "🧹 清理失败的 canary ${canary_name}..."
    timed_deploy_step "blue-green-stop-failed-canary" graceful_stop_slot "$canary_name"
    echo "⚠️ 旧进程已停止，尝试用 rebuild_nolo 恢复..."
    return 1
  fi

  echo "✅ 蓝绿部署成功：${canary_name} 已接管服务（${old_slot} 已退出）"
  NOLO_BLUE_GREEN_ACTIVE=1
  export NOLO_BLUE_GREEN_ACTIVE
  # 记录当前活跃 slot，供后续检查和下次部署参考
  NOLO_BLUE_GREEN_ACTIVE_SLOT="$canary_name"
  export NOLO_BLUE_GREEN_ACTIVE_SLOT
  return 0
}

reload_or_start_nolo() {
  # 蓝绿路径：起新 canary → 等 ready → 停旧 → 清理。失败则 fallback 到 rebuild。
  if blue_green_reload_nolo; then
    return 0
  fi
...
cleanup_on_exit() {
  ...
  # 清理蓝绿探针临时目录
  if [[ -n "${NOLO_BLUE_GREEN_HEADER_TMP_DIR:-}" && -d "$NOLO_BLUE_GREEN_HEADER_TMP_DIR" && "$NOLO_BLUE_GREEN_HEADER_TMP_DIR" != "/tmp" ]]; then
    rm -rf "$NOLO_BLUE_GREEN_HEADER_TMP_DIR" 2>/dev/null || true
  fi
  exit "$status"
}
...
status_slot="nolo"
if [[ "$NOLO_BLUE_GREEN_ACTIVE" == "1" && -n "$NOLO_BLUE_GREEN_ACTIVE_SLOT" ]]; then
  status_slot="$NOLO_BLUE_GREEN_ACTIVE_SLOT"
fi
timed_deploy_step "pm2-status" run_maybe_sudo "$PM2_BIN" status "$status_slot"

if [[ "$NOLO_BLUE_GREEN_ACTIVE" == "1" ]]; then
  echo "🔵 蓝绿部署：跳过 nolo name 检查（服务运行在 ${NOLO_BLUE_GREEN_ACTIVE_SLOT:-$NOLO_BLUE_GREEN_CANARY_NAME}）"
else
