...
start_nolo() {
  ...
  # 蓝绿部署：当 NOLO_BLUE_GREEN=1 时，所有 start_nolo 启动的进程都启用 reusePort，
  # 确保下次部署的 canary 能绑同端口（否则 EADDRINUSE）。
  # 首次启用蓝绿时，旧进程没 reusePort → precondition fail → fallback 到 rebuild →
  # rebuild 用 start_nolo（此时 NOLO_BLUE_GREEN=1，自动设 reusePort）→
  # 新进程带 reusePort → 下次部署蓝绿就能工作。
  local nolo_reuse_port="${NOLO_REUSE_PORT:-}"
  if [[ "$NOLO_BLUE_GREEN" == "1" ]]; then
    nolo_reuse_port="1"
  fi
...
#
# 环境变量 NOLO_BLUE_GREEN=1 启用蓝绿，默认关闭（向后兼容）。
NOLO_BLUE_GREEN="${NOLO_BLUE_GREEN:-0}"
...
start_nolo_canary() {
  local canary_name="$NOLO_BLUE_GREEN_CANARY_NAME"
...
graceful_stop_slot() {
  ...
  run_maybe_sudo "$PM2_BIN" stop "$slot_name" --kill-timeout "${graceful_timeout}000" 2>/dev/null || true
...
" "$slot_name" 2>/dev/null || echo 'error')"
  ...
  run_maybe_sudo "$PM2_BIN" delete "$slot_name" 2>/dev/null || true
}
...
" "$old_slot" 2>/dev/null || true)"
  ...
  graceful_stop_slot "$old_slot"

  # 等 canary DB ready（拿到锁后 /ready 200 + buildSha 匹配）
  if ! wait_for_slot_ready_sha "$canary_name"; then
...
reload_or_start_nolo() {
  # 蓝绿优先路径（NOLO_BLUE_GREEN=1 时启用）：
  # 起新 canary → 等 ready → 停旧 → 清理。失败则 fallback 到 rebuild。
...
status_slot="nolo"
if [[ "$NOLO_BLUE_GREEN" == "1" && "$NOLO_BLUE_GREEN_ACTIVE" == "1" && -n "$NOLO_BLUE_GREEN_ACTIVE_SLOT" ]]; then
  status_slot="$NOLO_BLUE_GREEN_ACTIVE_SLOT"
fi
timed_deploy_step "pm2-status" run_maybe_sudo "$PM2_BIN" status "$status_slot"

if [[ "$NOLO_BLUE_GREEN" == "1" && "$NOLO_BLUE_GREEN_ACTIVE" == "1" ]]; then
