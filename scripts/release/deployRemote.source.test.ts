...
describe("deployRemote source contract", () => {
  ...
  it("implements blue-green reload with reusePort slot rotation when NOLO_BLUE_GREEN=1", () => {
    // 蓝绿总开关
    expect(source).toContain('NOLO_BLUE_GREEN="${NOLO_BLUE_GREEN:-0}"');
    ...
    // BLOCK-1 鸡生蛋修复：NOLO_BLUE_GREEN=1 时 start_nolo 自动设 reusePort
    expect(source).toContain('if [[ "$NOLO_BLUE_GREEN" == "1" ]]; then');
    expect(source).toContain('nolo_reuse_port="1"');
  ...
  it("adds x-nolo-slot header to health and ready endpoints for blue-green slot identification", () => {
    // entry.ts 读取 NOLO_SLOT 环境变量
    const entrySource = readFileSync(
      join(import.meta.dir, "..", "..", "packages", "server", "entry.ts"),
      "utf-8"
    );
    expect(entrySource).toContain('process.env.NOLO_SLOT');
