...
import { DEFAULT_THEME_NAME, SPACE, THEME_COLORS } from "app/theme/theme.config";
import {
  DEFAULT_FONT_PRESET,
    ...
    return {
      ...
      popover: (() => {
        ...
        return {
          ...
          shadow: [
            `0 18px 44px -24px ${c.shadowHeavy}`,
            `0 4px 12px -6px ${c.shadowMedium}`,
            `0 1px 1px -0.5px ${c.shadowLight}`,
          ].join(", "),
          radius,
          pad,
