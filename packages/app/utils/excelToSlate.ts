...
/**
 * 对某些列，我们「不信任 Excel 的格式化结果（displayValue）」，
 * 而是强制以原始数值 rawValue 为准。
 *
 * 当前只包含「定金」，后续有需要可以在这里扩展。
 */
const FORCE_RAW_HEADERS = ["定金"];
const PERCENT_RATIO_HEADER_KEYWORDS = [
  "%",
  "％",
  "百分比",
  "比例",
...
const shouldForceUseRawValue = (header: string): boolean => {
  const normalized = (header ?? "").trim();
  if (!normalized) return false;

  // 使用 includes 以兼容「定金(%)」「定金比例」等表头形式
  return FORCE_RAW_HEADERS.some((name) => normalized.includes(name));
};

const isPercentRatioHeader = (header: string): boolean => {
  const normalized = asTrimmedLowercaseString(header);
  if (!normalized) return false;
  return PERCENT_RATIO_HEADER_KEYWORDS.some((keyword) =>
...
const createIsDateHeaderFromKeywords = (keywords: string[]) => {
  const loweredKeywords = keywords.flatMap((k) => {
    if (!k) return [];
    const lowered = asTrimmedLowercaseString(k);
    return lowered.length > 0 ? [lowered] : [];
  });

  return (header: string): boolean => {
    const lower = asTrimmedLowercaseString(header);
...
/**
 * 将比例数值转成「百分比字符串」，尽量保留有效小数，而不是固定两位。
 * 例如：
 *   0.3443   -> "34.43%"
 *   0.1      -> "10%"
 *   0.123456789 -> "12.3456789%"
 *
 * 为了避免 JS 浮点误差，内部用较高精度 toFixed 再去掉多余 0。
 */
const formatRatioAsPercent = (ratio: number): string => {
  ...
  // 这里给一个较大的上限，例如 12 位小数，基本够实际业务使用
  const MAX_DECIMALS = 12;
  let str = percent.toFixed(MAX_DECIMALS);
...
/**
 * 非日期列的值格式化逻辑：
 * - 对某些「强制使用原始值」的列（如「定金」），用 rawValue 当作比例并转为百分比字符串；
 *   例如 rawValue = 0.3443 -> "34.43%"，不截断到两位。
 * - 对其他列：
 *   - 如果提供了 displayValue（来自 raw:false 的 sheet_to_json），优先使用它：
 *     这样可以保留百分比 / 货币 / 时间 等 Excel 的显示样式。
 *   - 否则退回到 String(rawValue)。
 */
const formatNonDateCellValue = (
  header: string,
  rawValue: any,
  displayValue: any
): string => {
  // 1. 对特殊列（例如「定金」），强制使用原始数值，并按比例转为百分比
  if (shouldForceUseRawValue(header)) {
    if (rawValue == null) return "";

    if (typeof rawValue === "number") {
      const n = rawValue;

      // 约定：在 [0,1] 范围内的数当作比例，转成百分比
      if (n >= 0 && n <= 1) {
        return formatRatioAsPercent(n);
      }

      // 不在 [0,1]，按普通数值展示（比如定金金额，不是比例）
      return String(n);
    }
  ...
  // 2. 其他列：优先采用 Excel 的显示文本（displayValue）
  ...
  if (
    ...
    rawValue >= 0 &&
    rawValue <= 1 &&
    isPercentRatioHeader(header)
  ) {
    return formatRatioAsPercent(rawValue);
  }
 ...
 * - 其他列：优先用 displayValue（Excel 的格式化结果），
 *   但对某些特殊列（如「定金」）强制使用 rawValue 并转为百分比。
 */
const formatCellValue = (
  ...
  if (headerIsDate) {
    ...
    // 原始值无法解析成日期时，退回「非日期列」逻辑
    return formatNonDateCellValue(header, rawValue, displayValue);
  }

  // 2. 非日期列：Excel 显示文本 + 特殊列强制 raw
  return formatNonDateCellValue(header, rawValue, displayValue);
};
