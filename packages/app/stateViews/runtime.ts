...
// 经 identity 读取，而不是自己读 Redux state 形状：identity 是 edition 注入点，
// 这里若另起一套直读，开源换 edition 时就有两个地方要改，必然漏一个。
// 外层的类型收窄（非 string 一律 undefined）是本模块 RuntimeSnapshot 的既有契约，
// 予以保留。
// Phase 5：identity selector 读显式绑定的 AccountSessionCore，不依赖 Redux auth
// slice（该 slice 已删除）。未绑定运行时的环境（SSR、局部 state）返回未登录值。
