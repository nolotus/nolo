...
const loadQuickChat = async () => {
  ...
  mock.module("react-i18next", () => ({
    ...
    useTranslation: () => ({
      t: (key: string, fallbackOrOptions?: unknown) => {
        ...
        if (key === "quickChat.placeholderBalanced") return "用 DeepSeek Pro 深度推理";
        if (key === "quickChat.placeholderQuality") return "用 GLM 5.2 深入分析";
        return key;
      },
      i18n: { language: "zh-CN" },
    }),
  ...
  mock.module("app/favorite/favoriteStore", () => ({
    ...
    initFavorites: () => ({ type: "favorite/init" }),
  }));
