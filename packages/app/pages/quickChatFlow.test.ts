...
describe("quickChatFlow", () => {
  ...
  it("only lets the default tier be model-overridden", () => {
    // 三档指向同一个 agent，逐档断言只是把同一件事写三遍。
    for (const tier of ["flash", "balanced", "quality"] as const) {
      expect(allowsQuickChatModelOverride(QUICK_CHAT_DEFAULT_TIER_AGENTS[tier])).toBe(
        true,
      );
    }
    // 专职 agent 有自己的模型要求，不适用模型层覆盖。
    expect(allowsQuickChatModelOverride(BUILTIN_FEEDBACK_AGENT_KEY)).toBe(false);
    expect(allowsQuickChatModelOverride("agent-0e95801d90-someone-else")).toBe(false);
  });
