...
import { selectCurrentSpaceId, selectSpaceById } from "create/space/spaceSlice";
...
const QuickChatRuntime: React.FC<QuickChatRuntimeProps> = ({
  ...
  // 三档 agent 已改为写死常量(QUICK_CHAT_DEFAULT_TIER_AGENTS),不再走用户设置;
  // 有图无图都走 flash 档（图片由预处理管道处理）。
  ...
  const storeSpaceId = useAppSelector(selectCurrentSpaceId) || undefined;
  ...
  const startQuickChat = useCallback(async (overrideText?: string) => {
    ...
    try {
      ...
      const quickChatModelOverride =
        autoOverrideAgent && QUICK_CHAT_GENERAL_TIER_AGENT_KEYS.has(effectiveAgentId)
      ...
      const runtimeOptions: AgentRuntimeOptions = {
        ...
        ...(quickChatModelOverride
          ...
          : {}),
        ...(!specialistAgentId &&
          effectiveAgentId === PUBLIC_DEEPSEEK_V4_FLASH_AGENT_KEY &&
          !quickChatModelOverride
