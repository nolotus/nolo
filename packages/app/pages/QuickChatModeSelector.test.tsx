...
describe("QuickChatModeSelector", () => {
  ...
  it("renders the shared agent picker trigger and opens the popover", async () => {
    ...
    expect(trigger?.textContent).toContain("nolo");
    expect(trigger?.getAttribute("aria-label")).toBe("选择对话模式：nolo");
    expect(
  ...
  it("shows the selected favorite agent name on the trigger", async () => {
    ...
    expect(trigger?.textContent).not.toContain("nolo");
  ...
  it("falls back to the nolo default label while the agent name is still loading", async () => {
    mockAutoAgentId = "agent-fav-1";
    mockAutoAgent = undefined;
    mockCandidates = [
      {
