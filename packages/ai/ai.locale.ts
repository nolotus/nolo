export default {
  en: {
    translation: {
      ...
      localCreate: {
        title: "Create local AI",
        subtitle:
          "Agent-first · unbound from Space by default · Token Plan / API via custom direct connect",
        descLabel: "Description",
        descPlaceholder: "Optional — what is it good at?",
        connectionMethod: "Connection method",
        configureLater: "Configure later",
        liveVoice: "Live voice",
        recommendedTokenPlan: "Recommended · Token Plan",
        apiTemplates: "API templates",
        apiKeyPlaceholder:
          "sk-... (enters credential path only at submit; never stored in Agent plaintext)",
        modelPlaceholder: "Model name",
        endpointRequired: "Please enter the endpoint (Token Plan / OpenAI-compatible)",
        apiKeyRequired: "Paste an API Key, or switch to Configure later.",
        ollamaUrlRequired:
          "Enter a reachable Ollama URL (do not default to this device's localhost)",
        createGlobalFirst:
          "Create a global Agent first (custom / direct). Model and key can be added later in settings.",
        liveVoiceHint:
          "Live voice still uses the platform Google Live model path.",
        ollamaHint:
          "Enter an Ollama address the phone can reach (LAN IP, etc.). Do not default to localhost on this device.",
        joinSpace: "Add to current Space",
        noBinding:
          "Unbound by default; global Agents are visible in all views",
        success: "AI created",
        failed: "Failed to create AI",
        defaultNewName: "New AI",
        submit: "Create and start chat",
      },
      editAgent: "Edit AI",
      ...
      form: {
        ...
        apiKey: "API Key",
        crossDeviceKey: "Use on other devices",
        prompt: "Prompt",
        ...
        maxTokensFollowModelDefault: "Follow model default (not set)",
        maxTokensSetCustom: "Custom",
        maxTokensClearToDefault: "Follow model default",
        maxTokensInputPlaceholder: "Type a value (empty = follow model default)",
        reasoningEffort: "Reasoning Effort",
        defaults: {
          greeting:
      ...
      tools: {
        createAgent: {
          ...
          params: {
            ...
            model: "The model name to use. Logic: Default 'nolo/deepseek-flash'. Complex tasks: 'openai/gpt-5.6-sol'. Cost-saving: 'openai/gpt-5.5'. Aesthetic/Creative: 'gemini-3.8-flash'. Reliable low-cost summary/reasoning: 'nolo/deepseek-flash'. Image Generation: 'gemini-3-pro-image-preview' (Best/Expensive), 'gemini-3.1-flash-image-preview' (Ordinary), or 'gemini-3.1-flash-lite-image' (Fastest/Cheapest).",
            provider: "The model provider identifier, must match the selected model, e.g., 'openrouter', 'nolo', 'fireworks', 'mistral', etc.",
  ...
  "zh-CN": {
    translation: {
      ...
      localCreate: {
        title: "创建本地 AI",
        subtitle:
          "Agent 优先 · 默认不绑定 Space · Token Plan / API 走自定义直连",
        descLabel: "简介",
        descPlaceholder: "可选，告诉它擅长什么",
        connectionMethod: "连接方式",
        configureLater: "稍后配置",
        liveVoice: "实时语音",
        recommendedTokenPlan: "推荐 · Token Plan",
        apiTemplates: "API 模板",
        apiKeyPlaceholder:
          "sk-...（仅提交瞬间进入凭据路径，不写入 Agent 明文）",
        modelPlaceholder: "模型名",
        endpointRequired: "请填写接口地址（Token Plan / OpenAI 兼容）",
        apiKeyRequired: "请粘贴 API Key，或改选「稍后配置」。",
        ollamaUrlRequired: "请填写可访问的 Ollama 地址（勿默认本机 localhost）",
        createGlobalFirst:
          "先创建全局 Agent（custom / 直连），模型与 Key 可稍后在设置里补。",
        liveVoiceHint: "实时语音仍走平台 Google Live 模型路径。",
        ollamaHint:
          "填写手机能访问的 Ollama 地址（局域网 IP 等），不要默认本机 localhost。",
        joinSpace: "加入当前 Space",
        noBinding: "默认不绑定；全局 Agent 在全部视图可见",
        success: "AI 已创建",
        failed: "创建 AI 失败",
        defaultNewName: "新 AI",
        submit: "创建并开始对话",
      ...
      form: {
        ...
        maxTokens: "最大Token数",
        maxTokensFollowModelDefault: "跟随模型默认（未设置）",
        maxTokensSetCustom: "自定义",
        maxTokensClearToDefault: "跟随模型默认",
        maxTokensInputPlaceholder: "输入数值（留空 = 跟随模型默认）",
        reasoningEffort: "推理强度",
        defaults: {
          greeting:
      ...
      tools: {
        createAgent: {
          ...
          params: {
            ...
            model: "默认使用 'nolo/deepseek-flash'。复杂任务: 'openai/gpt-5.6-sol'。省成本但复杂: 'openai/gpt-5.5'。追求美观/创意: 'gemini-3.8-flash'。低成本摘要/推理: 'nolo/deepseek-flash'。生成图片: 贵/高质量选 'gemini-3-pro-image-preview'，普通选 'gemini-3.1-flash-image-preview'，最快最便宜选 'gemini-3.1-flash-lite-image'。",
            provider: "模型提供方标识，必须与所选模型相匹配，例如 'openrouter'、'nolo'、'fireworks'、'mistral' 等。",
  ...
  "zh-Hant": {
    translation: {
      ...
      localCreate: {
        title: "建立本機 AI",
        subtitle:
          "Agent 優先 · 預設不綁定 Space · Token Plan / API 走自訂直連",
        descLabel: "簡介",
        descPlaceholder: "可選，告訴它擅長什麼",
        connectionMethod: "連線方式",
        configureLater: "稍後設定",
        liveVoice: "即時語音",
        recommendedTokenPlan: "推薦 · Token Plan",
        apiTemplates: "API 範本",
        apiKeyPlaceholder:
          "sk-...（僅在提交瞬間進入憑證路徑，不寫入 Agent 明文）",
        modelPlaceholder: "模型名",
        endpointRequired: "請填寫介面位址（Token Plan / OpenAI 相容）",
        apiKeyRequired: "請貼上 API Key，或改選「稍後設定」。",
        ollamaUrlRequired: "請填寫可存取的 Ollama 位址（勿預設本機 localhost）",
        createGlobalFirst:
          "先建立全域 Agent（custom / 直連），模型與 Key 可稍後在設定裡補。",
        liveVoiceHint: "即時語音仍走平台 Google Live 模型路徑。",
        ollamaHint:
          "填寫手機能存取的 Ollama 位址（區域網路 IP 等），不要預設本機 localhost。",
        joinSpace: "加入目前 Space",
        noBinding: "預設不綁定；全域 Agent 在全部檢視可見",
        success: "AI 已建立",
        failed: "建立 AI 失敗",
        defaultNewName: "新 AI",
        submit: "建立並開始對話",
      ...
      form: {
        ...
        maxTokens: "最大Token數",
        maxTokensFollowModelDefault: "跟隨模型預設（未設定）",
        maxTokensSetCustom: "自訂",
        maxTokensClearToDefault: "跟隨模型預設",
        maxTokensInputPlaceholder: "輸入數值（留空 = 跟隨模型預設）",
        reasoningEffort: "推理強度",
        defaults: {
          greeting:
  ...
  ja: {
    translation: {
      ...
      localCreate: {
        title: "ローカル AI を作成",
        subtitle:
          "Agent 優先 · 既定で Space 非バインド · Token Plan / API はカスタム直結",
        descLabel: "紹介",
        descPlaceholder: "任意 — 得意なことを教えてください",
        connectionMethod: "接続方法",
        configureLater: "あとで設定",
        liveVoice: "リアルタイム音声",
        recommendedTokenPlan: "おすすめ · Token Plan",
        apiTemplates: "API テンプレート",
        apiKeyPlaceholder:
          "sk-...（送信時のみ資格情報パスへ。Agent 平文には保存しません）",
        modelPlaceholder: "モデル名",
        endpointRequired:
          "エンドポイントを入力してください（Token Plan / OpenAI 互換）",
        apiKeyRequired:
          "API キーを貼り付けるか、「あとで設定」に切り替えてください。",
        ollamaUrlRequired:
          "到達可能な Ollama アドレスを入力（この端末の localhost を既定にしない）",
        createGlobalFirst:
          "まずグローバル Agent（custom / 直結）を作成。モデルとキーは設定で後から追加できます。",
        liveVoiceHint:
          "リアルタイム音声は引き続きプラットフォームの Google Live モデル経路を使います。",
        ollamaHint:
          "スマホから届く Ollama アドレス（LAN IP など）を入力。この端末の localhost を既定にしないでください。",
        joinSpace: "現在の Space に追加",
        noBinding:
          "既定は非バインド。グローバル Agent はすべてのビューで表示されます",
        success: "AI を作成しました",
        failed: "AI の作成に失敗しました",
        defaultNewName: "新しい AI",
        submit: "作成してチャット開始",
      },
      editAgent: "AIを編集",
      ...
      form: {
        ...
        maxTokens: "最大トークン数",
        maxTokensFollowModelDefault: "モデルのデフォルトに従う（未設定）",
        maxTokensSetCustom: "カスタム",
        maxTokensClearToDefault: "モデルのデフォルトに従う",
        maxTokensInputPlaceholder: "数値を入力（空欄 = モデルのデフォルトに従う）",
        reasoningEffort: "推論強度",
        defaults: {
          greeting:
