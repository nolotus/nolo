...
const resolveOpenAIBuiltInImageSurcharge = (
  ...
  const imageGenerationCount =
    asOptionalFiniteNumber(usage.image_generation_count) ?? 0;
  const pricePerImage =
    getApproxPricePerImage(model) ??
    // gpt-image-2 已收进 nolo 平台托管目录（provider=nolo）
    getApproxPricePerImage(findModelConfig("nolo", "gpt-image-2")) ??
    0;
...
export const calculatePrice = ({
  ...
  try {
    ...
    if (isPlatformHostedImageModel(modelName)) {
      const hostedImageModel = getModelConfig("nolo", modelName);
      const costs = calculateBasicCost(
        hostedImageModel,
        usage,
        provider ?? "nolo",
      ...
      const surcharge = isPlatformHostedOpenAIImageModel(modelName)
        ? resolveOpenAIBuiltInImageSurcharge(hostedImageModel, usage)
        : 0;
      const adjusted =
        surcharge > 0
          ? {
              ...costs,
              regular: costs.regular + surcharge,
              charge: costs.charge + surcharge,
            }
          : costs;
      const pay = calculatePayDistribution(adjusted, externalPrice, sharingLevel);
      return { cost: sanitizeCost(adjusted.charge), pay };
    }
    ...
    if (
      ...
      // pro 已恢复为一等模型，按各自目录配置计价；flash 旧名经 alias 落到 flash 配置。
      const hostedModel = getModelConfig(
        "nolo",
        asTrimmedLowercaseString(modelName) === PLATFORM_HOSTED_DEEPSEEK_PRO_MODEL
          ? PLATFORM_HOSTED_DEEPSEEK_PRO_MODEL
          : PLATFORM_HOSTED_DEEPSEEK_FLASH_MODEL,
      );
      const costs = calculateBasicCost(
