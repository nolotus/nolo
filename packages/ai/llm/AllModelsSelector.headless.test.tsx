...
describe("AllModelsSelector headless", () => {
  ...
  test("ALL_MODELS exposes grok-4.6 under provider=xai (data source contract)", () => {
    const xaiGrok46 = ALL_MODELS.filter(
      (m) => m.provider === "xai" && m.name === "grok-4.6"
    );
    expect(xaiGrok46).toHaveLength(1);
    const model = xaiGrok46[0];
  ...
  test("dedupes same-name models: Claude Sonnet 5 appears exactly once", async () => {
    // 前置：ALL_MODELS 中 anthropic/claude-sonnet-5 确实双份注册
    // （平台托管 nolo + deepinfra 直连，上游同源）
    expect(
