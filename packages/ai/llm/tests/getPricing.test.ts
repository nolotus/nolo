...
describe("getAgentPriceHint", () => {
  ...
  it("returns a default-profile image estimate for the generator agent", async () => {
    ...
    // 1K estimate: 420 * 1120 / 1_000_000 = 0.4704 (not 2K's 0.7056)
    expect(hint).toEqual({
      type: "per_image",
      amount: 0.4704,
  ...
  it("uses 1K default profile pricing for generator even when agent is configured for 2K", async () => {
    ...
    // Should use 1K pricing (1120 tokens * 420 per million / 1_000_000 = 0.4704)
    // NOT 2K pricing (1680 tokens * 420 per million / 1_000_000 = 0.7056)
    expect(hint).toEqual({
      type: "per_image",
      amount: 0.4704,
      labelKey: "defaultImageProfileEstimate",
  ...
  it("uses 1K default profile pricing for generator even when agent is configured for 4K", async () => {
    ...
    // Should use 1K pricing (1120 tokens * 420 per million / 1_000_000 = 0.4704)
    // NOT 4K pricing (2520 tokens * 420 per million / 1_000_000 = 1.0584)
    expect(hint).toEqual({
      type: "per_image",
      amount: 0.4704,
      labelKey: "defaultImageProfileEstimate",
  ...
  it("returns DeepSeek V4 pricing", async () => {
    ...
    expect(
      getModelPricingForModel("deepseek", "deepseek-v4-flash", {
        price: { input: 0.14 * 7, inputCacheHit: 0.0028 * 7, output: 0.28 * 7 },
      })
    ).toEqual({
      inputPrice: 0.14 * 7,
      inputCacheHitPrice: 0.0028 * 7,
      outputPrice: 0.28 * 7,
    });
    expect(
      getModelPricingForModel("deepseek", "deepseek-v4-pro", {
        price: { input: 0.435 * 7, inputCacheHit: 0.003625 * 7, output: 0.87 * 7 },
      })
    ).toEqual({
      inputPrice: 0.435 * 7,
      inputCacheHitPrice: 0.003625 * 7,
      outputPrice: 0.87 * 7,
    });
  });
...
describe("getApproxPricePerImage", () => {
  ...
  it("falls back to 1K pricing when no size is specified", async () => {
    ...
    expect(
      getApproxPricePerImage({
        imageTokenPricePerMillion: 30 * 7,
        imageOutputTokenEstimateBySize: {
          "1K": 1290,
        },
      })
