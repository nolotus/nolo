...
/**
 * 清洗单个文本字符串，使其可被严格 JSON 解析器安全接受。
 *
 * 背景：历史消息 content 纯文本里可能混入两类「脏」内容，上游严格解析器
 * 在把请求体当 JSON 解析时会炸出 `unexpected end of hex escape` 400：
 *
 * 1. 孤立代理字符（unpaired surrogate）：JS 字符串里单独出现的高/低代理
 *    字符（如 `\uD800` 无配对）。JSON.stringify 会把它编码成 `\ud800`，
 *    某些严格解析器（如 serde_json 严格模式）因缺少配对代理而报 hex
 *    escape 错误。这里统一替换为 U+FFFD（�）。
 * 2. 字面残缺 hex 转义序列：文本里字面出现的 `\x`（后不足 2 位 hex）或
 *    `\u`（后不足 4 位 hex）半截序列。宽松解析器会尝试按转义解码而炸出
 *    hex escape 错误。这里把这类半截序列的反斜杠转义为 `\\`，使其成为
 *    字面反斜杠，不再被当作转义解码。
 *
 * 函数是幂等的：清洗后的字符串再清洗一遍结果不变，因此天然兼容
 * double-encode 场景（清洗后再次 JSON.stringify 依然合法）。
 *
 * 返回清洗后的新字符串（不修改入参）。
 */
export function sanitizeTextContent(text: string): string {
  ...
  while (i < n) {
    ...
    // 2) 字面残缺 hex 转义处理（反斜杠 + x/u）
    if (code === 0x5c /* \ */) {
      // 判断当前反斜杠是否已被转义（前面有奇数个连续反斜杠）。
      // 若已转义，则它是字面反斜杠，原样保留，避免重复转义破坏幂等性。
      let backslashRun = 0;
      let k = i;
      while (k >= 0 && text.charCodeAt(k) === 0x5c) {
        backslashRun++;
        k--;
      }
      const alreadyEscaped = backslashRun % 2 === 0; // 含当前，偶数个 → 当前被转义
      const nextCh = i + 1 < n ? text[i + 1] : "";
      if (!alreadyEscaped && (nextCh === "x" || nextCh === "u")) {
        const hexLen = nextCh === "x" ? 2 : 4;
        let hexCount = 0;
        let j = i + 2;
        while (j < n && hexCount < hexLen && isHexDigit(text[j])) {
          hexCount++;
          j++;
        }
        if (hexCount < hexLen) {
          // 残缺 hex 转义：把反斜杠转义为 `\\`，使其成为字面反斜杠，
          // 避免宽松解析器把它当转义解码。后续字符原样保留。
          out += "\\\\";
          i += 1;
          continue;
        }
        // 完整 hex 转义（字面 `\xNN` / `\uNNNN`），原样保留
        out += text[i];
        i += 1;
        continue;
      }
...
/**
 * 清洗消息/input item 的 content 字段中的纯文本，同时不破坏 image part。
 *
 * content 可能是：
 *   - string（纯文本）
 *   - part 数组（如 `[{type:"text",text}, {type:"image_url",image_url}]`）
 *
 * 只清洗 text 类 part 的 text 字段；image_url 等其它 part 原样保留。
 * 返回新的 content（不修改入参）。
 */
function sanitizeContentText(content: unknown): unknown {
  ...
  if (Array.isArray(content)) {
    return content.map((part) => {
      if (part && typeof part === "object") {
        const p = part as any;
        // 只清洗带 text 字段的 part；image_url 等其它 part 原样保留
        if (typeof p.text === "string") {
          return { ...p, text: sanitizeTextContent(p.text) };
        }
      }
      return part;
    });
...
/**
 * 清洗 responses API 单个 input item 的纯文本字段。
 * 返回新的 item（不修改入参）。
 */
function sanitizeResponsesItemText(item: any): any {
  ...
  if (type === "function_call_output") {
    // output 可能是 string 或 part 数组
    if (typeof item.output === "string") {
      return { ...item, output: sanitizeTextContent(item.output) };
    }
    if (Array.isArray(item.output)) {
      return { ...item, output: sanitizeContentText(item.output) };
    }
    return item;
  }
  if (type === "message" && "content" in item) {
    return { ...item, content: sanitizeContentText(item.content) };
  }
  return item;
}
...
export function sanitizeOutboundMessages(
  ...
  const result: any[] = [];
  for (const msg of cloned) {
    // 清洗 content 纯文本（孤立代理字符 / 残缺 hex 转义），不破坏 image part
    if (msg && typeof msg === "object" && "content" in msg) {
      msg.content = sanitizeContentText(msg.content);
    }
...
export function sanitizeOutboundResponsesInput(
  ...
  for (const item of input) {
    ...
    // 清洗 input_text / output_text / function_call_output.output 等纯文本字段
    result.push(sanitizeResponsesItemText(item));
  }

  return result;
}
/**
 * 防护 B（出站 body 守卫）：按 wire 形状清洗出站请求体。
 *
 * body 可能是 chat.completions 形状（含 `messages` 数组）或 responses 形状
 * （含 `input` 数组）。按实际形状守卫：
 *   - 仅当 `messages` 是数组时注入清洗后的 messages；
 *   - 否则若 `input` 是数组，注入清洗后的 input；
 *   - 两者都不是则原样返回，绝不注入多余的 `messages: []`。
 *
 * 返回新的 body（不修改入参）。
 */
export function sanitizeOutboundBody(body: Record<string, any>): Record<string, any> {
  const next = { ...body };
  if (Array.isArray(body.messages)) {
    next.messages = sanitizeOutboundMessages(body.messages);
  } else if (Array.isArray(body.input)) {
    next.input = sanitizeOutboundResponsesInput(body.input);
  }
  return next;
}
