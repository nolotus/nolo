...
describe("runtimeGuidance", () => {
  ...
  it("warns shell-capable agents not to use shell for production webpage extraction", () => {
    const blocks = buildRuntimeGuidanceBlocks(["execShell", "fetchWebpage"]);

    expect(blocks.webResearchToolPolicy).toContain("生产环境网页研究");
    expect(blocks.webResearchToolPolicy).toContain("不要用 execShell");
    expect(blocks.webResearchToolPolicy).toContain("fetchWebpage");
    expect(blocks.webResearchToolPolicy).toContain("Markdown");
  });
