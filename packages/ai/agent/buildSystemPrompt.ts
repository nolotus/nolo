...
import { wrapHistoricalSummaryWithReplayGuard } from "ai/context/staleReplayGuard";
...
当前处于窄屏或终端视图，请采用紧凑、高扫描率的排版：
- **结构与留白**：结论或关键状态置顶，直奔主题；短段落（2-4 句）并保持空行呼吸，拒绝长段文字墙。
- **列表与缩进**：列表项用「- **核心词**：说明」；嵌套最多 2 层，子列表缩进 2 空格，防止终端与移动端折行错位；并列用无序列表，步骤用有序列表。
- **表格降级**：严禁使用 ≥4 列的大宽表（窄屏/终端下折行严重错位），改用「键值清单」或纵向卡片。
- **符号与代码**：克制使用「---」分割线（仅大主题切换或结论前，前后留空行）；路径、命令与参数一律用反引号 \` \` 包裹；代码块标明语言且避免横向超长行；克制 Emoji。`;
  }
  return `--- 响应展示指南（宽屏 / 桌面端） ---
当前处于大屏视图，可充分发挥空间优势：
- **结构与留白**：提供层次清晰的全景结构，重要模块用二级/三级标题划分；段落保持 2-4 句并留空行呼吸，直奔主题。
- **列表与缩进**：列表项用「- **核心词**：说明」；嵌套最多 2 层，子列表缩进 2 空格；并列用无序列表，步骤用有序列表。
- **表格与对比**：宽屏下可适度使用 Markdown 多列表格进行方案横向对比、参数汇总或矩阵分析。
- **符号与代码**：可展示更完整的代码上下文与多步执行示例；路径、命令与参数用反引号 \` \` 包裹，代码块标明语言；克制使用分割线与 Emoji。`;
};
...
export const buildSystemPromptContext = (options: {
  ...
  const effectiveViewport = viewport ?? contexts.viewport;
  const isMobile =
    effectiveViewport !== undefined
      ? effectiveViewport.width < mobileBreakpoint
      : typeof contexts.isMobile === "boolean"
      ? contexts.isMobile
