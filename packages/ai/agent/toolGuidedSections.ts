...
**分档标准（按逻辑步骤数和触及文件数，领域数仅作复杂档辅助判据）**：
- 简单：逻辑步骤 <5 且只读/搜/改 ≤2 个文件 → 自己做，不派发。
- 中等：超出简单档、但未跨 3 个独立领域 → 先分析给方案，方案认可后派发执行者（优先低成本模型如 DeepSeek Flash）；自己只做规划、review、集成。
- 复杂：跨 3 个以上独立领域，或含仓库改动＋验证＋发布/迁移的长链路 → 先与候选 agent 讨论达成一致，再按领域拆分派发；此档默认至少派 2 个独立子任务或审计，若选择不派，必须在计划里写明成本/收益理由。
