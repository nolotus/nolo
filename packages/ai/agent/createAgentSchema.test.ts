...
// valibot issue.path 是对象数组 [{key: ...}]，提取字段错误消息。
const fieldError = (issues: any[] | undefined, field: string): string | undefined => {
  if (!issues) return undefined;
  const issue = issues.find((i) =>
    i.path?.some((p: any) => p && typeof p === "object" && p.key === field),
  );
...
describe("MAX_TOKENS_LIMIT single authority", () => {
  ...
  it("createAgentSchema's max_tokens maxValue() references the shared constant", () => {
    ...
    expect(schemaSource).toContain("maxValue(MAX_TOKENS_LIMIT");
  });

  it("ModelParamsSection slider max references the shared constant", () => {
    const uiSource = readFileSync(
      join(import.meta.dir, "web/ModelParamsSection.tsx"),
      "utf8",
    );
...
describe("createAgentSchema machine binding validation", () => {
  ...
  it("allows max_tokens at the shared upper limit (500000)", () => {
    const result = safeParse(schema, { ...base, max_tokens: MAX_TOKENS_LIMIT });
    expect(result.success).toBe(true);
  });

  it("rejects max_tokens above the shared upper limit (500001)", () => {
    const result = safeParse(schema, { ...base, max_tokens: MAX_TOKENS_LIMIT + 1 });
    expect(result.success).toBe(false);
  });

  it("accepts max_tokens null (follow model default)", () => {
    const result = safeParse(schema, { ...base, max_tokens: null });
    expect(result.success).toBe(true);
  });

  it("accepts max_tokens undefined (follow model default)", () => {
    const result = safeParse(schema, { ...base });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.output?.max_tokens).toBeUndefined();
    }
  });
  ...
  it("rejects machine binding for remote custom providers", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(fieldError(result.issues, "machineId")).toBe(
  ...
  it("accepts a hosted exec runtime policy for advanced web execution", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.output?.runtimeToolPolicy).toEqual({
        version: 1,
        runtimeTools: ["execShell"],
        workspace: { mode: "lease" },
      });
