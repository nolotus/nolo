...
const readNonEmptyApiKey = (value: unknown): string => asTrimmedString(value);
...
const processAgentCreateForm = (formData: AgentFormData, userId: string) => {
  ...
  const machineId = asTrimmedString((formData as any).machineId);
...
const processAgentUpdateChanges = (
  ...
  if ("tags" in data) {
    ...
    if (Array.isArray(raw)) {
      arr = raw.map((s) => toTrimmedString(s)).filter(Boolean);
