  ...
  return (
    <div className="tools-tab-container">
      <section className="tools-selection-card">
        <FormField error={errors.tools?.message} {...commonProps}>
          <Controller
            name="tools"
            control={control}
            render={({ field }) => (
                      ...
                      <Controller
                        ...
                        render={({ field: skillsField }) => (
                          <ToolsTabField
                            ...
                            setValue={setValue as (name: string, value: unknown, options?: unknown) => void}
                            ...
                            onSkillConfigChange={(next) => {
                              ...
                              // 「启用」档下次读取会退化成「禁用」。
                              // 传 previous：skills 是嵌套对象，patch 走
                              // deepMerge，少写一个 key 不等于删除——必须为
                              // 「从有到无」的 slug 显式写 null，否则「禁用」
                              // 存不下去（旧值会被复活）。
                              ...
                              skillsField.onChange(patch.skills);
                              packsField.onChange(patch.enabledPacks);
                            }}
                          />
                        )}
                      />
