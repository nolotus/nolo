...
const AdvancedRuntimeSection: React.FC<AdvancedRuntimeSectionProps> = ({
  ...
  const runtimeToolPolicy = values.runtimeToolPolicy;
  ...
  return (
    <section className="adv-settings__group">
      ...
      >
        <ToggleSwitch
          checked={!!values.useServerProxy}
          onChange={(v) => set("useServerProxy", v)}
        />
      </FormField>
      ...
      {values.enableThinking ? (
        ...
        >
          <Slider
            value={(values.thinkingBudget as number | null) ?? 8000}
