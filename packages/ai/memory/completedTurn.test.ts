...
describe("captureCompletedMemoryTurn", () => {
  ...
  it("skips LLM extraction for short user input (< 20 chars)", async () => {
    let llmCalled = false;
    await captureCompletedMemoryTurn({
      db,
      userId: "user1",
      agentKey: "agent-a",
      dialogId: "dialog-short-input",
      userInput: "好的", // 只有 2 个字符
      trace: [
        { role: "user", content: "好的" },
        { role: "assistant", content: "还有什么需要帮忙的吗？" },
        { role: "user", content: "好的" },
      ],
      llmCall: async () => {
        llmCalled = true;
  ...
  it("runs LLM extraction for substantive user input (>= 20 chars)", async () => {
    let llmCalled = false;
    await captureCompletedMemoryTurn({
      db,
      userId: "user1",
      agentKey: "agent-a",
      dialogId: "dialog-substantive",
      userInput: "我更怕第一封把用户吓跑，所以信任感比转化率更重要，你觉得呢",
      trace: [
        { role: "user", content: "我更怕第一封把用户吓跑，所以信任感比转化率更重要" },
        { role: "assistant", content: "明白，你更在意信任感。" },
        { role: "user", content: "对，先建立信任再谈转化" },
      ],
      llmCall: async () => {
        llmCalled = true;
        return "[]";
