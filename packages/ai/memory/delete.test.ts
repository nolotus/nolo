...
describe("memory delete", () => {
  ...
  it("deletes memory records and indexes by owner filters", async () => {
    ...
    const deleteByContentResult = await deleteMemoriesForOwnerFromDb(
      db,
      { ownerType: "user", ownerId: "user-1" },
      { contentSubstring: "信任感" }
    );
    expect(deleteByContentResult.deletedCount).toBe(1);
    expect(deleteByContentResult.matchedItems[0]?.content).toContain("信任感");

    const afterAll = await loadMemoryCandidatesFromDb(db, {
      owners: [{ ownerType: "user", ownerId: "user-1" }],
      subjects: [{ subjectType: "agent", subjectId: "agent-a" }],
      kinds: ["episodic", "semantic"],
      ownerLimit: 20,
  ...
  it("supports dryRun preview without deleting database records", async () => {
    const { deleteMemoriesForOwnerFromDb, createMemoryItem, writeMemoryItemWithIndexesToDb, loadMemoryCandidatesFromDb } = await loadDeps();
    await writeMemoryItemWithIndexesToDb(
      db,
      createMemoryItem({
    ...
    const dryRunResult = await deleteMemoriesForOwnerFromDb(
      db,
      { ownerType: "user", ownerId: "user-dryrun" },
      { contentSubstring: "小明", dryRun: true }
    );

    expect(dryRunResult.deletedCount).toBe(0);
    expect(dryRunResult.deletedIds).toHaveLength(0);
    expect(dryRunResult.matchedItems).toHaveLength(1);
    expect(dryRunResult.deletionToken).toBeDefined();

    // Verify record still exists in DB
    const remaining = await loadMemoryCandidatesFromDb(db, {
      owners: [{ ownerType: "user", ownerId: "user-dryrun" }],
      subjects: [],
      kinds: ["semantic"],
      ownerLimit: 10,
    });
    expect(remaining).toHaveLength(1);
    expect(remaining[0]?.content).toContain("小明");
  });

  it("rejects deletion when deletionToken mismatches", async () => {
    const { deleteMemoriesForOwnerFromDb, createMemoryItem, writeMemoryItemWithIndexesToDb } = await loadDeps();
    await writeMemoryItemWithIndexesToDb(
      db,
      createMemoryItem({
        ownerType: "user",
        ownerId: "user-token-test",
        visibility: "private",
        subjectType: "user",
        subjectId: "user-token-test",
        kind: "semantic",
        content: "用户偏好明亮模式",
        importance: 0.9,
        confidence: 0.8,
    ...
    await expect(
      deleteMemoriesForOwnerFromDb(
        db,
        { ownerType: "user", ownerId: "user-token-test" },
        { contentSubstring: "明亮模式", deletionToken: "tok-wrong-token" }
      )
    ).rejects.toThrow("deletionToken mismatch");
  });

  it("rejects implicit wildcard deletion when no filters are provided", async () => {
    const { deleteMemoriesForOwnerFromDb } = await loadDeps();
    await expect(
      deleteMemoriesForOwnerFromDb(
        db,
        { ownerType: "user", ownerId: "user-1" },
