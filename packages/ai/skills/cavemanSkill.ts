/**
 * Platform-owned `caveman` built-in skill.
 *
 * Promoted from the repo's `.agents/skills/caveman/` (2026-09-20): the content
 ...
 *
 * Upstream provenance: github.com/JuliusBrussee/caveman (the repo copy was
 * vendored from there via `skills-lock.json`; that lock entry was dropped when
 * the copy was deleted, so this comment is the remaining attribution).
 */
...
export const CAVEMAN_SKILL_SLUGS = ["caveman"] as const;

export type CavemanSkillSlug = (typeof CAVEMAN_SKILL_SLUGS)[number];
...
export function buildCavemanSkillId(
  slug: CavemanSkillSlug,
): string {
  return deterministicId("01SK", normalizeSkillSeed(slug) || slug);
}

export function buildCavemanSkillPageKey(
  userId: string,
  slug: CavemanSkillSlug,
): string {
  return `page-${userId}-${buildCavemanSkillId(slug)}`;
}

export function buildCavemanSkillConfig(
  slug: CavemanSkillSlug,
): SkillDocConfig {
  return {
    version: "0.1",
    kind: "skill",
    id: buildCavemanSkillId(slug),
    name: "压缩表达（caveman）",
...
export function buildCavemanSkillContentBySlug(
  slug: CavemanSkillSlug,
): string {
  return buildSkillDocMarkdown({
    ...
    skillConfig: buildCavemanSkillConfig(slug),
  });
}
