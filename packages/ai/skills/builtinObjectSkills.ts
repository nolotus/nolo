...
export const BUILTIN_OBJECT_SKILL_IDS = {
  ...
  // key 改名为 code-style（它本来就叫「编码风格技能」，"code" 这个名字是它抢的，
  // 与「代码执行」能力包撞车）。**id 值保持不变**——skillId 与页面 key 都从它派生，
  // 改了会让存量 agent.references 失配。
  "code-style": "builtin-code-skill-v1",
  image: "builtin-image-skill-v1",
  file: "builtin-file-skill-v1",
  pagebuilder: "builtin-pagebuilder-skill-v1",
} as const;

export type BuiltinObjectSkillKind = "doc" | "table" | "code-style" | "image" | "file" | "pagebuilder";
...
const SKILL_DEFS: Record<BuiltinObjectSkillKind, SkillDef> = {
  doc: {
    ...
    name: "文档编辑技能",
    description: "提供文档读取与更新能力，供文档助手使用。触发词：文档、润色、改写、续写、排版、文章、document、polish。",
  ...
  table: {
    ...
    name: "表格编辑技能",
    description: "提供表格创建、查询、新增、更新与删除能力，供表格助手使用。触发词：表格、建表、新增一行、记录、字段、table、spreadsheet、csv。",
    // 与 app-builder 能力包共用同一份清单（TOOL_PACKS.TABLE）。
    // 以前两边各抄一份，改一个忘另一个就会静默不同步。
    toolNames: [...TOOL_PACKS.TABLE],
    promptPatch: [
      "表格操作指南：",
  ...
  "code-style": {
    id: BUILTIN_OBJECT_SKILL_IDS["code-style"],
    name: "编码风格技能",
    description: "提供编码规范与风格约束，供代码任务使用。触发词：代码、编码、重构、修bug、code、refactor、bugfix、coding-style。",
    toolNames: [],
    promptPatch: [
      "编码风格与规范指南：",
  ...
  image: {
    ...
    name: "图片分析技能",
    description: "提供图片理解、描述与整理建议能力，供图片助手使用。触发词：图片、图像、分析、描述、命名、整理、image、photo、picture。",
    toolNames: [],
    promptPatch: [
      "图片分析指南：",
  ...
  file: {
    ...
    name: "文件处理技能",
    description: "提供文件理解、整理与处理建议能力，供文件助手使用。触发词：文件、解析、提取、整理、处理、file、document、pdf。",
    toolNames: [],
    promptPatch: [
      "文件处理指南：",
  ...
  pagebuilder: {
    ...
    name: "动态交互组件生成技能",
    description:
      "把一句话需求变成可看的交互页面、报告、看板、主页、教程、对比页或计划页。任何 agent 在需要输出可交互可视化消息时按需加载。触发词：页面、报表、看板、日报、周报、主页、教程、对比页、计划页、可视化、交互页面、仪表盘、dashboard、report、page、landing。",
    toolNames: [],
    promptPatch: [
      "动态交互组件生成规则：",
...
export function buildBuiltinObjectSkillConfig(
...
export function buildBuiltinObjectSkillPageContent(
  ...
  return buildSkillDocMarkdown({
    body: `# ${def.name}\n\n${def.description}`,
    skillConfig: buildBuiltinObjectSkillConfig(kind),
  });
}
