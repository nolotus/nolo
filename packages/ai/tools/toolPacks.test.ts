...
describe("applyDisabledTools", () => {
  ...
  test("disabledTools 现在可以关掉 ask_user（CORE 默认注入但用户可关）", () => {
    expect(
      applyDisabledTools(
        ["ask_user", "exa_search"],
        ["ask_user", "exa_search"],
...
describe("FORCED_TOOLS", () => {
  test("当前为空（ask_user 走 CORE 默认下发，非强制）", () => {
    expect(FORCED_TOOLS.size).toBe(0);
  });
});
...
describe("CAPABILITY_PACKS", () => {
  ...
  test("long-term-memory pack contains queryMemory + rememberMemory + deleteMemory and defaults on", () => {
    const pack = CAPABILITY_PACK_BY_ID["long-term-memory"];
    expect(pack).toBeDefined();
    expect(pack.tools).toEqual(["queryMemory", "rememberMemory", "deleteMemory"]);
...
describe("CORE 不再常驻可选工具", () => {
  ...
  test("ask_user 常驻 CORE（Web/TUI 默认启用，desktop 运行时剥离）", () => {
    const { TOOL_PACKS } = require("./toolPacks");
