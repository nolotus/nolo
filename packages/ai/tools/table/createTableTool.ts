...
export async function createTableFunc(
    ...
    const spaceId = asOptionalTrimmedString(args?.spaceId);
    const title = asOptionalTrimmedString(args?.title);
    const purpose = asOptionalTrimmedString(args?.purpose);
    const description = asOptionalTrimmedString(args?.description);
    const tags =
        Array.isArray(args?.tags) && args.tags.length
            ? asTrimmedNonEmptyStringArray(args.tags)
            : undefined;
    const publicIntake = isRecord(args?.publicIntake)
        ? args.publicIntake
        : undefined;
    const categoryId = asOptionalTrimmedString(args?.categoryId);
    ...
    for (const col of columnsInput) {
        ...
        const label = asOptionalTrimmedString(col.label);
        if (label) {
        ...
        const description = asOptionalTrimmedString(col.description);
        if (description) {
        ...
        if (Array.isArray(col.options)) {
            const opts = asTrimmedNonEmptyStringArray(col.options);
