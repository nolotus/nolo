export const queryModelUsageFunctionSchema = {
  ...
  parameters: {
    ...
    properties: {
      analysisType: {
        type: "string",
        enum: ["billing", "workload", "cacheBreakdown", "pricingReplay", "breakEven", "prefixChurn", "dialogCacheHealth"],
        description:
          ...
          + "dialogCacheHealth=按对话分组的 cache hit 健康探针（cache_read 钉死/input 持续涨的对话，hitRatio 升序最差在前）。",
        default: "billing",
      },
      scope: {
        type: "string",
      ...
      model: {
        type: "string",
        description: "可选模型过滤，例如 gemini-3.8-flash。",
      },
      serviceTier: {
        type: "string",
      ...
      dialogId: {
        type: "string",
        description: "可选：按对话 id 过滤 prefixChurn / dialogCacheHealth 诊断",
      },
      minCalls: {
        type: "number",
        description:
          "可选：dialogCacheHealth 的最小调用次数阈值，低于该值的对话会被过滤（避免首轮 0% 噪音）。默认 3。",
      },
      currency: {
        type: "string",
