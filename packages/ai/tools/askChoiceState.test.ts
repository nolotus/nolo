...
describe("normalizeAskChoiceArgs", () => {
  ...
  test("header is parsed, trimmed, and capped at 12 chars", () => {
    const result = normalizeAskChoiceArgs({
      questions: [
        {
          id: "q0",
        ...
        },
        {
          id: "q1",
          question: "Q2?",
          header: "  一个特别长的标题用来验证截断  ",
          choices: [{ id: "b", label: "B" }],
        },
      ],
    });
    expect(result.questions[0].header).toBe("格式");
    // 12-char cap + trim
    expect(result.questions[1].header?.length).toBeLessThanOrEqual(12);
    expect(result.questions[1].header).toBe(result.questions[1].header?.trim());
  });

  test("missing header falls back to undefined (renderer uses Qn)", () => {
    const result = normalizeAskChoiceArgs({
      questions: [
        { id: "q0", question: "Q1?", choices: [{ id: "a", label: "A" }] },
      ],
