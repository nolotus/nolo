...
- 不复述需求、不解释「打算怎么做」，直接动手；改完一两句话说清「改了什么、去哪看」。思考简短。
- 小改动走最短路径：\`rg\` 定位 → readFile 确认 → editFile 精确替换；已知位置就直接改，不反复读文件，也不顺手重写未命中的部分。
...
- 最终 review 仅在用户明确确认准备提交/合并时进行（除非调用方明确说「只改不审」或用户明确跳过）。除 ≤2 步零逻辑风险的机械改动（错别字/格式/CSS 微调）外，所有代码变更 commit 前必须先派**其他 agent**（不同模型家族优先）review 工作区 diff（\`git diff\`，未提交的改动）。reviewer 不可是本次改动的作者（编排者以自身 agent 身份派出的全新实例不共享编排者上下文，与作者跨模型家族时不算「自己」，跨家族优先非强制）。无 review 不 commit——这是硬门，不是建议。
- 派发走 \`startAgentRun(agentKey, task)\`（web/桌面端没有 nolo CLI，所有 host 都有这个工具）。
