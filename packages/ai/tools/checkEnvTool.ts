...
const formatProbeOutput = (probe: any): string => {
    ...
    const stdout = asTrimmedString(probe?.stdout);
    const stderr = asTrimmedString(probe?.stderr);
    const error = asTrimmedString(probe?.error);
...
export async function checkEnvFunc(
    ...
    const key = asTrimmedLowercaseString(args?.key);
