...
export async function updateDocFunc(
    ...
    try {
        ...
        // docStore 持有编辑器的渲染态（slateData），只写记录不会触发重渲染，
        // 表现为「AI 说改好了但编辑器没变化」；且编辑器随后可能用过期的
        // in-memory slate 自动保存，把刚写的内容覆盖掉。
        // applyExternalDocUpdate 同步替换 slateData + lastSaved 标记并递增
        // externalUpdateSeq 触发静默重挂载，不走 initDoc 全量 loading 重载
        // （用户期待内容就地出现，不是整页刷新）。
        const { getDocState, applyExternalDocUpdate } = await import(
            "render/page/docStore"
        );
        if (getDocState().pageKey === id) {
            applyExternalDocUpdate({
                slateData: finalSlateData as any,
                content: parsedProtocol.content,
        ...
        throw new Error(`更新页面失败：${toErrorMessage(error)}`);
    }
}
