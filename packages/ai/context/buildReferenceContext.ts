...
const fetchDialogReference = async (
  ...
  if (checkpoint && typeof checkpoint === "object") {
    ...
    if (
      Array.isArray(checkpoint.toolSurface?.finalToolNames) &&
      checkpoint.toolSurface.finalToolNames.length
    ) {
      checkpointLines.push(`- availableToolNames: ${checkpoint.toolSurface.finalToolNames.join(", ")}`);
    }
  }
