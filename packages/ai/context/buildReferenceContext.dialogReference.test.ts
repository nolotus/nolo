...
describe("dialog reference user stories", () => {
  it("builds a conversation reference that supports resume/reference stories without loading irrelevant early history", async () => {
    ...
    const dialogRecord = {
      ...
      runtimeCheckpoint: {
        ...
        toolSurface: {
          finalToolNames: ["read", "searchDialogMessages", "applyEdit"],
        },
      },
    };
    ...
    expect(content).toContain("availableToolNames: read, searchDialogMessages, applyEdit");
  ...
  it("does not emit availableToolNames line for a legacy-only availableToolNames checkpoint", async () => {
    const allMessages = makeOriginalDialogMessages();
    const recentMessages = allMessages.slice(-20).reverse();
    const { fetchReferenceContents } = await loadFetchReferenceContents(recentMessages);

    const dialogKey = "dialog-user-01LEGACYTOOL0000000001";
    const dialogRecord = {
      type: DataType.DIALOG,
      dbKey: dialogKey,
      title: "旧对话：仅携带 legacy availableToolNames",
      status: "done",
      summary: "旧 checkpoint 只有 availableToolNames，没有 toolSurface。",
      runtimeCheckpoint: {
        status: "done",
        lastToolNames: ["legacyTool"],
    ...
    const content = result.get(dialogKey) ?? "";
    expect(content).toContain("Runtime Checkpoint:");
    expect(content).toContain("lastToolNames: legacyTool");
    expect(content).not.toContain("availableToolNames:");
  });
});
