...
const entrySource = await Bun.file(new URL("./entry.ts", import.meta.url)).text();

describe("desktopRuntimeRoutes", () => {
  it("passes the route-family map directly to Bun.serve", () => {
    expect(entrySource).toContain("const routes = desktopRuntimeRoutes;");
    expect(entrySource).not.toContain("for (const route of desktopRuntimeRoutes)");
  });

  it("does not serve the SPA document for unknown API routes", () => {
    expect(entrySource).toContain(`if (url.pathname.startsWith("/api/")) {
