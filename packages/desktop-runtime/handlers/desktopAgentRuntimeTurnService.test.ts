...
describe("desktop agent runtime turn service", () => {
  ...
  const DEFAULT_LOCAL_CODING_TOOL_NAMES = WORKSPACE_TOOL_NAMES.filter(
  ...
  const SHELL_LOCAL_CODING_TOOL_NAMES = [
    ...orderWorkspaceToolNames(buildLocalWorkspaceToolset({
  ...
  test("runs a text-only local turn through the desktop record store and provider", async () => {
    ...
    expect(reads).toEqual([
      { key: "agent-user-1-frontend", remote: undefined },
    ...
    expect(providerRequests[0]).toMatchObject({
      ...
      body: { model: "qwen-coder", stream: false },
    });
    expect(providerRequests[0]?.body.messages[0]).toMatchObject({ role: "system" });
    expect(providerRequests[0]?.body.messages[0]?.content).toContain(
      "Fix UI with concise implementation steps.",
    );
    expect(providerRequests[0]?.body.messages.slice(1)).toEqual([
  ...
  test("uses request-scoped agentConfigSnapshot when host store has no agent record", async () => {
    ...
    expect(providerBodies[0]?.messages[0]).toMatchObject({ role: "system" });
    expect(providerBodies[0]?.messages[0]?.content).toContain("You are a local agent");
    expect(providerBodies[0]?.messages.slice(1)).toEqual([
      { role: "user", content: "previous" },
      { role: "assistant", content: "prior reply" },
      ...
      expect(providerRequests[1]?.body.messages).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            ...
            content: expect.stringContaining("dialog output"),
          }),
        ])
      );
      expect(result).toMatchObject({
    ...
    try {
      const store = new Map<string, any>([
        ["agent-user-1-general", {
          ...
          model: "qwen-coder",
          toolNames: ["globFiles", "readFile", "writeFile", "editFile"],
        }],
      ]);
      const toolCalls = [
        ...
        { name: "globFiles", arguments: { path: "notes", pattern: "*" } },
      ];
      const requests: Array<{ url?: string; body: any }> = [];
      let completeCount = 0;
...
describe("quick-chat tier agent workspace tool gating", () => {
  test("does not inject workspace tools for tier agent without workspaceToolsHint", async () => {
    ...
    const toolNames = (providerRequests[0]?.body.tools ?? []).map(
      (tool: any) => tool.function?.name ?? tool.name,
    );
    // No workspace tools, and FORCED_TOOLS is now empty (ask_user moved to
    // explicit opt-in) — a tier agent with no declared tools gets none.
    expect(toolNames).toEqual([]);
  });
  ...
  test("mounts full code-planning skill toolset for tier agent with workspaceToolsHint=true", async () => {
    ...
    expect(toolNames).toContain("globFiles");
...
describe("desktop startAgentRun runtime inheritance and web bridges", () => {
  ...
  test("exposes startAgentRun only when declared; undeclared is not exposed and not callable", async () => {
    ...
    await runDesktopAgentRuntimeTurn({
      ...
      store: createParentDialogStore("agent-with-call", ["startAgentRun", "globFiles"]),
    ...
    await runDesktopAgentRuntimeTurn({
      ...
      store: createParentDialogStore("agent-no-call", ["globFiles"]),
  ...
  test("does not expose undeclared web tools", async () => {
    ...
    await runDesktopAgentRuntimeTurn({
      ...
      store: createParentDialogStore("agent-no-web", ["globFiles"]),
...
describe("desktop agent runtime boundFolder resolution", () => {
  ...
  test("resolves boundFolder from dialog space when cwd is not provided", async () => {
    ...
    try {
      ...
      expect(providerRequests[1]?.body.messages.at(-1).content).toContain("wrote hello.txt");
    } finally {
      rmSync(workspaceRoot, { recursive: true, force: true });
...
describe("desktop agent runtime turn context layers", () => {
  ...
  test("adds no context layers when the dialog has no spaceId", async () => {
    ...
    expect(systemContent).toContain("You are ctx.");
    expect(systemContent).not.toContain("当前空间（Space）");
  });
  ...
  test("skips dialog-record resolution entirely when dialogKey is absent or mismatched (env derivation removed)", async () => {
    ...
    expect(absent.systemContent).toContain("You are ctx.");
    expect(absent.systemContent).not.toContain("上下文空间");

    const mismatched = await runContextTurn({
      records,
      dialogKey: "dialog-user-1-other-dialog",
    });
    expect(mismatched.systemContent).toContain("You are ctx.");
    expect(mismatched.systemContent).not.toContain("上下文空间");
  });
});
...
describe("desktop agent runtime T12-T14 context layers (dialog summary / global prompt / memory)", () => {
  ...
  test("T13 skips the global prompt layer when there is no dialog record (no continueDialogId)", async () => {
    ...
    expect(systemContent).toContain("You are layer.");
    expect(systemContent).not.toContain("用户全局偏好");
    expect(systemContent).not.toContain("平台回复语言策略");
  });
  ...
  test("layer order keeps session memory before turn-scoped global prompt, space, and summary", async () => {
    const records = new Map<string, any>([
      ["agent-user-1-layer", baseAgentRecord],
      ["dialog-user-1-dlg-layer", {
        dbKey: "dialog-user-1-dlg-layer",
    ...
    expect(globalIdx).toBeGreaterThan(memoryIdx);
    expect(spaceIdx).toBeGreaterThan(globalIdx);
    expect(summaryIdx).toBeGreaterThan(spaceIdx);
  });
