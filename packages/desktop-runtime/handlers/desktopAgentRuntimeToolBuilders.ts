...
export function buildDesktopLocalWorkspaceToolset(args: {
  ...
  // Exclude FORCED_TOOLS from the "all chrome connector?" check (currently empty;
  // kept for structure so future forced tools don't flip a chrome-only agent
  // into full-workspace mode).
  const nonForcedNames = toolNames.filter((n) => !FORCED_TOOLS.has(n));
