...
// packages/ai/token/query.ts
var iterateDb = async (db, options, filter) => {
  const records = [];
...
var getTokenStats = async (db, params) => {
...
// packages/ai/token/queryUserTokens.ts
var queryUserTokens = async (db, params) => {
    ...
    for await (const [_, value] of db.iterator({
      ...
      if (!model || value.model === model) {
        if (count >= offset && records.length < pageSize) {
          records.push({
            ...value,
            createdAt: typeof value.createdAt === "number" ? value.createdAt : value.timestamp
          });
        }
        count++;
      }
...
// packages/ai/token/tokenThunks.ts
var getTokenStatsThunk = createAsyncThunk("token/getStats", async (params, { extra }) => {
  const { db } = extra;
  if (!db) throw new Error("Database not available");
  return await getTokenStats(db, params);
});
var queryUserTokensThunk = createAsyncThunk("token/queryUserTokens", async (params, { extra }) => {
  const { db } = extra;
  if (!db) throw new Error("Database not available");
  return await queryUserTokens(db, params);
});

// packages/ai/token/hooks/useRecords.ts
var import_react = __toESM(require_react());
var ITEMS_PER_PAGE = 10;
var useRecords = (userId, filter) => {
  const dispatch = useAppDispatch();
  const [records, setRecords] = (0, import_react.useState)([]);
  const [totalCount, setTotalCount] = (0, import_react.useState)(0);
  const [loading, setLoading] = (0, import_react.useState)(false);
  const fetchRecords = (0, import_react.useCallback)(async () => {
    try {
      setLoading(true);
      ...
      const result = await dispatch(queryUserTokensThunk({
        userId,
        startTime,
        model: filter.model === "\u5168\u90E8\u6A21\u578B" ? void 0 : filter.model,
...
function getTimeZoneOffset(timeZone, originalDate) {
  var timeZoneOffset = timeZone ? tzParseTimezone(timeZone, originalDate, true) / MILLISECONDS_IN_MINUTE3 : originalDate.getTimezoneOffset();
