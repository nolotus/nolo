...
var TreeStreamImpl = class extends StreamImpl {
  constructor(root4, children, options) {
    super(() => ({
      iterators: (options === null || options === void 0 ? void 0 : options.includeRoot) ? [[root4][Symbol.iterator]()] : [children(root4)[Symbol.iterator]()],
      pruned: false
    }), (state) => {
      if (state.pruned) {
        state.iterators.pop();
...
function streamContents(node, options) {
  ...
  return new StreamImpl(() => ({
    ...
    while (state.keyIndex < state.keys.length) {
      const property4 = state.keys[state.keyIndex];
      if (!property4.startsWith("$")) {
        const value = node[property4];
        if (isAstNode(value)) {
          state.keyIndex++;
...
function streamAllContents(root4, options) {
  if (!root4) {
    throw new Error("Root node must be an AstNode.");
  }
  return new TreeStreamImpl(root4, (node) => streamContents(node, options));
}
function streamAst(root4, options) {
  if (!root4) {
    throw new Error("Root node must be an AstNode.");
  } else if ((options === null || options === void 0 ? void 0 : options.range) && !isAstNodeInRange(root4, options.range)) {
    return new TreeStreamImpl(root4, () => []);
  }
  return new TreeStreamImpl(root4, (node) => streamContents(node, options), { includeRoot: true });
}
    ...
    while (state.keyIndex < state.keys.length) {
      ...
      if (!property4.startsWith("$")) {
        ...
        if (isReference(value)) {
          state.keyIndex++;
          return { done: false, value: { reference: value, container: node, property: property4 } };
        } else if (Array.isArray(value)) {
          while (state.arrayIndex < value.length) {
            const index = state.arrayIndex++;
            const element = value[index];
            if (isReference(element)) {
              return { done: false, value: { reference: element, container: node, property: property4, index } };
            }
          }
          state.arrayIndex = 0;
        }
...
function assignMandatoryProperties(reflection3, node) {
  ...
  for (const property4 of typeMetaData.properties) {
    if (property4.defaultValue !== void 0 && genericNode[property4.name] === void 0) {
      genericNode[property4.name] = copyDefaultValue(property4.defaultValue);
    }
  }
}
...
function findNodesForProperty(node, property4) {
  if (!node || !property4) {
    return [];
  }
  return findNodesForPropertyInternal(node, property4, node.astNode, true);
}
function findNodeForProperty(node, property4, index) {
  if (!node || !property4) {
    return void 0;
  }
  const nodes = findNodesForPropertyInternal(node, property4, node.astNode, true);
  if (nodes.length === 0) {
    return void 0;
  }
  if (index !== void 0) {
...
function findNodesForPropertyInternal(node, property4, element, first2) {
  if (!first2) {
    const nodeFeature = getContainerOfType(node.grammarSource, isAssignment);
    if (nodeFeature && nodeFeature.feature === property4) {
      return [node];
    }
  }
  if (isCompositeCstNode(node) && node.astNode === element) {
    return node.content.flatMap((e) => findNodesForPropertyInternal(e, property4, element, false));
  }
  return [];
}
...
function baseDifference(array, values3, iteratee, comparator) {
  var index = -1, includes3 = arrayIncludes_default, isCommon2 = true, length = array.length, result = [], valuesLength = values3.length;
  if (!length) {
    return result;
  }
  if (iteratee) {
...
var INFINITY4 = 1 / 0;
var createSet = !(Set_default && 1 / setToArray_default(new Set_default([, -0]))[1] == INFINITY4) ? noop_default : function(values3) {
...
function baseUniq(array, iteratee, comparator) {
  var index = -1, includes3 = arrayIncludes_default, length = array.length, isCommon2 = true, result = [], seen = result;
  if (comparator) {
    isCommon2 = false;
    includes3 = arrayIncludesWith_default;
...
var trimmedEndIndex_default2 = trimmedEndIndex2;

// node_modules/@chevrotain/gast/node_modules/lodash-es/_baseTrim.js
var reTrimStart2 = /^\s+/;
function baseTrim2(string) {
  return string ? string.slice(0, trimmedEndIndex_default2(string) + 1).replace(reTrimStart2, "") : string;
}
var baseTrim_default2 = baseTrim2;
...
function toNumber2(value) {
  ...
  value = baseTrim_default2(value);
  var isBinary = reIsBinary2.test(value);
  return isBinary || reIsOctal2.test(value) ? freeParseInt2(value.slice(2), isBinary ? 2 : 8) : reIsBadHex2.test(value) ? NAN2 : +value;
}
var toNumber_default2 = toNumber2;
...
var toFinite_default2 = toFinite2;

// node_modules/@chevrotain/gast/node_modules/lodash-es/toInteger.js
function toInteger2(value) {
  var result = toFinite_default2(value), remainder = result % 1;
  return result === result ? remainder ? result - remainder : result : 0;
}
var toInteger_default2 = toInteger2;
...
function assignValue2(object, key, value) {
  var objValue = object[key];
  if (!(hasOwnProperty20.call(object, key) && eq_default2(objValue, value)) || value === void 0 && !(key in object)) {
...
var copyObject_default2 = copyObject2;
...
function baseRest2(func, start) {
  return setToString_default2(overRest_default2(func, start, identity_default2), func + "");
}
var baseRest_default2 = baseRest2;
...
function isIterateeCall2(value, index, object) {
  ...
  var type = typeof index;
  if (type == "number" ? isArrayLike_default2(object) && isIndex_default2(index, object.length) : type == "string" && index in object) {
    return eq_default2(object[index], value);
  }
  return false;
}
var isIterateeCall_default2 = isIterateeCall2;

// node_modules/@chevrotain/gast/node_modules/lodash-es/_createAssigner.js
function createAssigner2(assigner) {
  return baseRest_default2(function(object, sources) {
    var index = -1, length = sources.length, customizer = length > 1 ? sources[length - 1] : void 0, guard = length > 2 ? sources[2] : void 0;
    customizer = assigner.length > 3 && typeof customizer == "function" ? (length--, customizer) : void 0;
    if (guard && isIterateeCall_default2(sources[0], sources[1], guard)) {
      customizer = length < 3 ? void 0 : customizer;
      length = 1;
    }
    object = Object(object);
...
var createAssigner_default2 = createAssigner2;

// node_modules/@chevrotain/gast/node_modules/lodash-es/_isPrototype.js
var objectProto25 = Object.prototype;
function isPrototype2(value) {
  var Ctor = value && value.constructor, proto = typeof Ctor == "function" && Ctor.prototype || objectProto25;
  return value === proto;
}
...
var isArguments2 = baseIsArguments_default2(/* @__PURE__ */ function() {
  ...
  return isObjectLike_default2(value) && hasOwnProperty21.call(value, "callee") && !propertyIsEnumerable3.call(value, "callee");
};
...
var Buffer3 = moduleExports4 ? root_default2.Buffer : void 0;
...
function arrayLikeKeys2(value, inherited) {
  ...
  for (var key in value) {
    if ((inherited || hasOwnProperty22.call(value, key)) && !(skipIndexes && // Safari 9 has enumerable `arguments.length` in strict mode.
    (key == "length" || // Node.js 0.10 has enumerable non-index properties on buffers.
    isBuff && (key == "offset" || key == "parent") || // PhantomJS 2 has enumerable non-index properties on typed arrays.
    isType2 && (key == "buffer" || key == "byteLength" || key == "byteOffset") || // Skip index properties.
...
var assign2 = createAssigner_default2(function(object, source) {
  if (isPrototype_default2(source) || isArrayLike_default2(source)) {
    copyObject_default2(source, keys_default2(source), object);
    return;
  }
  for (var key in source) {
    if (hasOwnProperty24.call(source, key)) {
      assignValue_default2(object, key, source[key]);
    }
  }
});
var assign_default2 = assign2;
...
function baseKeysIn2(object) {
  ...
  for (var key in object) {
    if (!(key == "constructor" && (isProto || !hasOwnProperty25.call(object, key)))) {
      result.push(key);
    }
  }
  return result;
...
var hasOwnProperty26 = objectProto31.hasOwnProperty;
...
var hasOwnProperty27 = objectProto32.hasOwnProperty;
function hashHas2(key) {
  var data = this.__data__;
  return nativeCreate_default2 ? data[key] !== void 0 : hasOwnProperty27.call(data, key);
}
...
function toKey2(value) {
  ...
  var result = value + "";
  return result == "0" && 1 / value == -INFINITY7 ? "-0" : result;
}
...
var getSymbolsIn2 = !nativeGetSymbols4 ? stubArray_default2 : function(object) {
  var result = [];
  while (object) {
...
if (DataView_default2 && getTag2(new DataView_default2(new ArrayBuffer(1))) != dataViewTag7 || Map_default2 && getTag2(new Map_default2()) != mapTag9 || Promise_default2 && getTag2(Promise_default2.resolve()) != promiseTag2 || Set_default2 && getTag2(new Set_default2()) != setTag9 || WeakMap_default2 && getTag2(new WeakMap_default2()) != weakMapTag5) {
...
var Uint8Array_default2 = Uint8Array2;
...
function equalObjects2(object, other, bitmask, customizer, equalFunc, stack) {
  ...
  while (index--) {
    var key = objProps[index];
    if (!(isPartial ? key in other : hasOwnProperty28.call(other, key))) {
      return false;
    }
  }
  var objStacked = stack.get(object);
...
var hasOwnProperty29 = objectProto35.hasOwnProperty;
function baseIsEqualDeep2(object, other, bitmask, customizer, equalFunc, stack) {
  ...
  if (!(bitmask & COMPARE_PARTIAL_FLAG10)) {
    var objIsWrapped = objIsObj && hasOwnProperty29.call(object, "__wrapped__"), othIsWrapped = othIsObj && hasOwnProperty29.call(other, "__wrapped__");
    if (objIsWrapped || othIsWrapped) {
      var objUnwrapped = objIsWrapped ? object.value() : object, othUnwrapped = othIsWrapped ? other.value() : other;
      stack || (stack = new Stack_default2());
      return equalFunc(objUnwrapped, othUnwrapped, bitmask, customizer, stack);
...
var arrayEvery_default2 = arrayEvery2;
...
var baseEvery_default2 = baseEvery2;

// node_modules/@chevrotain/gast/node_modules/lodash-es/every.js
function every2(collection, predicate, guard) {
  var func = isArray_default2(collection) ? arrayEvery_default2 : baseEvery_default2;
  if (guard && isIterateeCall_default2(collection, predicate, guard)) {
    predicate = void 0;
  }
...
var stringTag8 = "[object String]";
function isString2(value) {
  return typeof value == "string" || !isArray_default2(value) && isObjectLike_default2(value) && baseGetTag_default2(value) == stringTag8;
}
...
function baseValues2(object, props) {
  return arrayMap_default2(props, function(key) {
    return object[key];
  });
}
var baseValues_default2 = baseValues2;

// node_modules/@chevrotain/gast/node_modules/lodash-es/values.js
function values2(object) {
  return object == null ? [] : baseValues_default2(object, keys_default2(object));
}
...
function includes2(collection, value, fromIndex, guard) {
  collection = isArrayLike_default2(collection) ? collection : values_default2(collection);
  fromIndex = fromIndex && !guard ? toInteger_default2(fromIndex) : 0;
  var length = collection.length;
  if (fromIndex < 0) {
    fromIndex = nativeMax6(length + fromIndex, 0);
  }
  return isString_default2(collection) ? fromIndex <= length && collection.indexOf(value, fromIndex) > -1 : !!length && baseIndexOf_default2(collection, value, fromIndex) > -1;
}
var includes_default2 = includes2;

// node_modules/@chevrotain/gast/node_modules/lodash-es/_baseIsRegExp.js
var regexpTag8 = "[object RegExp]";
function baseIsRegExp2(value) {
  return isObjectLike_default2(value) && baseGetTag_default2(value) == regexpTag8;
}
var baseIsRegExp_default2 = baseIsRegExp2;

// node_modules/@chevrotain/gast/node_modules/lodash-es/isRegExp.js
var nodeIsRegExp2 = nodeUtil_default2 && nodeUtil_default2.isRegExp;
var isRegExp2 = nodeIsRegExp2 ? baseUnary_default2(nodeIsRegExp2) : baseIsRegExp_default2;
var isRegExp_default2 = isRegExp2;
...
var baseSome_default2 = baseSome2;

// node_modules/@chevrotain/gast/node_modules/lodash-es/some.js
function some2(collection, predicate, guard) {
  var func = isArray_default2(collection) ? arraySome_default2 : baseSome_default2;
  if (guard && isIterateeCall_default2(collection, predicate, guard)) {
    predicate = void 0;
  }
  return func(collection, baseIteratee_default2(predicate, 3));
...
function hasTokenLabel(obj) {
  return isString_default2(obj.LABEL) && obj.LABEL !== "";
}
...
var NonTerminal = class extends AbstractProduction {
  constructor(options) {
    super([]);
    this.idx = 1;
    assign_default2(this, pickBy_default2(options, (v) => v !== void 0));
  }
...
function serializeProduction(node) {
  ...
  if (node instanceof NonTerminal) {
    ...
    if (isString_default2(node.label)) {
...
function addToMapOfArrays(map4, key, value) {
  if (map4[key] === void 0) {
    map4[key] = [value];
  } else {
    map4[key].push(value);
...
function lookAheadSequenceFromAlternatives(altsDefs, k) {
  ...
  const altsHashes = map_default(partialAlts, (currAltPaths) => {
    ...
    forEach_default(currAltPaths, (item) => {
      const keys4 = pathToHashKeys(item.partialPath);
      forEach_default(keys4, (currKey) => {
        dict[currKey] = true;
      });
    });
    return dict;
...
// node_modules/chevrotain-allstar/node_modules/lodash-es/_arrayMap.js
...
var arrayMap_default3 = arrayMap3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_listCacheClear.js
function listCacheClear3() {
  this.__data__ = [];
  this.size = 0;
}
var listCacheClear_default3 = listCacheClear3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/eq.js
function eq3(value, other) {
  return value === other || value !== value && other !== other;
}
var eq_default3 = eq3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_assocIndexOf.js
function assocIndexOf3(array, key) {
  var length = array.length;
  while (length--) {
    if (eq_default3(array[length][0], key)) {
      return length;
    }
  }
  return -1;
}
var assocIndexOf_default3 = assocIndexOf3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_listCacheDelete.js
var arrayProto3 = Array.prototype;
var splice3 = arrayProto3.splice;
function listCacheDelete3(key) {
  var data = this.__data__, index = assocIndexOf_default3(data, key);
  if (index < 0) {
    return false;
  }
  var lastIndex = data.length - 1;
...
var listCacheDelete_default3 = listCacheDelete3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_listCacheGet.js
function listCacheGet3(key) {
  var data = this.__data__, index = assocIndexOf_default3(data, key);
  return index < 0 ? void 0 : data[index][1];
}
var listCacheGet_default3 = listCacheGet3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_listCacheHas.js
function listCacheHas3(key) {
  return assocIndexOf_default3(this.__data__, key) > -1;
}
var listCacheHas_default3 = listCacheHas3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_listCacheSet.js
...
var listCacheSet_default3 = listCacheSet3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_ListCache.js
...
var ListCache_default3 = ListCache3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_stackClear.js
function stackClear3() {
  this.__data__ = new ListCache_default3();
  this.size = 0;
}
var stackClear_default3 = stackClear3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_stackDelete.js
function stackDelete3(key) {
  var data = this.__data__, result = data["delete"](key);
  this.size = data.size;
  return result;
}
var stackDelete_default3 = stackDelete3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_stackGet.js
function stackGet3(key) {
  return this.__data__.get(key);
}
var stackGet_default3 = stackGet3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_stackHas.js
function stackHas3(key) {
  return this.__data__.has(key);
}
var stackHas_default3 = stackHas3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_freeGlobal.js
var freeGlobal3 = typeof global == "object" && global && global.Object === Object && global;
var freeGlobal_default3 = freeGlobal3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_root.js
var freeSelf3 = typeof self == "object" && self && self.Object === Object && self;
var root3 = freeGlobal_default3 || freeSelf3 || Function("return this")();
var root_default3 = root3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_Symbol.js
var Symbol4 = root_default3.Symbol;
var Symbol_default3 = Symbol4;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_getRawTag.js
var objectProto36 = Object.prototype;
var hasOwnProperty30 = objectProto36.hasOwnProperty;
var nativeObjectToString5 = objectProto36.toString;
var symToStringTag5 = Symbol_default3 ? Symbol_default3.toStringTag : void 0;
function getRawTag3(value) {
  var isOwn = hasOwnProperty30.call(value, symToStringTag5), tag = value[symToStringTag5];
  try {
...
var getRawTag_default3 = getRawTag3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_objectToString.js
var objectProto37 = Object.prototype;
var nativeObjectToString6 = objectProto37.toString;
function objectToString3(value) {
  return nativeObjectToString6.call(value);
}
var objectToString_default3 = objectToString3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseGetTag.js
var nullTag3 = "[object Null]";
var undefinedTag3 = "[object Undefined]";
var symToStringTag6 = Symbol_default3 ? Symbol_default3.toStringTag : void 0;
function baseGetTag3(value) {
  if (value == null) {
    return value === void 0 ? undefinedTag3 : nullTag3;
  }
  return symToStringTag6 && symToStringTag6 in Object(value) ? getRawTag_default3(value) : objectToString_default3(value);
}
var baseGetTag_default3 = baseGetTag3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/isObject.js
function isObject3(value) {
  var type = typeof value;
  return value != null && (type == "object" || type == "function");
}
var isObject_default3 = isObject3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/isFunction.js
...
function isFunction3(value) {
  if (!isObject_default3(value)) {
    return false;
  }
  var tag = baseGetTag_default3(value);
  return tag == funcTag6 || tag == genTag4 || tag == asyncTag3 || tag == proxyTag3;
}
var isFunction_default3 = isFunction3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_coreJsData.js
var coreJsData3 = root_default3["__core-js_shared__"];
var coreJsData_default3 = coreJsData3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_isMasked.js
...
var isMasked_default3 = isMasked3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_toSource.js
...
var toSource_default3 = toSource3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseIsNative.js
var reRegExpChar3 = /[\\^$.*+?()[\]{}|]/g;
var reIsHostCtor3 = /^\[object .+?Constructor\]$/;
var funcProto6 = Function.prototype;
var objectProto38 = Object.prototype;
var funcToString6 = funcProto6.toString;
var hasOwnProperty31 = objectProto38.hasOwnProperty;
var reIsNative3 = RegExp(
  "^" + funcToString6.call(hasOwnProperty31).replace(reRegExpChar3, "\\$&").replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, "$1.*?") + "$"
);
function baseIsNative3(value) {
  if (!isObject_default3(value) || isMasked_default3(value)) {
    return false;
  }
  var pattern = isFunction_default3(value) ? reIsNative3 : reIsHostCtor3;
  return pattern.test(toSource_default3(value));
}
var baseIsNative_default3 = baseIsNative3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_getValue.js
function getValue3(object, key) {
  return object == null ? void 0 : object[key];
}
var getValue_default3 = getValue3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_getNative.js
function getNative3(object, key) {
  var value = getValue_default3(object, key);
  return baseIsNative_default3(value) ? value : void 0;
}
var getNative_default3 = getNative3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_Map.js
var Map4 = getNative_default3(root_default3, "Map");
var Map_default3 = Map4;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_nativeCreate.js
var nativeCreate3 = getNative_default3(Object, "create");
var nativeCreate_default3 = nativeCreate3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_hashClear.js
function hashClear3() {
  this.__data__ = nativeCreate_default3 ? nativeCreate_default3(null) : {};
  this.size = 0;
}
var hashClear_default3 = hashClear3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_hashDelete.js
function hashDelete3(key) {
  var result = this.has(key) && delete this.__data__[key];
  this.size -= result ? 1 : 0;
  return result;
}
var hashDelete_default3 = hashDelete3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_hashGet.js
var HASH_UNDEFINED7 = "__lodash_hash_undefined__";
var objectProto39 = Object.prototype;
var hasOwnProperty32 = objectProto39.hasOwnProperty;
function hashGet3(key) {
  ...
  return hasOwnProperty32.call(data, key) ? data[key] : void 0;
}
var hashGet_default3 = hashGet3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_hashHas.js
var objectProto40 = Object.prototype;
var hasOwnProperty33 = objectProto40.hasOwnProperty;
function hashHas3(key) {
  var data = this.__data__;
  return nativeCreate_default3 ? data[key] !== void 0 : hasOwnProperty33.call(data, key);
}
var hashHas_default3 = hashHas3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_hashSet.js
var HASH_UNDEFINED8 = "__lodash_hash_undefined__";
function hashSet3(key, value) {
  var data = this.__data__;
  this.size += this.has(key) ? 0 : 1;
  data[key] = nativeCreate_default3 && value === void 0 ? HASH_UNDEFINED8 : value;
  return this;
}
var hashSet_default3 = hashSet3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_Hash.js
...
var Hash_default3 = Hash3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_mapCacheClear.js
...
var mapCacheClear_default3 = mapCacheClear3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_isKeyable.js
function isKeyable3(value) {
  var type = typeof value;
  return type == "string" || type == "number" || type == "symbol" || type == "boolean" ? value !== "__proto__" : value === null;
}
var isKeyable_default3 = isKeyable3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_getMapData.js
function getMapData3(map4, key) {
  var data = map4.__data__;
  return isKeyable_default3(key) ? data[typeof key == "string" ? "string" : "hash"] : data.map;
}
var getMapData_default3 = getMapData3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_mapCacheDelete.js
function mapCacheDelete3(key) {
  var result = getMapData_default3(this, key)["delete"](key);
  this.size -= result ? 1 : 0;
  return result;
}
var mapCacheDelete_default3 = mapCacheDelete3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_mapCacheGet.js
function mapCacheGet3(key) {
  return getMapData_default3(this, key).get(key);
}
var mapCacheGet_default3 = mapCacheGet3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_mapCacheHas.js
function mapCacheHas3(key) {
  return getMapData_default3(this, key).has(key);
}
var mapCacheHas_default3 = mapCacheHas3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_mapCacheSet.js
function mapCacheSet3(key, value) {
  var data = getMapData_default3(this, key), size = data.size;
  data.set(key, value);
  this.size += data.size == size ? 0 : 1;
  return this;
}
var mapCacheSet_default3 = mapCacheSet3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_MapCache.js
...
var MapCache_default3 = MapCache3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_stackSet.js
var LARGE_ARRAY_SIZE5 = 200;
function stackSet3(key, value) {
  var data = this.__data__;
  if (data instanceof ListCache_default3) {
    var pairs = data.__data__;
    if (!Map_default3 || pairs.length < LARGE_ARRAY_SIZE5 - 1) {
      pairs.push([key, value]);
      this.size = ++data.size;
      return this;
    }
    data = this.__data__ = new MapCache_default3(pairs);
  }
  data.set(key, value);
  this.size = data.size;
  return this;
}
var stackSet_default3 = stackSet3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_Stack.js
function Stack3(entries) {
  var data = this.__data__ = new ListCache_default3(entries);
  this.size = data.size;
}
...
var Stack_default3 = Stack3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_setCacheAdd.js
var HASH_UNDEFINED9 = "__lodash_hash_undefined__";
function setCacheAdd3(value) {
  this.__data__.set(value, HASH_UNDEFINED9);
  return this;
}
var setCacheAdd_default3 = setCacheAdd3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_setCacheHas.js
function setCacheHas3(value) {
  return this.__data__.has(value);
}
var setCacheHas_default3 = setCacheHas3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_SetCache.js
...
var SetCache_default3 = SetCache3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_arraySome.js
...
var arraySome_default3 = arraySome3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_cacheHas.js
function cacheHas3(cache, key) {
  return cache.has(key);
}
var cacheHas_default3 = cacheHas3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_equalArrays.js
var COMPARE_PARTIAL_FLAG13 = 1;
var COMPARE_UNORDERED_FLAG9 = 2;
function equalArrays3(array, other, bitmask, customizer, equalFunc, stack) {
  var isPartial = bitmask & COMPARE_PARTIAL_FLAG13, arrLength = array.length, othLength = other.length;
  if (arrLength != othLength && !(isPartial && othLength > arrLength)) {
    return false;
  }
  var arrStacked = stack.get(array);
...
var equalArrays_default3 = equalArrays3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_Uint8Array.js
var Uint8Array3 = root_default3.Uint8Array;
var Uint8Array_default3 = Uint8Array3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_mapToArray.js
function mapToArray3(map4) {
  var index = -1, result = Array(map4.size);
  map4.forEach(function(value, key) {
    result[++index] = [key, value];
  });
...
var mapToArray_default3 = mapToArray3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_setToArray.js
...
var setToArray_default3 = setToArray3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_equalByTag.js
var COMPARE_PARTIAL_FLAG14 = 1;
var COMPARE_UNORDERED_FLAG10 = 2;
var boolTag7 = "[object Boolean]";
var dateTag7 = "[object Date]";
var errorTag6 = "[object Error]";
var mapTag11 = "[object Map]";
var numberTag7 = "[object Number]";
var regexpTag9 = "[object RegExp]";
var setTag11 = "[object Set]";
var stringTag9 = "[object String]";
var symbolTag7 = "[object Symbol]";
var arrayBufferTag7 = "[object ArrayBuffer]";
var dataViewTag9 = "[object DataView]";
var symbolProto6 = Symbol_default3 ? Symbol_default3.prototype : void 0;
var symbolValueOf4 = symbolProto6 ? symbolProto6.valueOf : void 0;
function equalByTag3(object, other, tag, bitmask, customizer, equalFunc, stack) {
  switch (tag) {
    case dataViewTag9:
      ...
      object = object.buffer;
      other = other.buffer;
    case arrayBufferTag7:
      if (object.byteLength != other.byteLength || !equalFunc(new Uint8Array_default3(object), new Uint8Array_default3(other))) {
        return false;
      }
      return true;
    case boolTag7:
    case dateTag7:
    case numberTag7:
      return eq_default3(+object, +other);
    case errorTag6:
      return object.name == other.name && object.message == other.message;
    case regexpTag9:
    case stringTag9:
    ...
    case setTag11:
      ...
      stack.set(object, other);
      var result = equalArrays_default3(convert(object), convert(other), bitmask, customizer, equalFunc, stack);
      stack["delete"](object);
      return result;
    case symbolTag7:
      if (symbolValueOf4) {
...
var equalByTag_default3 = equalByTag3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_arrayPush.js
function arrayPush3(array, values3) {
  var index = -1, length = values3.length, offset = array.length;
  while (++index < length) {
    array[offset + index] = values3[index];
  }
...
var arrayPush_default3 = arrayPush3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/isArray.js
var isArray3 = Array.isArray;
var isArray_default3 = isArray3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseGetAllKeys.js
function baseGetAllKeys3(object, keysFunc, symbolsFunc) {
  var result = keysFunc(object);
  return isArray_default3(object) ? result : arrayPush_default3(result, symbolsFunc(object));
}
var baseGetAllKeys_default3 = baseGetAllKeys3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_arrayFilter.js
...
var arrayFilter_default3 = arrayFilter3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/stubArray.js
function stubArray3() {
  return [];
}
var stubArray_default3 = stubArray3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_getSymbols.js
var objectProto41 = Object.prototype;
var propertyIsEnumerable5 = objectProto41.propertyIsEnumerable;
var nativeGetSymbols5 = Object.getOwnPropertySymbols;
var getSymbols3 = !nativeGetSymbols5 ? stubArray_default3 : function(object) {
  if (object == null) {
    return [];
  }
  object = Object(object);
  return arrayFilter_default3(nativeGetSymbols5(object), function(symbol) {
    return propertyIsEnumerable5.call(object, symbol);
  });
};
var getSymbols_default3 = getSymbols3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseTimes.js
function baseTimes3(n, iteratee) {
  var index = -1, result = Array(n);
  while (++index < n) {
    result[index] = iteratee(index);
  }
...
var baseTimes_default3 = baseTimes3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/isObjectLike.js
function isObjectLike3(value) {
  return value != null && typeof value == "object";
}
var isObjectLike_default3 = isObjectLike3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseIsArguments.js
var argsTag8 = "[object Arguments]";
function baseIsArguments3(value) {
  return isObjectLike_default3(value) && baseGetTag_default3(value) == argsTag8;
}
var baseIsArguments_default3 = baseIsArguments3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/isArguments.js
var objectProto42 = Object.prototype;
var hasOwnProperty34 = objectProto42.hasOwnProperty;
var propertyIsEnumerable6 = objectProto42.propertyIsEnumerable;
var isArguments3 = baseIsArguments_default3(/* @__PURE__ */ function() {
  return arguments;
}()) ? baseIsArguments_default3 : function(value) {
  return isObjectLike_default3(value) && hasOwnProperty34.call(value, "callee") && !propertyIsEnumerable6.call(value, "callee");
};
var isArguments_default3 = isArguments3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/stubFalse.js
function stubFalse3() {
  return false;
}
var stubFalse_default3 = stubFalse3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/isBuffer.js
...
var Buffer4 = moduleExports6 ? root_default3.Buffer : void 0;
var nativeIsBuffer3 = Buffer4 ? Buffer4.isBuffer : void 0;
var isBuffer3 = nativeIsBuffer3 || stubFalse_default3;
var isBuffer_default3 = isBuffer3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_isIndex.js
...
var isIndex_default3 = isIndex3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/isLength.js
var MAX_SAFE_INTEGER6 = 9007199254740991;
function isLength3(value) {
  return typeof value == "number" && value > -1 && value % 1 == 0 && value <= MAX_SAFE_INTEGER6;
}
var isLength_default3 = isLength3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseIsTypedArray.js
var argsTag9 = "[object Arguments]";
var arrayTag6 = "[object Array]";
var boolTag8 = "[object Boolean]";
var dateTag8 = "[object Date]";
var errorTag7 = "[object Error]";
var funcTag7 = "[object Function]";
var mapTag12 = "[object Map]";
var numberTag8 = "[object Number]";
var objectTag8 = "[object Object]";
var regexpTag10 = "[object RegExp]";
var setTag12 = "[object Set]";
var stringTag10 = "[object String]";
var weakMapTag6 = "[object WeakMap]";
var arrayBufferTag8 = "[object ArrayBuffer]";
var dataViewTag10 = "[object DataView]";
...
typedArrayTags3[argsTag9] = typedArrayTags3[arrayTag6] = typedArrayTags3[arrayBufferTag8] = typedArrayTags3[boolTag8] = typedArrayTags3[dataViewTag10] = typedArrayTags3[dateTag8] = typedArrayTags3[errorTag7] = typedArrayTags3[funcTag7] = typedArrayTags3[mapTag12] = typedArrayTags3[numberTag8] = typedArrayTags3[objectTag8] = typedArrayTags3[regexpTag10] = typedArrayTags3[setTag12] = typedArrayTags3[stringTag10] = typedArrayTags3[weakMapTag6] = false;
function baseIsTypedArray3(value) {
  return isObjectLike_default3(value) && isLength_default3(value.length) && !!typedArrayTags3[baseGetTag_default3(value)];
}
var baseIsTypedArray_default3 = baseIsTypedArray3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseUnary.js
function baseUnary3(func) {
  return function(value) {
    return func(value);
  };
}
var baseUnary_default3 = baseUnary3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_nodeUtil.js
...
var freeProcess3 = moduleExports7 && freeGlobal_default3.process;
var nodeUtil3 = function() {
  try {
    var types = freeModule7 && freeModule7.require && freeModule7.require("util").types;
    if (types) {
      return types;
    }
...
var nodeUtil_default3 = nodeUtil3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/isTypedArray.js
var nodeIsTypedArray3 = nodeUtil_default3 && nodeUtil_default3.isTypedArray;
var isTypedArray3 = nodeIsTypedArray3 ? baseUnary_default3(nodeIsTypedArray3) : baseIsTypedArray_default3;
var isTypedArray_default3 = isTypedArray3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_arrayLikeKeys.js
var objectProto43 = Object.prototype;
var hasOwnProperty35 = objectProto43.hasOwnProperty;
function arrayLikeKeys3(value, inherited) {
  var isArr = isArray_default3(value), isArg = !isArr && isArguments_default3(value), isBuff = !isArr && !isArg && isBuffer_default3(value), isType2 = !isArr && !isArg && !isBuff && isTypedArray_default3(value), skipIndexes = isArr || isArg || isBuff || isType2, result = skipIndexes ? baseTimes_default3(value.length, String) : [], length = result.length;
  for (var key in value) {
    if ((inherited || hasOwnProperty35.call(value, key)) && !(skipIndexes && // Safari 9 has enumerable `arguments.length` in strict mode.
    (key == "length" || // Node.js 0.10 has enumerable non-index properties on buffers.
    isBuff && (key == "offset" || key == "parent") || // PhantomJS 2 has enumerable non-index properties on typed arrays.
    isType2 && (key == "buffer" || key == "byteLength" || key == "byteOffset") || // Skip index properties.
    isIndex_default3(key, length)))) {
      result.push(key);
    }
  }
  return result;
}
var arrayLikeKeys_default3 = arrayLikeKeys3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_isPrototype.js
var objectProto44 = Object.prototype;
function isPrototype3(value) {
  var Ctor = value && value.constructor, proto = typeof Ctor == "function" && Ctor.prototype || objectProto44;
  return value === proto;
}
var isPrototype_default3 = isPrototype3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_overArg.js
function overArg3(func, transform) {
  return function(arg) {
    return func(transform(arg));
  };
}
var overArg_default3 = overArg3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_nativeKeys.js
var nativeKeys3 = overArg_default3(Object.keys, Object);
var nativeKeys_default3 = nativeKeys3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseKeys.js
var objectProto45 = Object.prototype;
var hasOwnProperty36 = objectProto45.hasOwnProperty;
function baseKeys3(object) {
  if (!isPrototype_default3(object)) {
    return nativeKeys_default3(object);
  }
  var result = [];
  for (var key in Object(object)) {
    if (hasOwnProperty36.call(object, key) && key != "constructor") {
      result.push(key);
    }
  }
  return result;
}
var baseKeys_default3 = baseKeys3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/isArrayLike.js
function isArrayLike3(value) {
  return value != null && isLength_default3(value.length) && !isFunction_default3(value);
}
var isArrayLike_default3 = isArrayLike3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/keys.js
function keys3(object) {
  return isArrayLike_default3(object) ? arrayLikeKeys_default3(object) : baseKeys_default3(object);
}
var keys_default3 = keys3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_getAllKeys.js
function getAllKeys3(object) {
  return baseGetAllKeys_default3(object, keys_default3, getSymbols_default3);
}
var getAllKeys_default3 = getAllKeys3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_equalObjects.js
var COMPARE_PARTIAL_FLAG15 = 1;
var objectProto46 = Object.prototype;
var hasOwnProperty37 = objectProto46.hasOwnProperty;
function equalObjects3(object, other, bitmask, customizer, equalFunc, stack) {
  var isPartial = bitmask & COMPARE_PARTIAL_FLAG15, objProps = getAllKeys_default3(object), objLength = objProps.length, othProps = getAllKeys_default3(other), othLength = othProps.length;
  if (objLength != othLength && !isPartial) {
    return false;
  }
  var index = objLength;
  while (index--) {
    var key = objProps[index];
    if (!(isPartial ? key in other : hasOwnProperty37.call(other, key))) {
      return false;
    }
  }
  var objStacked = stack.get(object);
...
var equalObjects_default3 = equalObjects3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_DataView.js
var DataView3 = getNative_default3(root_default3, "DataView");
var DataView_default3 = DataView3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_Promise.js
var Promise4 = getNative_default3(root_default3, "Promise");
var Promise_default3 = Promise4;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_Set.js
var Set4 = getNative_default3(root_default3, "Set");
var Set_default3 = Set4;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_WeakMap.js
var WeakMap3 = getNative_default3(root_default3, "WeakMap");
var WeakMap_default3 = WeakMap3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_getTag.js
var mapTag13 = "[object Map]";
var objectTag9 = "[object Object]";
var promiseTag3 = "[object Promise]";
var setTag13 = "[object Set]";
var weakMapTag7 = "[object WeakMap]";
var dataViewTag11 = "[object DataView]";
var dataViewCtorString3 = toSource_default3(DataView_default3);
var mapCtorString3 = toSource_default3(Map_default3);
var promiseCtorString3 = toSource_default3(Promise_default3);
var setCtorString3 = toSource_default3(Set_default3);
var weakMapCtorString3 = toSource_default3(WeakMap_default3);
var getTag3 = baseGetTag_default3;
if (DataView_default3 && getTag3(new DataView_default3(new ArrayBuffer(1))) != dataViewTag11 || Map_default3 && getTag3(new Map_default3()) != mapTag13 || Promise_default3 && getTag3(Promise_default3.resolve()) != promiseTag3 || Set_default3 && getTag3(new Set_default3()) != setTag13 || WeakMap_default3 && getTag3(new WeakMap_default3()) != weakMapTag7) {
  getTag3 = function(value) {
    var result = baseGetTag_default3(value), Ctor = result == objectTag9 ? value.constructor : void 0, ctorString = Ctor ? toSource_default3(Ctor) : "";
    if (ctorString) {
      switch (ctorString) {
        case dataViewCtorString3:
          return dataViewTag11;
        case mapCtorString3:
          return mapTag13;
...
var getTag_default3 = getTag3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseIsEqualDeep.js
var COMPARE_PARTIAL_FLAG16 = 1;
var argsTag10 = "[object Arguments]";
var arrayTag7 = "[object Array]";
var objectTag10 = "[object Object]";
var objectProto47 = Object.prototype;
var hasOwnProperty38 = objectProto47.hasOwnProperty;
function baseIsEqualDeep3(object, other, bitmask, customizer, equalFunc, stack) {
  var objIsArr = isArray_default3(object), othIsArr = isArray_default3(other), objTag = objIsArr ? arrayTag7 : getTag_default3(object), othTag = othIsArr ? arrayTag7 : getTag_default3(other);
  objTag = objTag == argsTag10 ? objectTag10 : objTag;
  othTag = othTag == argsTag10 ? objectTag10 : othTag;
  var objIsObj = objTag == objectTag10, othIsObj = othTag == objectTag10, isSameTag = objTag == othTag;
  if (isSameTag && isBuffer_default3(object)) {
  ...
  if (isSameTag && !objIsObj) {
    ...
    return objIsArr || isTypedArray_default3(object) ? equalArrays_default3(object, other, bitmask, customizer, equalFunc, stack) : equalByTag_default3(object, other, objTag, bitmask, customizer, equalFunc, stack);
  }
  if (!(bitmask & COMPARE_PARTIAL_FLAG16)) {
    var objIsWrapped = objIsObj && hasOwnProperty38.call(object, "__wrapped__"), othIsWrapped = othIsObj && hasOwnProperty38.call(other, "__wrapped__");
    if (objIsWrapped || othIsWrapped) {
      var objUnwrapped = objIsWrapped ? object.value() : object, othUnwrapped = othIsWrapped ? other.value() : other;
      stack || (stack = new Stack_default3());
      return equalFunc(objUnwrapped, othUnwrapped, bitmask, customizer, stack);
  ...
  stack || (stack = new Stack_default3());
  return equalObjects_default3(object, other, bitmask, customizer, equalFunc, stack);
}
var baseIsEqualDeep_default3 = baseIsEqualDeep3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseIsEqual.js
function baseIsEqual3(value, other, bitmask, customizer, stack) {
  if (value === other) {
    return true;
  }
  if (value == null || other == null || !isObjectLike_default3(value) && !isObjectLike_default3(other)) {
    return value !== value && other !== other;
  }
  return baseIsEqualDeep_default3(value, other, bitmask, customizer, baseIsEqual3, stack);
}
var baseIsEqual_default3 = baseIsEqual3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseIsMatch.js
var COMPARE_PARTIAL_FLAG17 = 1;
var COMPARE_UNORDERED_FLAG11 = 2;
function baseIsMatch3(object, source, matchData, customizer) {
  var index = matchData.length, length = index, noCustomizer = !customizer;
  if (object == null) {
    return !length;
  }
...
var baseIsMatch_default3 = baseIsMatch3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_isStrictComparable.js
function isStrictComparable3(value) {
  return value === value && !isObject_default3(value);
}
var isStrictComparable_default3 = isStrictComparable3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_getMatchData.js
function getMatchData3(object) {
  var result = keys_default3(object), length = result.length;
  while (length--) {
    var key = result[length], value = object[key];
    result[length] = [key, value, isStrictComparable_default3(value)];
  }
  return result;
}
var getMatchData_default3 = getMatchData3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_matchesStrictComparable.js
...
var matchesStrictComparable_default3 = matchesStrictComparable3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseMatches.js
function baseMatches3(source) {
  ...
  return function(object) {
    return object === source || baseIsMatch_default3(object, source, matchData);
  };
}
var baseMatches_default3 = baseMatches3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/isSymbol.js
var symbolTag8 = "[object Symbol]";
function isSymbol3(value) {
  return typeof value == "symbol" || isObjectLike_default3(value) && baseGetTag_default3(value) == symbolTag8;
}
var isSymbol_default3 = isSymbol3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_isKey.js
var reIsDeepProp3 = /\.|\[(?:[^[\]]*|(["'])(?:(?!\1)[^\\]|\\.)*?\1)\]/;
var reIsPlainProp3 = /^\w*$/;
function isKey3(value, object) {
  if (isArray_default3(value)) {
    return false;
  }
  var type = typeof value;
  if (type == "number" || type == "symbol" || type == "boolean" || value == null || isSymbol_default3(value)) {
    return true;
  }
  return reIsPlainProp3.test(value) || !reIsDeepProp3.test(value) || object != null && value in Object(object);
}
var isKey_default3 = isKey3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/memoize.js
...
function memoize3(func, resolver) {
  ...
  memoized.cache = new (memoize3.Cache || MapCache_default3)();
  return memoized;
}
memoize3.Cache = MapCache_default3;
var memoize_default3 = memoize3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_memoizeCapped.js
...
var memoizeCapped_default3 = memoizeCapped3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_stringToPath.js
var rePropName3 = /[^.[\]]+|\[(?:(-?\d+(?:\.\d+)?)|(["'])((?:(?!\2)[^\\]|\\.)*?)\2)\]|(?=(?:\.|\[\])(?:\.|\[\]|$))/g;
var reEscapeChar3 = /\\(\\)?/g;
var stringToPath3 = memoizeCapped_default3(function(string) {
  var result = [];
  if (string.charCodeAt(0) === 46) {
    result.push("");
  }
  string.replace(rePropName3, function(match, number, quote, subString) {
    result.push(quote ? subString.replace(reEscapeChar3, "$1") : number || match);
  });
  return result;
});
var stringToPath_default3 = stringToPath3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseToString.js
var INFINITY8 = 1 / 0;
var symbolProto7 = Symbol_default3 ? Symbol_default3.prototype : void 0;
var symbolToString3 = symbolProto7 ? symbolProto7.toString : void 0;
function baseToString3(value) {
  if (typeof value == "string") {
    return value;
  }
  if (isArray_default3(value)) {
    return arrayMap_default3(value, baseToString3) + "";
  }
  if (isSymbol_default3(value)) {
    return symbolToString3 ? symbolToString3.call(value) : "";
  }
  var result = value + "";
  return result == "0" && 1 / value == -INFINITY8 ? "-0" : result;
}
var baseToString_default3 = baseToString3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/toString.js
function toString4(value) {
  return value == null ? "" : baseToString_default3(value);
}
var toString_default3 = toString4;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_castPath.js
function castPath3(value, object) {
  if (isArray_default3(value)) {
    return value;
  }
  return isKey_default3(value, object) ? [value] : stringToPath_default3(toString_default3(value));
}
var castPath_default3 = castPath3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_toKey.js
var INFINITY9 = 1 / 0;
function toKey3(value) {
  if (typeof value == "string" || isSymbol_default3(value)) {
    return value;
  }
  var result = value + "";
  return result == "0" && 1 / value == -INFINITY9 ? "-0" : result;
}
var toKey_default3 = toKey3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseGet.js
function baseGet3(object, path) {
  path = castPath_default3(path, object);
  var index = 0, length = path.length;
  while (object != null && index < length) {
    object = object[toKey_default3(path[index++])];
  }
  return index && index == length ? object : void 0;
}
var baseGet_default3 = baseGet3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/get.js
function get3(object, path, defaultValue) {
  var result = object == null ? void 0 : baseGet_default3(object, path);
  return result === void 0 ? defaultValue : result;
}
var get_default3 = get3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseHasIn.js
function baseHasIn3(object, key) {
  return object != null && key in Object(object);
}
var baseHasIn_default3 = baseHasIn3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_hasPath.js
function hasPath3(object, path, hasFunc) {
  path = castPath_default3(path, object);
  var index = -1, length = path.length, result = false;
  while (++index < length) {
    var key = toKey_default3(path[index]);
    if (!(result = object != null && hasFunc(object, key))) {
      break;
    }
    object = object[key];
  ...
  length = object == null ? 0 : object.length;
  return !!length && isLength_default3(length) && isIndex_default3(key, length) && (isArray_default3(object) || isArguments_default3(object));
}
var hasPath_default3 = hasPath3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/hasIn.js
function hasIn3(object, path) {
  return object != null && hasPath_default3(object, path, baseHasIn_default3);
}
var hasIn_default3 = hasIn3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseMatchesProperty.js
var COMPARE_PARTIAL_FLAG18 = 1;
var COMPARE_UNORDERED_FLAG12 = 2;
function baseMatchesProperty3(path, srcValue) {
  if (isKey_default3(path) && isStrictComparable_default3(srcValue)) {
    return matchesStrictComparable_default3(toKey_default3(path), srcValue);
  }
  return function(object) {
    var objValue = get_default3(object, path);
    return objValue === void 0 && objValue === srcValue ? hasIn_default3(object, path) : baseIsEqual_default3(srcValue, objValue, COMPARE_PARTIAL_FLAG18 | COMPARE_UNORDERED_FLAG12);
  };
}
var baseMatchesProperty_default3 = baseMatchesProperty3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/identity.js
function identity3(value) {
  return value;
}
var identity_default3 = identity3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseProperty.js
function baseProperty3(key) {
  return function(object) {
    return object == null ? void 0 : object[key];
  };
}
var baseProperty_default3 = baseProperty3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_basePropertyDeep.js
function basePropertyDeep3(path) {
  return function(object) {
    return baseGet_default3(object, path);
  };
}
var basePropertyDeep_default3 = basePropertyDeep3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/property.js
function property3(path) {
  return isKey_default3(path) ? baseProperty_default3(toKey_default3(path)) : basePropertyDeep_default3(path);
}
var property_default3 = property3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseIteratee.js
function baseIteratee3(value) {
  ...
  if (typeof value == "object") {
    return isArray_default3(value) ? baseMatchesProperty_default3(value[0], value[1]) : baseMatches_default3(value);
  }
  return property_default3(value);
}
var baseIteratee_default3 = baseIteratee3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_createBaseFor.js
...
var createBaseFor_default3 = createBaseFor3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseFor.js
var baseFor3 = createBaseFor_default3();
var baseFor_default3 = baseFor3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseForOwn.js
function baseForOwn3(object, iteratee) {
  return object && baseFor_default3(object, iteratee, keys_default3);
}
var baseForOwn_default3 = baseForOwn3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_createBaseEach.js
function createBaseEach3(eachFunc, fromRight) {
  return function(collection, iteratee) {
    if (collection == null) {
      return collection;
    }
    if (!isArrayLike_default3(collection)) {
      return eachFunc(collection, iteratee);
    }
    var length = collection.length, index = fromRight ? length : -1, iterable = Object(collection);
    while (fromRight ? index-- : ++index < length) {
...
var createBaseEach_default3 = createBaseEach3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseEach.js
var baseEach3 = createBaseEach_default3(baseForOwn_default3);
var baseEach_default3 = baseEach3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseMap.js
function baseMap3(collection, iteratee) {
  var index = -1, result = isArrayLike_default3(collection) ? Array(collection.length) : [];
  baseEach_default3(collection, function(value, key, collection2) {
    result[++index] = iteratee(value, key, collection2);
  });
  return result;
}
var baseMap_default3 = baseMap3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/map.js
function map3(collection, iteratee) {
  var func = isArray_default3(collection) ? arrayMap_default3 : baseMap_default3;
  return func(collection, baseIteratee_default3(iteratee, 3));
}
var map_default3 = map3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseFilter.js
function baseFilter2(collection, predicate) {
  var result = [];
  baseEach_default3(collection, function(value, index, collection2) {
    if (predicate(value, index, collection2)) {
      result.push(value);
    }
  });
...
// node_modules/chevrotain-allstar/node_modules/lodash-es/filter.js
function filter2(collection, predicate) {
  var func = isArray_default3(collection) ? arrayFilter_default3 : baseFilter_default2;
  return func(collection, baseIteratee_default3(predicate, 3));
}
...
function alternation(atn, rule, alternation2) {
  ...
  const alts = map_default3(alternation2.definition, (e) => atom(atn, rule, e));
...
function block(atn, rule, block2) {
  const handles = filter_default2(map_default3(block2.definition, (e) => atom(atn, rule, e)), (e) => e !== void 0);
  if (handles.length === 1) {
    return handles[0];
...
var ATNConfigSet = class {
  ...
  get alts() {
    return map_default3(this.configs, (e) => e.alt);
  }
  get key() {
    let value = "";
...
// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseExtremum.js
function baseExtremum(array, iteratee, comparator) {
  var index = -1, length = array.length;
  while (++index < length) {
    var value = array[index], current = iteratee(value);
    if (current != null && (computed === void 0 ? current === current && !isSymbol_default3(current) : comparator(current, computed))) {
...
// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseLt.js
function baseLt(value, other) {
  return value < other;
}
var baseLt_default = baseLt;

// node_modules/chevrotain-allstar/node_modules/lodash-es/min.js
function min(array) {
  return array && array.length ? baseExtremum_default(array, identity_default3, baseLt_default) : void 0;
}
var min_default = min;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_isFlattenable.js
var spreadableSymbol2 = Symbol_default3 ? Symbol_default3.isConcatSpreadable : void 0;
function isFlattenable2(value) {
  return isArray_default3(value) || isArguments_default3(value) || !!(spreadableSymbol2 && value && value[spreadableSymbol2]);
}
var isFlattenable_default2 = isFlattenable2;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseFlatten.js
  ...
  while (++index < length) {
    ...
    if (depth > 0 && predicate(value)) {
      if (depth > 1) {
        ...
        arrayPush_default3(result, value);
      }
    } else if (!isStrict) {
      result[result.length] = value;
    }
...
// node_modules/chevrotain-allstar/node_modules/lodash-es/flatMap.js
function flatMap2(collection, iteratee) {
  return baseFlatten_default2(map_default3(collection, iteratee), 1);
}
var flatMap_default2 = flatMap2;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseFindIndex.js
function baseFindIndex3(array, predicate, fromIndex, fromRight) {
  var length = array.length, index = fromIndex + (fromRight ? 1 : -1);
  while (fromRight ? index-- : ++index < length) {
    if (predicate(array[index], index, array)) {
      return index;
...
var baseFindIndex_default3 = baseFindIndex3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseIsNaN.js
function baseIsNaN3(value) {
  return value !== value;
}
var baseIsNaN_default3 = baseIsNaN3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_strictIndexOf.js
function strictIndexOf3(array, value, fromIndex) {
  var index = fromIndex - 1, length = array.length;
  while (++index < length) {
    if (array[index] === value) {
      return index;
...
var strictIndexOf_default3 = strictIndexOf3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseIndexOf.js
function baseIndexOf3(array, value, fromIndex) {
  return value === value ? strictIndexOf_default3(array, value, fromIndex) : baseFindIndex_default3(array, baseIsNaN_default3, fromIndex);
}
var baseIndexOf_default3 = baseIndexOf3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_arrayIncludes.js
function arrayIncludes2(array, value) {
  var length = array == null ? 0 : array.length;
  return !!length && baseIndexOf_default3(array, value, 0) > -1;
}
var arrayIncludes_default2 = arrayIncludes2;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_arrayIncludesWith.js
...
// node_modules/chevrotain-allstar/node_modules/lodash-es/noop.js
function noop2() {
}
var noop_default2 = noop2;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_createSet.js
var INFINITY10 = 1 / 0;
var createSet2 = !(Set_default3 && 1 / setToArray_default3(new Set_default3([, -0]))[1] == INFINITY10) ? noop_default2 : function(values3) {
  return new Set_default3(values3);
};
var createSet_default2 = createSet2;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseUniq.js
var LARGE_ARRAY_SIZE6 = 200;
function baseUniq2(array, iteratee, comparator) {
  var index = -1, includes3 = arrayIncludes_default2, length = array.length, isCommon2 = true, result = [], seen = result;
  if (comparator) {
    isCommon2 = false;
    includes3 = arrayIncludesWith_default2;
  } else if (length >= LARGE_ARRAY_SIZE6) {
    var set = iteratee ? null : createSet_default2(array);
    if (set) {
      return setToArray_default3(set);
    }
    isCommon2 = false;
    includes3 = cacheHas_default3;
...
// node_modules/chevrotain-allstar/node_modules/lodash-es/uniqBy.js
function uniqBy(array, iteratee) {
  return array && array.length ? baseUniq_default2(array, baseIteratee_default3(iteratee, 2)) : [];
}
var uniqBy_default = uniqBy;

// node_modules/chevrotain-allstar/node_modules/lodash-es/flatten.js
...
// node_modules/chevrotain-allstar/node_modules/lodash-es/_arrayEach.js
...
var arrayEach_default3 = arrayEach3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/_castFunction.js
function castFunction3(value) {
  return typeof value == "function" ? value : identity_default3;
}
var castFunction_default3 = castFunction3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/forEach.js
function forEach3(collection, iteratee) {
  var func = isArray_default3(collection) ? arrayEach_default3 : baseEach_default3;
  return func(collection, castFunction_default3(iteratee));
}
var forEach_default3 = forEach3;

// node_modules/chevrotain-allstar/node_modules/lodash-es/isEmpty.js
var mapTag14 = "[object Map]";
var setTag14 = "[object Set]";
var objectProto48 = Object.prototype;
var hasOwnProperty39 = objectProto48.hasOwnProperty;
function isEmpty2(value) {
  if (value == null) {
    return true;
  }
  if (isArrayLike_default3(value) && (isArray_default3(value) || typeof value == "string" || typeof value.splice == "function" || isBuffer_default3(value) || isTypedArray_default3(value) || isArguments_default3(value))) {
    return !value.length;
  }
  var tag = getTag_default3(value);
  if (tag == mapTag14 || tag == setTag14) {
    return !value.size;
  }
  if (isPrototype_default3(value)) {
    return !baseKeys_default3(value).length;
  }
  for (var key in value) {
    if (hasOwnProperty39.call(value, key)) {
      return false;
    }
  }
  return true;
...
// node_modules/chevrotain-allstar/node_modules/lodash-es/_arrayReduce.js
...
// node_modules/chevrotain-allstar/node_modules/lodash-es/_baseReduce.js
...
// node_modules/chevrotain-allstar/node_modules/lodash-es/reduce.js
function reduce2(collection, iteratee, accumulator) {
  var func = isArray_default3(collection) ? arrayReduce_default2 : baseReduce_default2, initAccum = arguments.length < 3;
  return func(collection, baseIteratee_default3(iteratee, 4), accumulator, initAccum, baseEach_default3);
}
...
var LLStarLookaheadStrategy = class extends LLkLookaheadStrategy {
  ...
  buildLookaheadForAlternation(options) {
    ...
    const partialAlts = map_default3(getLookaheadPaths({
      maxLookahead: 1,
      occurrence: prodOccurrence,
      prodType: "Alternation",
      rule
    }), (currAlt) => map_default3(currAlt, (path) => path[0]));
  ...
  buildLookaheadForOptional(options) {
    ...
    const decisionState = this.atn.decisionMap[key];
    const decisionIndex = decisionState.decision;
    const alts = map_default3(getLookaheadPaths({
      maxLookahead: 1,
      occurrence: prodOccurrence,
      prodType,
      rule
    }), (e) => {
      return map_default3(e, (g) => g[0]);
    });
...
function buildAmbiguityError(options) {
  const pathMsg = map_default3(options.prefixPath, (currtok) => tokenLabel2(currtok)).join(", ");
...
(function(Is2) {
  ...
  Is2.boolean = boolean;
  function string(value) {
    return toString5.call(value) === "[object String]";
  }
  Is2.string = string;
  function number(value) {
    return toString5.call(value) === "[object Number]";
  }
  Is2.number = number;
  function numberRange(value, min2, max) {
    return toString5.call(value) === "[object Number]" && min2 <= value && value <= max;
  }
  Is2.numberRange = numberRange;
  function integer2(value) {
    return toString5.call(value) === "[object Number]" && -2147483648 <= value && value <= 2147483647;
  }
  Is2.integer = integer2;
  function uinteger2(value) {
    return toString5.call(value) === "[object Number]" && 0 <= value && value <= 2147483647;
  }
  Is2.uinteger = uinteger2;
  function func(value) {
    return toString5.call(value) === "[object Function]";
  }
  Is2.func = func;
...
var DefaultLinker = class {
  ...
  buildReference(node, property4, refNode, refText) {
    const linker = this;
    const reference = {
      $refNode: refNode,
      $refText: refText,
      get ref() {
        ...
        if (isAstNode(this._ref)) {
          ...
          this._ref = linkedNode !== null && linkedNode !== void 0 ? linkedNode : linker.createLinkingError({ reference, container: node, property: property4 }, this._nodeDescription);
        } else if (this._ref === void 0) {
          this._ref = ref_resolving;
          const document = findRootNode(node).$document;
          const refData = linker.getLinkedNode({ reference, container: node, property: property4 });
          ...
          throw new Error(`Cyclic reference resolution detected: ${linker.astNodeLocator.getAstNodePath(node)}/${property4} (symbol '${refText}')`);
        }
...
var MultiMap = class {
  ...
  delete(key, value) {
    if (value === void 0) {
      ...
      if (values3) {
        const index = values3.indexOf(value);
        if (index >= 0) {
          if (values3.length === 1) {
            this.map.delete(key);
          } else {
  ...
  has(key, value) {
    if (value === void 0) {
      ...
      const values3 = this.map.get(key);
      if (values3) {
        return values3.indexOf(value) >= 0;
      }
      return false;
    }
  }
  ...
  addAll(key, values3) {
    if (this.map.has(key)) {
      this.map.get(key).push(...values3);
    } else {
      this.map.set(key, Array.from(values3));
    }
    return this;
  }
  /**
...
var DefaultJsonSerializer = class {
  ...
  linkNode(node, root4, options, container, containerProperty, containerIndex) {
    for (const [propertyName, item] of Object.entries(node)) {
      if (Array.isArray(item)) {
        for (let index = 0; index < item.length; index++) {
          const element = item[index];
          if (isIntermediateReference(element)) {
            item[index] = this.reviveReference(node, propertyName, root4, element, options);
          } else if (isAstNode(element)) {
            this.linkNode(element, root4, options, node, propertyName, index);
          }
        }
      } else if (isIntermediateReference(item)) {
        node[propertyName] = this.reviveReference(node, propertyName, root4, item, options);
      } else if (isAstNode(item)) {
        this.linkNode(item, root4, options, node, propertyName);
      }
    }
    const mutable = node;
  ...
  reviveReference(container, property4, root4, reference, options) {
    let refText = reference.$refText;
    let error = reference.$error;
    if (reference.$ref) {
      const ref = this.getRefNode(root4, reference.$ref, options.uriConverter);
      if (isAstNode(ref)) {
        if (!refText) {
  ...
  getRefNode(root4, uri, uriConverter) {
    try {
      ...
      if (fragmentIndex === 0) {
        const node2 = this.astNodeLocator.getAstNode(root4, uri.substring(1));
        if (!node2) {
...
var DefaultAstNodeLocator = class {
  ...
  getAstNode(node, path) {
    ...
    return segments.reduce((previousValue, currentValue) => {
      ...
      if (propertyIndex > 0) {
        const property4 = currentValue.substring(0, propertyIndex);
        const arrayIndex = parseInt(currentValue.substring(propertyIndex + 1));
        const array = previousValue[property4];
