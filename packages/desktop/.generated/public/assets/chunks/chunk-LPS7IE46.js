...
// packages/ai/agent/cliProviders.ts
var CLI_PROVIDER_VALUES = [
  "copilot",
  "gemini",
  "codex",
  "claude",
...
function isCliProvider(value) {
  return typeof value === "string" && CLI_PROVIDER_VALUES.includes(value);
}
var CLI_CAPABILITY_BY_PROVIDER = {
  copilot: "copilot-cli",
  gemini: "gemini-cli",
  codex: "codex-cli",
  claude: "claude-code",
...
var CLI_PROVIDER_DISPLAY_LABELS = {
...
var CLI_PROVIDER_OPTIONS = CLI_PROVIDER_VALUES.map((value) => ({
  value,
  label: CLI_PROVIDER_DISPLAY_LABELS[value]
}));

// packages/core/localOrigins.ts
var DEFAULT_LOCAL_HOST = "127.0.0.1";
var DEFAULT_LOCAL_API_PORT = "38123";
var DEFAULT_LOCAL_API_ORIGIN = `http://${DEFAULT_LOCAL_HOST}:${DEFAULT_LOCAL_API_PORT}`;
...
var LOCAL_SERVER_URL_PATTERN = /^https?:\/\/(?:(?:\d{1,3}\.){3}\d{1,3}|localhost|nolotus\.local)(?::\d+)?$/i;
...
var PROVIDER_REASONING_EFFORT_VALUES = {
...
function getAvailableReasoningEfforts(provider) {
  const p = (provider ?? "").toLowerCase();
  if (p === "anthropic" || p === "google") return [];
  return PROVIDER_REASONING_EFFORT_VALUES[p] ?? ["low", "medium", "high"];
}
var greetingMenuItemSchema = external_exports.object({
  id: external_exports.string().min(1),
  label: external_exports.string().min(1),
  userMessage: external_exports.string().optional()
});
var greetingConfigSchema = external_exports.object({
  text: external_exports.string().trim().optional(),
  menu: external_exports.array(greetingMenuItemSchema).optional()
});
var runtimeToolPolicySchema = external_exports.object({
  version: external_exports.literal(1).optional(),
  agentTools: external_exports.array(external_exports.string()).optional(),
  runtimeTools: external_exports.array(external_exports.string()).optional(),
  workspace: external_exports.object({
    mode: external_exports.enum(["none", "current", "lease"]).optional(),
    writableRoots: external_exports.array(external_exports.string()).optional(),
    cwd: external_exports.string().optional()
  }).optional(),
  shell: external_exports.record(external_exports.unknown()).optional(),
  isolation: external_exports.record(external_exports.unknown()).optional(),
  git: external_exports.record(external_exports.unknown()).optional(),
  budget: external_exports.record(external_exports.unknown()).optional(),
  audit: external_exports.record(external_exports.unknown()).optional()
}).passthrough();
function isLocalCustomProviderUrl(value) {
  return isLoopbackUrl(value);
}
function runtimePolicyAllowsHostedExec(value) {
  if (!isRecord(value)) return false;
  const policy = value;
  return Array.isArray(policy.runtimeTools) && policy.runtimeTools.includes("execShell") || policy.workspace?.mode === "lease";
}
var referenceItemSchema = external_exports.object({
  dbKey: external_exports.string(),
  title: external_exports.string(),
  type: external_exports.enum(["knowledge", "instruction", "page"])
}).transform((data) => ({
  ...data,
  type: data.type === "page" ? "knowledge" : data.type
}));
var getCreateAgentSchema = (t) => external_exports.object({
  name: external_exports.string().trim().min(1, t("validation.nameRequired")).max(50, t("validation.nameTooLong")),
  /**
   ...
   */
  handle: external_exports.string().trim().nullable().optional().or(external_exports.string().length(0)),
  /**
   * provider 不再必填：只是一个可选的标识字段
   */
  provider: external_exports.string().trim().nullable().optional().or(external_exports.string().length(0)),
  /**
   * 模型：所有模式下必须选择（通过 refine 条件校验）
   */
  model: external_exports.string().trim().nullable().optional().or(external_exports.string().length(0)),
  /**
   * 模型是否支持图像（来自模型元数据，方便持久化 / 服务端快速判断）
   */
  hasVision: external_exports.boolean().optional().default(false),
  /**
   * API 来源：平台 / 自定义 / CLI
   */
  apiSource: external_exports.enum(["platform", "custom", "cli"]).default("platform"),
  /**
   * 默认交互模式：文本聊天 / 实时语音通话
   */
  defaultInteractionMode: external_exports.enum(["text", "live_audio"]).default("text"),
  /**
   * 语音配置：例如预设音色
   */
  voiceConfig: external_exports.object({
    voiceId: external_exports.string().optional()
  }).nullable().optional(),
  /**
   * CLI provider（apiSource=cli 时有效）：见 CLI_PROVIDER_VALUES
   */
  cliProvider: external_exports.enum(CLI_PROVIDER_VALUES).nullable().optional().or(external_exports.literal("")),
  /**
   ...
   */
  machineId: external_exports.string().trim().nullable().optional().or(external_exports.string().length(0)),
  customProviderUrl: external_exports.string().trim().nullable().optional().or(external_exports.string().length(0)).refine((val) => !val || external_exports.string().url().safeParse(val).success, {
    message: t("validation.invalidUrl")
  }),
  /**
   * API Key：完全可选（本地 / 无鉴权的自定义接口不需要）
   */
  apiKey: external_exports.string().trim().nullable().optional().or(external_exports.string().length(0)),
  /**
   * apiKeyRef：指向 OAuth 凭据库的 provider 名称（例如 "chatgpt"）。
   * 设置后由 provider 解析层加载对应 OAuth token 作为 Bearer 鉴权，
   * 与静态 apiKey 互斥优先使用 apiKeyRef。
   */
  apiKeyRef: external_exports.string().trim().nullable().optional().or(external_exports.string().length(0)),
  /**
   ...
   */
  credentialSynced: external_exports.boolean().optional(),
  /**
   * apiKeyHeader：自定义鉴权 header 名（例如 "x-api-key"）。
   * 不传时按 endpoint 自动推断，通常为 "Authorization"。
   */
  apiKeyHeader: external_exports.string().trim().nullable().optional().or(external_exports.string().length(0)),
  useServerProxy: external_exports.boolean().default(true),
  prompt: external_exports.string().trim().nullable().optional().or(external_exports.string().length(0)),
  tools: external_exports.array(external_exports.string()).default([]),
  disabledTools: external_exports.array(external_exports.string()).default([]),
  enabledPacks: external_exports.array(external_exports.string()).default([]),
  runtimeToolPolicy: runtimeToolPolicySchema.nullable().optional(),
  isPublic: external_exports.boolean().default(false),
  allowFork: external_exports.boolean().default(false),
  greeting: external_exports.union([external_exports.string(), greetingConfigSchema]).nullable().optional(),
  introduction: external_exports.string().trim().nullable().optional().or(external_exports.string().length(0)),
  inputPrice: external_exports.number().min(0, t("validation.priceMin")).default(0),
  outputPrice: external_exports.number().min(0, t("validation.priceMin")).default(0),
  sharingLevel: external_exports.enum(["default", "split", "full"]).nullable().optional(),
  avatarFileId: external_exports.string().nullable().optional().or(external_exports.string().length(0)),
  tags: external_exports.string().trim().nullable().optional().or(external_exports.string().length(0)),
  references: external_exports.array(referenceItemSchema).optional().default([]).refine(
    (refs) => {
  ...
  temperature: external_exports.number().min(0, t("validation.temperatureRange")).max(2, t("validation.temperatureRange")).nullable().optional(),
  top_p: external_exports.number().min(0, t("validation.topPRange")).max(1, t("validation.topPRange")).nullable().optional(),
  frequency_penalty: external_exports.number().min(-2, t("validation.frequencyPenaltyRange")).max(2, t("validation.frequencyPenaltyRange")).nullable().optional(),
  presence_penalty: external_exports.number().min(-2, t("validation.presencePenaltyRange")).max(2, t("validation.presencePenaltyRange")).nullable().optional(),
  max_tokens: external_exports.number().min(1, t("validation.maxTokensMin")).max(MAX_TOKENS_LIMIT, t("validation.maxTokensMax")).nullable().optional(),
  reasoning_effort: external_exports.enum(REASONING_EFFORT_OPTIONS, {
    errorMap: () => ({ message: t("validation.reasoningEffortInvalid") })
  }).nullable().optional().transform((v) => v ?? DEFAULT_REASONING_EFFORT),
  /**
   ...
   */
  enableThinking: external_exports.boolean().optional().default(false),
  /**
   ...
   */
  thinkingBudget: external_exports.number().min(1024, t("validation.thinkingBudgetMin")).max(32e3, t("validation.thinkingBudgetMax")).nullable().optional(),
  /**
   * whitelist：白名单
   */
  whitelist: external_exports.array(external_exports.string().trim().min(1)).optional().default([]),
  /**
   * linkedSpaces：关联的其他 Space ID 列表
   * Agent 可以访问这些 Space 的目录结构作为粗略上下文
   */
  linkedSpaces: external_exports.array(external_exports.string().trim().min(1)).optional().default([])
}).refine(
  (data) => {
    if (data.apiSource === "custom") {
      const oauthRefs = /* @__PURE__ */ new Set([
        "chatgpt",
        "xai",
        "antigravity",
        "claude",
  ...
  const canUseMachineBinding = data.apiSource === "cli" || data.apiSource === "custom" && isLocalCustomProviderUrl(data.customProviderUrl);
...
var oauthModel = ({ name, displayName, contextWindow, thinkingMode, maxOutputTokens }) => ({
  name,
  displayName,
  hasVision: true,
...
var anthropicOAuthModels = [
  // 5 代（1M + Adaptive thinking always-on；官方 max output 128k）
  oauthModel({ name: "claude-fable-5", displayName: "Claude Fable 5", contextWindow: 1e6, thinkingMode: "adaptive", maxOutputTokens: 128e3 }),
  oauthModel({ name: "claude-opus-5", displayName: "Claude Opus 5", contextWindow: 1e6, thinkingMode: "adaptive", maxOutputTokens: 128e3 }),
  oauthModel({ name: "claude-sonnet-5", displayName: "Claude Sonnet 5", contextWindow: 1e6, thinkingMode: "adaptive", maxOutputTokens: 128e3 }),
  // 4.6/4.7/4.8 系（Adaptive thinking；context 官方未单列，参照 fuzzy claude-4.6→1M）
  oauthModel({ name: "claude-opus-4-8", displayName: "Claude Opus 4.8", contextWindow: 1e6, thinkingMode: "adaptive" }),
  oauthModel({ name: "claude-opus-4-7", displayName: "Claude Opus 4.7", contextWindow: 1e6, thinkingMode: "adaptive" }),
  oauthModel({ name: "claude-sonnet-4-6", displayName: "Claude Sonnet 4.6", contextWindow: 1e6, thinkingMode: "adaptive" }),
  oauthModel({ name: "claude-opus-4-6", displayName: "Claude Opus 4.6", contextWindow: 1e6, thinkingMode: "adaptive" }),
  // 4.5 系（Extended thinking；官方发布规格 1M）
  oauthModel({ name: "claude-opus-4-5-20251101", displayName: "Claude Opus 4.5", contextWindow: 1e6, thinkingMode: "extended" }),
  oauthModel({ name: "claude-sonnet-4-5-20250929", displayName: "Claude Sonnet 4.5", contextWindow: 1e6, thinkingMode: "extended" }),
  // haiku-4-5（官方明确 context=200k + Extended thinking）
  oauthModel({ name: "claude-haiku-4-5-20251001", displayName: "Claude Haiku 4.5", contextWindow: 2e5, thinkingMode: "extended" })
];
var OAUTH_MODEL_THINKING_MODE = new Map(
  anthropicOAuthModels.map((m) => [m.name, m.thinkingMode])
);

// packages/integrations/qwen/models.ts
var qwenModels = [
  // ── Qwen3 旗舰 ──
  {
    name: "qwen3.8-max",
...
var kimiCodeModels = [
  // ── 旗舰编程 ──
  {
    ...
    price: { input: 0, output: 0 },
    provider: "moonshot",
    description: "Kimi \u6700\u5F3A\u65D7\u8230\u7F16\u7A0B\u6A21\u578B\uFF0C2.8T \u53C2\u6570\uFF0C1M \u4E0A\u4E0B\u6587\uFF0Clow/high/max \u4E09\u6863\u601D\u8003\u6DF1\u5EA6\uFF0C\u64C5\u957F\u590D\u6742\u5DE5\u7A0B\u4E0E\u957F\u7A0B\u63A8\u7406\u3002Moderato \u53CA\u4EE5\u4E0A\u53EF\u7528\u3002"
  },
  // ── 旗舰编程（256K 上下文版）──
  {
    name: "k3-256k",
    ...
    price: { input: 0, output: 0 },
    provider: "moonshot",
    description: "K3 \u7684 256K \u4E0A\u4E0B\u6587\u7248\uFF0C\u6D88\u8017\u7EA6\u4E3A k3 \u7684\u4E00\u534A\uFF0C\u9002\u5408\u65E5\u5E38\u95EE\u7B54\u4E0E\u4EE3\u7801\u8865\u5168\u3002\u4E0D\u652F\u6301\u89C6\u9891\u8F93\u5165\u3002Moderato \u53CA\u4EE5\u4E0A\u53EF\u7528\u3002"
  },
  // ── 成熟编程 ──
  {
    ...
    price: { input: 0, output: 0 },
    provider: "moonshot",
    description: "\u6210\u719F\u7A33\u5B9A\u7684 Coding \u6A21\u578B\uFF0CThinking \u5F00\u542F\u4E0B\u53EF\u9760\u9075\u5FAA\u6307\u4EE4\uFF0C\u7F16\u7A0B\u4EFB\u52A1\u6210\u529F\u7387\u9AD8\u3002\u6240\u6709\u4F1A\u5458\u53EF\u7528\u3002"
  },
  // ── 高速编程 ──
  {
    ...
    price: { input: 0, output: 0 },
    provider: "moonshot",
    description: "K2.7 Code \u9AD8\u901F\u7248\uFF0C\u7F16\u7801\u80FD\u529B\u4E00\u81F4\uFF0C\u8F93\u51FA\u901F\u5EA6\u7EA6 6 \u500D\u3001\u6D88\u8017\u7EA6 3 \u500D\u3002Allegretto \u53CA\u4EE5\u4E0A\u53EF\u7528\u3002"
  }
];

// packages/ai/llm/reasoningModels.ts
var REASONING_MODEL_NAMES = /* @__PURE__ */ new Set([
  "deepseek-v4-flash",
  "deepseek-v4-pro",
  "gemini-3.6-flash",
...
var isModelSupportReasoningEffort = (model) => REASONING_MODEL_NAMES.has(model);
var supportedReasoningModels = Array.from(REASONING_MODEL_NAMES);
var EFFORT_ORDER = [
  "none",
  "minimal",
  "low",
...
function nearestSupportedEffort(effort, supported) {
  const effortIdx = EFFORT_ORDER.indexOf(effort);
  if (effortIdx === -1) return void 0;
  const supportedIdxs = supported.map((s) => EFFORT_ORDER.indexOf(s)).filter((idx) => idx >= 0).sort((a, b) => a - b);
  if (supportedIdxs.length === 0) return void 0;
...
function clampReasoningEffort(effort, provider) {
...
var moonshotAllModels = [...moonshotModels, ...kimiCodeModels];
var qwenAllModels = [...qwenModels, ...qwenTokenPlanModels];
var MODEL_MAP = {
...
var normalizeLookupProvider = (provider) => {
...
var normalizeLookupModelName = (provider, name) => {
  if (provider === "anthropic") {
    return ANTHROPIC_MODEL_ALIASES[name] ?? name;
  }
  return name;
};
var toModel = (candidate) => ({
  name: typeof candidate.id === "string" && candidate.id.trim() ? candidate.id : candidate.name,
  displayName: candidate.displayName ?? (typeof candidate.id === "string" && candidate.id !== candidate.name ? candidate.name : void 0),
  hasVision: typeof candidate.hasVision === "boolean" ? candidate.hasVision : !!candidate.supportVision,
  ...
  fnCall: typeof candidate.fnCall === "boolean" ? candidate.fnCall : candidate.supportTool,
  ...
  supportsTool: typeof candidate.supportsTool === "boolean" ? candidate.supportsTool : candidate.supportTool,
  ...
  supportsReasoningEffort: typeof candidate.supportsReasoningEffort === "boolean" ? candidate.supportsReasoningEffort : candidate.supportReasoning,
  endpointKey: candidate.endpointKey
});
var findCandidateInProvider = (provider, name) => {
  const normalizedName = normalizeLookupModelName(provider, name);
  const list = MODEL_LOOKUP_MAP[provider];
  const candidate = list.find(
    (item) => item.name === normalizedName || item.displayName === normalizedName || item.id === normalizedName
  );
  return candidate ? toModel(candidate) : null;
};
var findOpenRouterUpstreamModel = (name) => {
...
function findModelConfig(provider, name) {
...
var availableProviderOptions = Object.keys(MODEL_MAP);
var API_ENDPOINTS = {
  openai: {
...
function getModelConfig(provider, name) {
...
function getApiEndpoint(agent) {
  ...
  if (customProviderUrl) {
    const url = customProviderUrl.trim().replace(/\/$/, "");
    if (/\/v\d+(beta\d*)?$/.test(url)) {
      return `${url}/chat/completions`;
    }
    return url;
  }
  const OAUTH_API_KEY_REFS = /* @__PURE__ */ new Set([
    "chatgpt",
    "xai",
    "antigravity",
    "claude",
  ...
  if (!effectiveProvider || effectiveProvider.toLowerCase() === "custom" || agent.apiSource === "custom") {
    if (agent.useServerProxy) {
      return "";
    }
    const apiKeyRef = agent.apiKeyRef;
  ...
  if (!key && effectiveModel) {
    try {
      key = getModelConfig(effectiveProvider, effectiveModel).endpointKey;
    } catch {
    }
  }
...
var DEFAULT_IMAGE_SIZE = "1K";
var DEFAULT_IMAGE_QUALITY = "medium";
var hasEstimateForSize = (tokenEstimates, size) => typeof tokenEstimates[size] !== "undefined";
var resolveTokenEstimate = (estimate, requestedQuality) => {
  if (typeof estimate === "number") return estimate;
  if (!estimate) return void 0;
  const quality = requestedQuality && typeof estimate[requestedQuality] === "number" ? requestedQuality : DEFAULT_IMAGE_QUALITY;
  return estimate[quality];
};
var getApproxPricePerImage = (model, requestedSize, requestedQuality) => {
  ...
  if (!tokenEstimates) return void 0;
  const resolvedSize = requestedSize && hasEstimateForSize(tokenEstimates, requestedSize) ? requestedSize : DEFAULT_IMAGE_SIZE;
  ...
  return model.imageTokenPricePerMillion * outputTokens / 1e6;
};
