...
Cron.prototype._next = function(prev) {
  ...
  if (!startAtInFutureWithInterval && nextRun !== this._states.once) {
    nextRun = nextRun.increment(this._states.pattern, this.options, hasPreviousRun);
  }
...
// packages/chat/dialog/schedule.ts
function computeNextScheduledAt(schedule, now = Date.now()) {
...
// packages/chat/dialog/actions/createAgentAutomationAction.ts
var createAgentAutomationAction = async (args, thunkApi) => {
  ...
  const id = dbKey.slice(`${"agent-automation" /* AGENT_AUTOMATION */}-${userId}-`.length);
  const nowIso = (/* @__PURE__ */ new Date()).toISOString();
  const title = asOptionalTrimmedString(args.title) ?? "Agent automation";
  const spaceId = args.spaceId || void 0;
  const automation = {
    id,
    dbKey,
    type: "agent-automation" /* AGENT_AUTOMATION */,
    title,
