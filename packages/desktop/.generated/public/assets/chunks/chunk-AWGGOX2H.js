...
// packages/create/editor/transforms/fromMarkdown/inline.ts
function getTagName(html) {
  if (!html) return null;
  const match = html.trimStart().match(/^<\/?([a-zA-Z0-9]+)/);
  return match ? match[1].toLowerCase() : null;
}
function getRawTextFromNodes(nodes) {
  if (!Array.isArray(nodes)) return "";
  return nodes.map((node2) => {
    if (!node2) return "";
    if (node2.type === "text") return node2.value || "";
    if (Array.isArray(node2.children))
      return getRawTextFromNodes(node2.children);
    if (node2.value && typeof node2.value === "string") return node2.value;
    return "";
  }).join("");
}
function mergeTextNodes(nodes) {
  ...
  for (const node2 of nodes) {
    if (merged.length > 0 && "text" in node2 && "text" in merged[merged.length - 1]) {
      const prev = merged[merged.length - 1];
      const cur = node2;
      if (prev.bold === cur.bold && prev.italic === cur.italic && prev.strikethrough === cur.strikethrough) {
        prev.text += cur.text;
        continue;
      }
    }
...
function processInlineNodes(mdastChildren) {
  ...
  try {
    for (const child of mdastChildren) {
      ...
      switch (child.type) {
        ...
        case "text": {
          const text4 = child.value || "";
          const mentionRegex = /@\[(\w+):([^\]|]+)\|([^\]]+)\]/g;
          if (!mentionRegex.test(text4)) {
            result.push({ text: text4 });
            break;
          }
          let lastIndex = 0;
          let match;
          mentionRegex.lastIndex = 0;
          while ((match = mentionRegex.exec(text4)) !== null) {
            if (match.index > lastIndex) {
              result.push({ text: text4.slice(lastIndex, match.index) });
            }
            result.push({
              type: "mention",
          ...
          if (lastIndex < text4.length) {
            result.push({ text: text4.slice(lastIndex) });
          }
          break;
        }
        default:
...
// packages/create/editor/transforms/fromMarkdown/table.ts
function createSlateCell(mdastCell, isHeader) {
...
function transformTable(node2) {
  if (node2.type !== "table" || !node2.children || node2.children.length === 0) {
    return null;
  }
  const maxColumns = node2.children.reduce(
    (max, row) => Math.max(max, row.children?.length || 0),
    0
  );
  if (maxColumns === 0) return null;
  const normalizedRows = node2.children.map((row) => {
    const cells = row.children || [];
    const diff = maxColumns - cells.length;
    if (diff > 0) {
      const emptyCells = Array.from({ length: diff }, () => ({
        type: "tableCell",
        children: []
      }));
  ...
  const slateRows = normalizedRows.map((row, rowIndex) => {
...
// packages/create/editor/transforms/fromMarkdown/index.ts
...
function getPreviewFlagFromMeta(node2) {
  const meta = asTrimmedString(node2?.meta);
  ...
  return hasPreview ? "true" : void 0;
}
function convertList(node2) {
  const start = typeof node2.start === "number" ? node2.start : 1;
  const items = getChildren(node2);
  return {
    type: "list",
    ordered: !!node2.ordered,
    start,
    children: items.map(
      (item, index2) => convertListItem(item, start + index2)
    )
  };
}
function convertListItem(item, value) {
...
function convertListItemChildren(item) {
  const children = getChildren(item);
  if (!children.length) return [createEmptyParagraph()];
  return convertBlockChildren(children);
}
function convertBlockChildren(nodes) {
  const result = [];
  for (const node2 of nodes || []) {
    const converted = convertBlockNode(node2);
...
function convertBlockNode(node2) {
  ...
  switch (node2.type) {
    ...
    case "paragraph":
      return {
        type: "paragraph",
        children: node2.children?.length ? processInlineNodes(node2.children) : [{ text: "" }]
      };
    // 标题
    case "heading": {
    ...
    case "code":
      return {
        type: "code-block",
        language: node2.lang || "text",
        preview: getPreviewFlagFromMeta(node2),
        children: (node2.value || "").split("\n").map((line) => ({
          type: "code-line",
          children: [{ text: line }]
        }))
      };
    // 列表
    case "list":
      return convertList(node2);
    // 引用块（blockquote）
    case "blockquote": {
      const children = convertBlockChildren(getChildren(node2));
      return {
        type: "quote",
        children: children.length ? children : [createEmptyParagraph()]
      };
    ...
    // 表格
    case "table": {
      const tableNode = transformTable(node2);
    ...
    // 原始 HTML（块级）
    case "html": {
      const isBlockHtml = !node2.position || node2.position.start?.column === 1;
    ...
    case "root":
      return convertBlockChildren(getChildren(node2));
    default:
      return null;
  }
}
function mdastToSlate(mdastTree) {
  if (!mdastTree) {
...
// packages/create/editor/transforms/markdownToSlate.ts
