...
// packages/ai/skills/skillDocProtocol.ts
var SKILL_DOC_ENUMS = {
...
var SKILL_CONFIG_BLOCK = "skill-config";
var EVAL_CONFIG_BLOCK = "eval-config";
var WORKFLOW_CONFIG_BLOCK = "workflow-config";
...
var normalizeSkillEnumValue = (key, value) => typeof value === "string" && SKILL_DOC_ENUMS[key].includes(value) ? value : void 0;
var normalizeSkillBudgetTier = (value) => normalizeSkillEnumValue("budgetTier", value);
var normalizeSkillModalities = (value) => {
  ...
  return filtered.length > 0 ? Array.from(new Set(filtered)) : void 0;
};
var normalizeSkillConfig = (value, fallbackTools) => {
  if (!value || typeof value !== "object") return void 0;
  const record = value;
  const name = asOptionalTrimmedString(record.name) ?? "";
  const description = asOptionalTrimmedString(record.description) ?? "";
  if (!name || !description) return void 0;
  ...
  const toolNames = normalizeStringArray(record.toolNames) ?? normalizeStringArray(fallbackTools) ?? void 0;
  const discover = record.discover && typeof record.discover === "object" ? {
    keywords: normalizeStringArray(
      record.discover.keywords
    ),
    examples: normalizeStringArray(
      record.discover.examples
    )
  } : void 0;
  return {
    version: "0.1",
    ...
    discover: discover?.keywords || discover?.examples ? discover : void 0
  };
};
var normalizeEvalConfig = (value) => {
  if (!value || typeof value !== "object") return void 0;
  const record = value;
  if (!Array.isArray(record.cases)) return void 0;
  const cases = [];
...
var normalizePageSkillMetadata = (value, fallbackTools) => {
  if (!value || typeof value !== "object") return void 0;
  const record = value;
  ...
  return meta.kind || meta.requiredSkills || meta.recommendedSkills || meta.skillConfig || meta.evalConfig || meta.workflowConfig ? meta : void 0;
};
var extractCommentBlock = (markdown, blockName) => {
...
var removeCommentBlock = (markdown, blockName) => markdown.replace(
  new RegExp(`\\n?<!--\\s*${blockName}\\s*\\n[\\s\\S]*?-->\\s*`, "gi"),
  "\n\n"
);
var parseYamlObject = (raw) => {
  if (!raw) return void 0;
  try {
...
var parseSkillDocProtocol = (markdown, existingMeta, fallbackTools) => {
  ...
  const parsedMeta = normalizePageSkillMetadata(
    {
      ...existingMeta ?? {},
      ...skillBlock ? {
        kind: existingMeta?.kind === "skill" || skillBlock.kind === "skill" ? "skill" : existingMeta?.kind,
        requiredSkills: skillBlock.requiredSkills ?? existingMeta?.requiredSkills,
        recommendedSkills: skillBlock.recommendedSkills ?? existingMeta?.recommendedSkills,
...
var resolvePageSkillMetadata = (page) => {
...
var yamlBlock = (value) => dump(value, { lineWidth: 120, noRefs: true }).trim();
var buildSkillConfigComment = (config) => {
  const payload = {
    ...
    ...config.requiredSkills?.length ? { requiredSkills: config.requiredSkills } : {},
    ...config.recommendedSkills?.length ? { recommendedSkills: config.recommendedSkills } : {},
    ...config.toolNames?.length ? { toolNames: config.toolNames } : {},
    ...config.preferredAgents?.length ? { preferredAgents: config.preferredAgents } : {},
    ...config.budgetTier ? { budgetTier: config.budgetTier } : {},
    ...typeof config.dispatchPreferred === "boolean" ? { dispatchPreferred: config.dispatchPreferred } : {},
    ...config.modalities?.length ? { modalities: config.modalities } : {},
    ...config.discover?.keywords?.length || config.discover?.examples?.length ? {
      discover: {
        ...config.discover?.keywords?.length ? { keywords: config.discover.keywords } : {},
        ...config.discover?.examples?.length ? { examples: config.discover.examples } : {}
      }
    } : {},
...
var buildEvalConfigComment = (config) => `<!-- ${EVAL_CONFIG_BLOCK}
${yamlBlock({
  version: config.version,
  cases: config.cases
})}
-->`;
var buildWorkflowConfigComment = (config) => `<!-- ${WORKFLOW_CONFIG_BLOCK}
${yamlBlock({
  version: config.version,
  kind: config.kind,
  ...config.id ? { id: config.id } : {},
...
-->`;
var buildSkillDocMarkdown = (options) => {
  const sections = [
...
var normalizeAllowedTools = (value) => {
...
var parseExternalSkillMarkdown = (markdown) => {
  ...
  return {
    ...
    metadata: frontmatter?.metadata && typeof frontmatter.metadata === "object" ? Object.fromEntries(
      Object.entries(frontmatter.metadata).filter(([, value]) => typeof value === "string").map(([key, value]) => [key, String(value)])
    ) : void 0,
    body
  };
};
