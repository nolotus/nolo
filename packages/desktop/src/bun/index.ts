...
/** Point local agent searchFiles at packaged ripgrep (pre-build ensures vendor binary). */
...
if (process.env.NOLO_DESKTOP_SERVER_CHILD === "1") {
  const { bootstrapServer } = await import("../../../../packages/server/entry.ts");
...
if (process.platform === "win32" && !isDev) {
  ...
  const { bootstrapServer, shutdownServer } = await import("../../../../packages/server/entry.ts");
  await bootstrapServer();
