 ...
 * @returns Promise<boolean> 请求是否成功 (response.ok)
 */
export type RNOverrideUploadHandler = (
  server: string,
  uploadConfig: {
    file: File;
    metadata: any;
...
let rnUploadAdapter: RNOverrideUploadHandler | null = null;

export const registerRNUploadAdapter = (adapter: RNOverrideUploadHandler) => {
...
export const noloUploadRequest = async (
  ...
  try {
    ...
    if (isReactNative && isRNFile(file)) {
      ...
      return await rnUploadAdapter(server, uploadConfig, state, signal);
    }
