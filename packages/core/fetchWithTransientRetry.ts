/**
 * Shared transient-error retry wrapper for client fetches.
 *
 * Sunk from packages/cli/client/localRuntimeFetchRetry.ts so every client
 * surface (CLI dialog commands, packages/ai workspace read tools, TUI proxy,
 * web, background runs) inherits the same drain-window protection from ONE
 * implementation instead of each call site hand-rolling retry or maintaining
 * a consumer list in comments.
 *
 * Deliberately free of node builtins: this module is safe to import from any
 * embedding of core (browser/RN included). The Node-only loopback bypass
 * (`defaultLoopbackRequest`) stays in
 * packages/cli/client/localRuntimeFetchRetry.ts, which re-exports everything
 * here so existing CLI import paths keep working unchanged.
 */
...
import { isLoopbackHostname } from "./localOrigins";
import {
...
/**
 * Injectable fetch impl, structurally identical to packages/cli/cliFetch.ts's
 * `CliFetchImpl` (Bun's `fetch` type requires `preconnect`, which plain test
 * stubs and `mock()` functions never provide; runtime code only calls fetch as
 * a function). Declared here so non-CLI packages (e.g. packages/ai) can use the
 * shared retry without importing the CLI package.
 */
export type SharedFetchImpl = (
  input: FetchInput,
  init?: FetchInit,
) => Promise<Response>;
 ...
 * (Private on purpose: core/localOrigins.isLoopbackUrl is the string-only
 * pure seam; the retry bypass needs the Request unwrap.)
 */
function isRetryLoopbackInput(input: FetchInput) {
  try {
    const target =
...
export async function fetchWithTransientRetry(
  ...
  for (let attempt = 1; attempt <= loopMaxAttempts; attempt += 1) {
    try {
      if (options.loopbackRequest && isRetryLoopbackInput(input)) {
