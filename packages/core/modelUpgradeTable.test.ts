...
test("GLM 5.2 升级为 GLM 5.3：nolo/glm-5.2 与 zai/glm-5.2 命中升级到 nolo/glm-5.3", () => {
  expect(lookupModelUpgrade("nolo", "glm-5.2")?.to).toEqual({
    provider: "nolo",
    model: "glm-5.3",
  });
  expect(lookupModelUpgrade("zai", "glm-5.2")?.to).toEqual({
    provider: "nolo",
    model: "glm-5.3",
  });
});
