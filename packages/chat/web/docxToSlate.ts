...
interface ParagraphParseContext {
  ns: string;
  relsMap: RelsMap;
  /** relId → base64 data URI（从 zip 提取的实际图片数据，uploadImage 不可用时使用） */
  imageDataMap: Record<string, string>;
  /** relId → 服务端 fileId（通过 uploadImage 上传后获取，优先级高于 imageDataMap） */
  imageFileIdMap: Record<string, string>;
...
function handleImageInRun(run: Element, ctx: ParagraphParseContext): boolean {
  ...
  // 优先使用服务端 fileId（通过 uploadImage 上传），其次 base64 data URI，最后相对路径
...
function parseBodyToSlateNodes(
  ...
  for (const child of bodyChildren) {
    ...
    if (localName === "p") {
      ...
      const ctx: ParagraphParseContext = { ns, relsMap, imageDataMap, imageFileIdMap, slateNodes };
 ...
 * - 使用 PizZip 解压 DOCX
 */
export const convertDocxToSlate = async (
  file: File,
  /** 可选：上传图片 Blob 并返回服务端 fileId。不传时回退为 base64 内嵌。 */
  ...
  try {
    ...
    // 4b. 提取图片 base64（从 zip 中的 word/media/ 目录）
    ...
    const imageFileIdMap: Record<string, string> = {};
    for (const [relId, relPath] of Object.entries(relsMap)) {
      // relPath 如 "media/image1.png"（相对 word/），或 "../media/..."
      const normalized = relPath.startsWith("../")
        ? relPath.slice(3)
        : `word/${relPath}`;
      ...
      try {
        if (uploadImage) {
          ...
          // 内嵌路径：base64 data URI（回退方案）
          const b64 = (zipFile as any).asBase64();
