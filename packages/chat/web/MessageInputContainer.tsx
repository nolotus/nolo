...
import AddAgentDialog from "chat/dialog/AddAgentDialog";
...
>(({ runtimeOptions, agentPicker }, ref) => {
  ...
  const [isSwitchModelDialogOpen, setIsSwitchModelDialogOpen] = useState(false);
  ...
  const handleOpenSwitchModelDialog = useCallback(() => {
    setIsSwitchModelDialogOpen(true);
  }, []);

  const handleCloseSwitchModelDialog = useCallback(() => {
    setIsSwitchModelDialogOpen(false);
  }, []);

  const handleSelectPrimaryAgent = useCallback(
    async (nextAgentId: string | string[]) => {
      const targetAgentId = Array.isArray(nextAgentId)
        ? nextAgentId[0]
        : nextAgentId;
  ...
  if (isProBlocked) {
    return (
      <>
        ...
        />
        {isSwitchModelDialogOpen && switchModelQueryUserId && activeAgentId && (
          <AddAgentDialog
            ...
            preferredCapabilities={currentModelCapabilities ?? undefined}
    ...
    return (
      <>
        <ErrorMessage
          ...
          onRecharge={() => navigate("/recharge")}
          showChooseModel={
            sendPermission.reason === "NO_MODEL_PRICING" && !!activeAgentId
          }
          onChooseModel={handleOpenSwitchModelDialog}
        />
        {isSwitchModelDialogOpen && switchModelQueryUserId && activeAgentId && (
