export default {
  en: {
    translation: {
      AddAgent: "Add Agent",
      selectedAgentsCount: "Selected {{count}} Agents",
      addSelectedAgents: "Add selected",
      clearSelection: "Clear",
      recommended: "Recommended",
      favoriteAgent: "Favorite",
      myAgent: "Mine",
      capabilityMatch: "Capability match",
      confirm: "Confirm",
      cancel: "Cancel",
      clickHere: "Click here",
      clearConversation: "Clear Conversation",
      compressionError: "Image compression failed. Sending original images.",
      compressingImages: "Compressing images...",
      copyContent: "Copy Content",
      copyDiagnostics: "Copy diagnostics",
      finishReasonLengthNotice: "Response was cut off at the model's output limit",
      retryProgress: "Auto-retrying {{attempt}}/{{maxAttempts}} · {{seconds}}s",
      sendErrorCard: {
        title: "Send Failed",
        retry: "Retry",
        viewDetails: "Technical details",
        hideDetails: "Hide details",
        suggestion: "Suggestion",
        verifyAccount: "Verify account",
      },
      copyFailed: "Copy failed",
      contextAutoCompressed: "Context auto-compressed to stay within the model window",
      contextAutoCompressFailed: "Auto context compression failed",
      copySuccess: "Copied successfully",
      createDialog: "Create Dialog",
      createDialogFromAI: "Create Dialog with AI",
      createRobot: "Create Robot",
      customizeYourAI: "Customize Your AI",
      AgentAddFailed: "Failed to add Agent",
      AgentAddedSuccess: "Agent added successfully",
      MultipleAgentsAddedSuccess: "{{count}} Agents added successfully",
      AgentRemoveFailed: "Failed to remove Agent",
      AgentRemovedSuccess: "Agent removed successfully",
      Agents: "Agents",
      delete: "Delete",
      deleteContentConfirmation:
        "Are you sure you want to delete this content? This action cannot be undone.",
      deleteContentTitle: 'Delete "{{title}}"',
      deleteMovedToTrash: '"{{title}}" moved to Recycle Bin',
      deleteFailed: "Deletion failed",
      deleteMessage: "Delete Message",
      deleteSuccess: "Deleted successfully",
      confirmDelete: "Click again to confirm deletion",
      dropToUpload: "Drop files here to upload",
      excelIsEmpty: "Excel file is empty",
      excelIsEmptyOrInvalid: "Excel file is empty or invalid",
      excelParseError: "Failed to parse Excel file",
      FailedToLoadAgents: "Failed to load Agents",
      fileReadError: "Failed to read file content",
      First: "First",
      Info: "Info",
      LoadingAgents: "Loading Agents...",
      messageInput: "Message input field",
      messageInputArea: "Message input area with file upload",
      messageOrFileHere: "Type a message or upload a file...",
      pastedTextChip: "Pasted text #{{id}} · {{lines}} lines · {{size}}",
      removePastedText: "Remove pasted text",
      expandPastedText: "Expand pasted text into the input",
      Mode: "Mode",
      ModeUpdateFailed: "Failed to update mode",
      ModeUpdatedSuccess: "Mode updated successfully",
      newchat: "New Chat",
      NoAgents: "No Agents available yet",
      NoDescription: "No description provided",
      Orchestrated: "Orchestrated",
      Parallel: "Parallel",
      playAudio: "Play Audio",
      resend: "Resend",
      Retry: "Retry",
      saveContent: "Save Content",
      saveFailed: "Save failed",
      saveSuccess: "Saved successfully",
      SelectAgentToAdd: "Select an Agent to add to the dialog",
      sendFail: "Failed to send message",
      Sequential: "Sequential",
      ShowDialogConfigInfo: "View dialog settings and info",
      Tokens: "Tokens",
      Unnamed: "Unnamed",
      uploadFile: "Upload File",
      useServerProxy: "Use Transit Service",
      viewDetails: "for more details",
      // 新增文件上传相关翻译
      fileReadErrorMessage: "Failed to read file, please try again.",
      excelEmptyMessage: "Excel file is empty or invalid.",
      excelNoDataMessage: "No data found in Excel file.",
      excelParseErrorMessage:
        "Failed to parse Excel file, please check the format.",
      excelUploadSuccess: "Excel file uploaded and converted successfully!",
      docxUploadSuccess: "DOCX file uploaded and converted successfully!",
      docxUploadError: "Failed to upload DOCX file, please check the format.",
      pdfUploadSuccess: "PDF file uploaded and converted successfully!",
      pdfUploadError: "Failed to upload PDF file, please check the format.",
      compressingImagesMessage: "Compressing images, please wait...",
      compressionErrorMessage:
        "Image compression failed, sending original images.",
      sendFailMessage: "Failed to send message, please try again.",
      // 新增余额不足相关翻译
      insufficientBalanceDetailed:
        "Insufficient balance. Model: {{modelName}}, max cost per message: {{pricePerMessage}} credits. Balance: {{balance}} credits.",
      insufficientBalance: "Insufficient balance",
      insufficientBalanceShortfall: "You need {{shortfall}} more credits.",
      agentConfigMissing: "Agent configuration missing",
      agentConfigLoadFailed:
        "Failed to load agent configuration. Please try again.",
      modelPricingMissing: "Model pricing info missing",
      chooseAnotherModelContinue: "Choose another model to continue",
      chooseAnotherModelDialogTitle: "Choose another model",
      useThisModelContinue: "Continue with this model",
      switchModelContinueSuccess: "Switched model and continued this conversation",
      contextReprocessingNotice:
        "The system may need to reread part of this conversation after switching models or editing an earlier message.",
      switchModelContinueFailed: "Failed to switch model. Please try again.",
      noAlternativeModels: "No other available models",
      noAvailableAgentMessage: "No available Agent message",
      creditsUnit: "credits",
      dialogUsageTitle: "Session usage",
      usageInputLabel: "input",
      usageOutputLabel: "output",
      contextWindow: "Context window",
      dialogUsageTokensSection: "Session tokens",
      dialogUsageBillingSection: "Billing",
      recharge: "Click to recharge", // 新增单独的充值翻译
      // 新增中止功能相关翻译
      stopAllGeneration: "Stop All Generations",
      allMessagesAborted: "All messages have been aborted",
      // 新增发送按钮翻译
      send: "Send",
      // Guide / queue / loop related
      guideFromMessage: "Quote message",
      guideSeeded: "Inserted into input",
      guideEmptyContent: "No quotable text in this message",
      editMessage: "Edit message",
      editSeeded: "Editing message",
      editEmptyContent: "No editable text in this message",
      editingMessageNotice:
        "Editing a previous message. Sending will discard all later messages. The system may need to reread part of this conversation.",
      cancelEditingMessage: "Cancel editing message",
      branchMessage: "Branch",
      branchCreated: "Branch chat created",
      branchFailed: "Failed to create branch chat",
      stopBeforeDelete: "Stop generation before deleting",
      messageQueued: "Message queued; will be sent after the current turn",
      cannotSendFileDuringLoop: "Agent is running. Please wait before sending attachments.",
      assistantReplyStarting: "Working…",
      queueBadgeLabel: "Queued messages",
      queueBadgeQueued: "queued",
      queueBadgeTitle: "Queued messages",
      queueBadgeClear: "Clear",
      queueBadgeMore: "{{n}} more",
      composerPlaceholderQueuing: "Replying… your input will be queued",
      composerPlaceholderDrainPaused: "Waiting for confirmation…",
      composerPlaceholderError: "Send failed — retry after the turn",
      MaxContext: "Max Context",
      Compression: "Context Compression",
      CompressionTimes: "{{count}} times",
      inheritedContextNotice: "This chat inherits context from a previous conversation",
      inheritedContextNoticeWithTitle:
        'This chat inherits context from "{{title}}"',
      openSourceDialog: "Open source chat",
      loadError: "Failed to load; some messages may be incomplete",
      selectADialog: "Select a dialog",
      privateDialogGuestTitle: "This is a private dialog link",
      privateDialogGuestHint:
        "The /dialog-… route is only available to signed-in users who have permission to this dialog. If you want to share it with someone else, use a /share/<token> link instead.",
      thinkingProcess: "Thinking Process",
      thinkingActive: "Thinking…",
      thinkingDone: "Thought for {{seconds}}s",
      thinkingContent: "Thinking Content",
      answerContent: "Answer",
      // Mobile Upload
      imagePickerError: "Image picker error: {{error}}",
      shareAttachmentWarning: "Attachments ignored for share command",
      uploading: "Uploading...",
      uploadSuccess: "Upload success",
      uploadFailed: "Upload failed",
      deleteMessageTitle: "Delete Message",
      delConfirm: "Delete this tool output?",
      delConfirmMessage: "Delete this message?",
      bash: {
        blockedTitle: "Dangerous Command Blocked",
        blockedDesc: "This command was detected as high-risk and was not executed. If necessary, please confirm the risk and continue.",
        runUnsafe: "Run anyway (unsafe)",
        exitCode: "exit: {{code}}",
        noOutput: "(No output)",
      },
      codeEdit: {
        viewFinal: "Final Result",
        viewDiff: "Diff Preview",
        conflictTitle: "Write Conflict: File Exists",
        conflictMsg: "The target file already exists and this call did not allow overwriting, so no changes were made.",
        overwrite: "Overwrite",
        confirmOverwritePrompt: "A previous attempt to write to {{path}} failed because it exists and overwrite was not permitted. Please confirm if you want to overwrite it with the latest content.",
      },
      tool: {
        failed: "Execution failed",
        executing: "Executing...",
        requiresApproval: "Requires Approval",
        resumePrompt: "Please continue with your previous plan based on the tool execution results; if the task is complete, summarize it concisely.",
        preflightRepairing: "Preflight found {{count}} issue(s), auto-repairing…",
        moreSteps: "{{count}} more steps…",
      },
      toolConfirm: {
        executing: "Executing…",
        patchApplied: "Patch applied",
        executed: "Executed {{name}}",
        retryApplyPatch: "Retry applying patch",
        retryExec: "Retry {{name}}",
        confirmDeleteSpaces: "Confirm deleting these Spaces",
        applyPatchDanger: "Apply this patch (dangerous)",
        confirmExec: "Run {{name}} (confirmation required)",
        successPatch: "✅ Successfully applied patch to {{path}}.",
        successExec: "✅ Successfully executed {{name}}.",
        failure: "❌ Execution failed: {{error}}",
        unknownError: "Unknown error",
        confirmDelete: "Confirm deleting {{entity}}",
        confirmExecGate: "Confirm running {{name}}",
        fallbackDialogs: "Matching dialogs",
        fallbackSpaces: "Matching spaces",
        failureDialogs: "Failed to delete dialogs",
        failureSpaces: "Failed to delete spaces",
        executedDialogs: "deleteDialogs executed",
        executedSpaces: "deleteSpaces executed",
        entityDialog: "dialog",
        entitySpace: "space",
        deleteLabelMultiple: "{{title}} and {{count}} more {{entity}}",
        fallbackMatched: "Matches",
      },
      toolNames: {
        tool: "Tool",
        exa_search: "Search",
        firecrawl_scrape: "Firecrawl scrape",
        firecrawl_search: "Firecrawl search",
        read_file: "Read file",
        readFile: "Read file",
        writeFile: "Write file",
        editFile: "Edit file",
        globFiles: "Find files",
        searchWorkspace: "Search workspace",
        readWorkspaceFile: "Read file",
        writeWorkspaceFile: "Write file",
        replaceWorkspaceText: "Replace text",
        startPreview: "Start preview",
        getPreviewStatus: "Preview status",
        stopPreview: "Stop preview",
        releasePreview: "Release preview",
        captureVisualState: "Screenshot",
        execShell: "Run command",
        listAgents: "List agents",
        readAgent: "Read agent",
        loadSkill: "Load skill",
        startAgentRun: "Start agent run",
      },
      toolVerbs: {
        readFile: "Read",
        readWorkspaceFile: "Read",
        writeFile: "Write",
        writeWorkspaceFile: "Write",
        editFile: "Edit",
        replaceWorkspaceText: "Edit",
        applyDiff: "Edit",
        globFiles: "Find files",
        searchWorkspace: "Find files",
        codeSearch: "Search",
        exa_search: "Web search",
        firecrawl_search: "Web search",
        fetchWebpage: "Fetch page",
        firecrawl_scrape: "Fetch page",
        execShell: "Run",
        loadSkill: "Load skill",
        listAgents: "List agents",
        readAgent: "Read agent",
        startAgentRun: "Start subtask",
        startPreview: "Start preview",
        getPreviewStatus: "Preview status",
        stopPreview: "Stop preview",
        releasePreview: "Release preview",
        captureVisualState: "Screenshot",
        appDeploy: "Deploy app",
        setTodoList: "Update plan",
        ask_user: "Ask",
        runStreamingAgent: "Handoff",
      },
      gemini: {
        title: "Image Generation",
        count: "{{count}} images generated",
      },
      page: {
        untitled: "Untitled Page",
      },
      table: {
        untitled: "Untitled Table",
        export: "Export",
        convertToDataTable: "Convert to data table",
        converting: "Converting...",
        convertSuccess: "Converted to data table",
        convertFailed: "Failed to convert data table",
        defaultColumnLabel: "Column {{index}}",
        convertedTitle: "Converted Table",
      },
      common: {
        open: "Open",
        retry: "Retry",
        run: "Run",
      },
      imgOpts: {
        defaultName: "Image {{index}}",
      },
      DialogSettings: "Dialog Settings",
      collaborativeConversation: "Collaborative conversation",
      childRunObserver: {
        panelAriaLabel: "Child runs",
        title: "Child runs",
        subtitleLoading: "Loading child runs…",
        subtitleCount: "{{count}} background child runs",
        refreshAriaLabel: "Refresh child run list",
        refreshTitle: "Refresh",
        expandAriaLabel: "Expand child runs",
        expandTitle: "Expand",
        collapseAriaLabel: "Collapse child runs",
        collapseTitle: "Collapse",
        loading: "Loading child runs…",
        loadFailed: "Failed to load",
        retry: "Retry",
        empty:
          "This dialog has no background child runs yet. They appear here after startAgentRun (async dispatch).",
        statusPending: "Pending",
        statusRunning: "Running",
        statusDone: "Done",
        statusFailed: "Failed",
        statusCancelled: "Cancelled",
        statusUnknown: "Unknown",
        defaultTitle: "Child run",
        missingDialogId: "Missing child dialog ID",
        detailStatus: "Status",
        detailAgent: "Agent",
        detailThread: "Thread",
        detailLoading: "Loading child dialog…",
        detailLoadFailed: "Unable to load child dialog",
        detailEmpty: "No messages or evidence yet.",
        messagesAriaLabel: "Child dialog messages",
        emptyContent: "(empty)",
        appendInstructionTitle: "Append instruction",
        appendInstructionPlaceholder: "Append instruction to running task…",
        appendInstructionButton: "Send",
        appendInstructionSending: "Sending…",
        appendInstructionSuccess: "Enqueued; will be consumed in the next turn",
        appendInstructionFailed: "Failed to send instruction",
        continueTaskTitle: "Continue task",
        continueTaskPlaceholder: "Enter instruction to continue task…",
        continueTaskButton: "Continue",
        continueTaskSuccess: "Continue instruction sent",
        queuedCount: "{{count}} queued",
        queuedCountTitle: "Number of queued instructions",
      },
      chat: {
        agentBehavior: {
          title: "Agent Automation Boundaries",
          selfUpdate: {
            title: "Fields exempted from updateSelf confirmation",
            greeting: "Greeting",
            greetingDesc: "Adjust the opener copy or quick menu.",
            introduction: "Introduction",
            introductionDesc: "Update the public introduction shown for this Agent.",
            tags: "Tags",
            tagsDesc: "Update tags and lightweight categorization info.",
            prompt: "System prompt",
            promptDesc: "High-impact field — recommended to keep asking by default.",
            references: "Knowledge references",
            referencesDesc: "Changes which docs and pages the Agent references by default.",
            tools: "Tools",
            toolsDesc: "Changes which tool capabilities the Agent can call.",
            model: "Model",
            modelDesc: "Switch the underlying model.",
            provider: "Provider",
            providerDesc: "Switch the model provider.",
            public: "Public visibility",
            publicDesc: "Toggle whether this Agent is shown publicly.",
            temperatureDesc: "Adjust generation randomness.",
            topPDesc: "Adjust nucleus sampling.",
            frequencyPenaltyDesc: "Adjust the repetition penalty.",
            presencePenaltyDesc: "Adjust the new-topic incentive.",
            maxTokensDesc: "Adjust the maximum tokens per reply.",
            reasoningEffort: "Reasoning effort",
            reasoningEffortDesc: "Adjust the reasoning effort level.",
            name: "Name",
            nameDesc: "Change the Agent's name.",
          },
          tone: {
            label: "Preferred tone",
            default: "Default",
            direct: "Direct",
            pragmatic: "Pragmatic",
            friendly: "Friendly",
            professional: "Professional",
          },
          knowledge: {
            label: "Knowledge capture",
            level1: "1: Don't create proactively",
            level2: "2: Ask me before creating",
            level3: "3: Suggest creation after answering",
            level4: "4: Auto-create on high-value results",
          },
          space: {
            label: "Current space reading",
            level1: "1: Don't auto-read",
            level2: "2: Structure and titles only",
            level3: "3: Light reading",
            level4: "4: Adaptive reading",
          },
        },
        scrollButtons: {
          title: "Quick scroll buttons",
          top: "Scroll to top",
          bottom: "Scroll to bottom",
        },
      },
    },
  },
  "zh-CN": {
    translation: {
      AddAgent: "添加智能体",
      selectedAgentsCount: "已选择 {{count}} 个智能体",
      addSelectedAgents: "添加选中",
      clearSelection: "清空",
      recommended: "推荐",
      favoriteAgent: "收藏",
      myAgent: "我的",
      capabilityMatch: "能力匹配",
      confirm: "确认",
      cancel: "取消",
      clickHere: "点击查看",
      clearConversation: "清除对话记录",
      compressionError: "图片压缩失败，将发送原始图片",
      compressingImages: "正在压缩图片...",
      copyContent: "复制内容",
      copyDiagnostics: "复制诊断信息",
      finishReasonLengthNotice: "回答因达到模型输出上限被截断",
      retryProgress: "自动重试 {{attempt}}/{{maxAttempts}} · {{seconds}}s",
      sendErrorCard: {
        title: "发送失败",
        retry: "重试",
        viewDetails: "查看详情",
        hideDetails: "收起详情",
        suggestion: "建议",
        verifyAccount: "验证账号",
      },
      copyFailed: "复制失败",
      contextAutoCompressed: "已自动压缩上下文，以保持在模型窗口内",
      contextAutoCompressFailed: "上下文自动压缩失败",
      copySuccess: "复制成功",
      createDialog: "创建对话",
      createDialogFromAI: "使用AI创建对话",
      createRobot: "创建机器人",
      customizeYourAI: "自定义AI助手",
      AgentAddFailed: "添加智能体失败",
      AgentAddedSuccess: "智能体添加成功",
      MultipleAgentsAddedSuccess: "已添加 {{count}} 个智能体",
      AgentRemoveFailed: "移除智能体失败",
      AgentRemovedSuccess: "智能体移除成功",
      Agents: "智能体",
      delete: "删除",
      deleteContentConfirmation: "您确定要删除此内容吗？此操作无法撤销。",
      deleteContentTitle: '删除 "{{title}}"',
      deleteMovedToTrash: '"{{title}}" 已移至回收站',
      deleteFailed: "删除失败",
      deleteMessage: "删除消息",
      deleteSuccess: "删除成功",
      confirmDelete: "再次点击确认删除",
      dropToUpload: "将文件拖放到此处上传",
      excelIsEmpty: "Excel 文件内容为空",
      excelIsEmptyOrInvalid: "Excel 文件为空或格式无效",
      excelParseError: "解析 Excel 文件失败",
      FailedToLoadAgents: "加载智能体失败",
      fileReadError: "读取文件内容失败",
      First: "首个",
      Info: "信息",
      LoadingAgents: "正在加载智能体...",
      messageInput: "消息输入框",
      messageInputArea: "消息输入区，支持文件上传",
      messageOrFileHere: "输入消息或上传文件...",
      pastedTextChip: "已粘贴文本 #{{id}} · {{lines}} 行 · {{size}}",
      removePastedText: "移除粘贴文本",
      expandPastedText: "展开粘贴文本到输入框",
      Mode: "模式",
      ModeUpdateFailed: "模式更新失败",
      ModeUpdatedSuccess: "模式更新成功",
      newchat: "新对话",
      NoAgents: "暂无可用智能体",
      NoDescription: "暂无描述信息",
      Orchestrated: "编排模式",
      Parallel: "并行模式",
      playAudio: "播放音频",
      resend: "重新发送",
      Retry: "重试",
      saveContent: "保存内容",
      saveFailed: "保存失败",
      saveSuccess: "保存成功",
      SelectAgentToAdd: "选择要添加到对话的智能体",
      sendFail: "消息发送失败",
      Sequential: "顺序模式",
      ShowDialogConfigInfo: "查看对话设置与信息",
      Tokens: "令牌数",
      Unnamed: "未命名",
      uploadFile: "上传文件",
      useServerProxy: "使用中转服务",
      viewDetails: "了解更多详情",
      // 新增文件上传相关翻译
      fileReadErrorMessage: "读取文件时出错，请重试。",
      excelEmptyMessage: "Excel 文件为空或格式无效。",
      excelNoDataMessage: "Excel 文件中没有数据。",
      excelParseErrorMessage: "解析 Excel 文件时出错，请检查文件格式。",
      excelUploadSuccess: "Excel 文件上传并转换成功！",
      docxUploadSuccess: "DOCX 文件上传并转换成功！",
      docxUploadError: "上传 DOCX 文件时出错，请检查文件格式。",
      pdfUploadSuccess: "PDF 文件上传并转换成功！",
      pdfUploadError: "上传 PDF 文件时出错，请检查文件格式。",
      compressingImagesMessage: "正在压缩图片，请稍候...",
      compressionErrorMessage: "图片压缩失败，将使用原始图片上传。",
      sendFailMessage: "发送消息失败，请重试。",
      // 新增余额不足相关翻译
      insufficientBalanceDetailed:
        "余额不足。当前模型：{{modelName}}，单次最多消耗：{{pricePerMessage}}积分。余额：{{balance}}积分。",
      insufficientBalance: "余额不足",
      insufficientBalanceShortfall: "还差 {{shortfall}} 积分。",
      agentConfigMissing: "智能体配置缺失",
      agentConfigLoadFailed: "智能体配置加载失败，请重试。",
      modelPricingMissing: "模型定价信息缺失",
      chooseAnotherModelContinue: "选择其他模型继续",
      chooseAnotherModelDialogTitle: "选择其他模型",
      useThisModelContinue: "用这个模型继续",
      switchModelContinueSuccess: "已切换模型，并继续当前对话",
      contextReprocessingNotice:
        "切换模型或编辑历史消息后，系统可能需要重新读取部分对话内容。",
      switchModelContinueFailed: "切换模型失败，请重试",
      noAlternativeModels: "没有其他可用模型",
      noAvailableAgentMessage: "没有可用的智能体消息",
      creditsUnit: "积分",
      dialogUsageTitle: "会话用量",
      usageInputLabel: "输入",
      usageOutputLabel: "输出",
      contextWindow: "上下文窗口",
      dialogUsageTokensSection: "本会话 token",
      dialogUsageBillingSection: "计费",
      recharge: "点此充值", // 新增单独的充值翻译
      // 新增中止功能相关翻译
      stopAllGeneration: "停止所有生成",
      allMessagesAborted: "所有消息已中止",
      // 新增发送按钮翻译
      send: "发送",
      // Guide / queue / loop 相关
      guideFromMessage: "引用消息",
      guideSeeded: "已填入输入框",
      guideEmptyContent: "消息没有可引用的文本",
      editMessage: "编辑消息",
      editSeeded: "已进入编辑模式",
      editEmptyContent: "消息没有可编辑的文本",
      editingMessageNotice:
        "正在编辑历史消息，发送后将丢弃其后的消息。系统可能需要重新读取部分对话内容。",
      cancelEditingMessage: "取消编辑消息",
      branchMessage: "分支",
      branchCreated: "已创建分支对话",
      branchFailed: "创建分支失败",
      stopBeforeDelete: "请等待生成停止后再删除",
      messageQueued: "消息已排队，将在当前轮次结束后发送",
      cannotSendFileDuringLoop: "Agent 运行中，含附件消息请等待完成后再发送",
      assistantReplyStarting: "正在处理…",
      queueBadgeLabel: "排队消息",
      queueBadgeQueued: "排队中",
      queueBadgeTitle: "排队消息",
      queueBadgeClear: "清空",
      queueBadgeMore: "还有 {{n}} 条",
      composerPlaceholderQueuing: "正在回复，输入将排队…",
      composerPlaceholderDrainPaused: "等待确认中…",
      composerPlaceholderError: "发送失败，请在回复结束后重试",
      MaxContext: "最大上下文",
      Compression: "上下文压缩",
      CompressionTimes: "{{count}} 次",
      inheritedContextNotice: "此对话继承自上一段对话的上下文",
      inheritedContextNoticeWithTitle: '此对话继承自“{{title}}”的上下文',
      openSourceDialog: "查看原对话",
      loadError: "加载出错，部分消息可能不完整",
      selectADialog: "请选择一个对话",
      privateDialogGuestTitle: "这是私有对话链接",
      privateDialogGuestHint:
        "当前 /dialog-… 路由只对登录且有权限的账号可见。若要把这段对话发给别人，请改用 /share/<token> 链接。",
      thinkingProcess: "思考过程",
      thinkingActive: "思考中…",
      thinkingDone: "思考了 {{seconds}}s",
      thinkingContent: "思考内容",
      answerContent: "回答",
      deleteMessageTitle: "删除消息",
      delConfirm: "删除此工具输出？",
      delConfirmMessage: "删除此消息？",
      bash: {
        blockedTitle: "危险命令已被拦截",
        blockedDesc: "该命令被检测为高危操作，默认未执行。如确有必要，请确认风险后继续。",
        runUnsafe: "仍要执行（unsafe）",
        exitCode: "退出码: {{code}}",
        noOutput: "(无输出)",
      },
      codeEdit: {
        viewFinal: "最终结果",
        viewDiff: "差异预览",
        conflictTitle: "写入冲突：文件已存在",
        conflictMsg: "目标文件已存在，且本次调用未允许覆盖，因此没有进行写入。",
        overwrite: "覆盖写入",
        confirmOverwritePrompt: "刚才尝试写入文件 {{path}} 时发现文件已存在，且未覆盖。请确认覆盖该文件，并使用你刚才生成的最新内容。",
      },
      tool: {
        failed: "执行失败",
        executing: "正在执行...",
        requiresApproval: "等待确认",
        resumePrompt: "请基于刚才工具执行的结果继续完成你之前的计划；如果任务已经完成，请用简洁的方式总结结果。",
        preflightRepairing: "预检发现 {{count}} 个问题，正在自动修复…",
        moreSteps: "还有 {{count}} 步…",
      },
      toolConfirm: {
        executing: "正在执行…",
        patchApplied: "补丁已应用",
        executed: "已执行 {{name}}",
        retryApplyPatch: "重试应用补丁",
        retryExec: "重试执行 {{name}}",
        confirmDeleteSpaces: "确认删除这些 Space",
        applyPatchDanger: "应用这个补丁（危险操作）",
        confirmExec: "执行 {{name}}（需要确认）",
        successPatch: "✅ 已成功将补丁应用到 {{path}}。",
        successExec: "✅ 已成功执行 {{name}}。",
        failure: "❌ 执行失败：{{error}}",
        unknownError: "未知错误",
        confirmDelete: "确认删除{{entity}}",
        confirmExecGate: "确认执行 {{name}}",
        fallbackDialogs: "匹配的对话",
        fallbackSpaces: "匹配的空间",
        failureDialogs: "删除对话执行失败",
        failureSpaces: "删除空间执行失败",
        // executedSummary 仅写入 toolMessage 内部记录，不直接展示给用户，故保留英文标识符（zh/zh-Hant/ja 同）
        executedDialogs: "deleteDialogs executed",
        executedSpaces: "deleteSpaces executed",
        entityDialog: "对话",
        entitySpace: "空间",
        deleteLabelMultiple: "{{title}} 等 {{count}} 个{{entity}}",
        fallbackMatched: "匹配项",
      },
      toolNames: {
        tool: "工具",
        exa_search: "搜索",
        firecrawl_scrape: "Firecrawl 抓取",
        firecrawl_search: "Firecrawl 搜索",
        read_file: "读取文件",
        readFile: "读取文件",
        writeFile: "写入文件",
        editFile: "修改文件",
        globFiles: "查找文件",
        searchWorkspace: "搜索工作区",
        readWorkspaceFile: "读取文件",
        writeWorkspaceFile: "写入文件",
        replaceWorkspaceText: "替换文本",
        startPreview: "启动预览",
        getPreviewStatus: "预览状态",
        stopPreview: "停止预览",
        releasePreview: "释放预览",
        captureVisualState: "截图检查",
        execShell: "运行命令",
        listAgents: "列出助手",
        readAgent: "读取助手",
        loadSkill: "加载技能",
        startAgentRun: "启动子任务",
      },
      toolVerbs: {
        readFile: "读取",
        readWorkspaceFile: "读取",
        writeFile: "写入",
        writeWorkspaceFile: "写入",
        editFile: "修改",
        replaceWorkspaceText: "修改",
        applyDiff: "修改",
        globFiles: "查找文件",
        searchWorkspace: "查找文件",
        codeSearch: "搜索",
        exa_search: "网页搜索",
        firecrawl_search: "网页搜索",
        fetchWebpage: "网页抓取",
        firecrawl_scrape: "网页抓取",
        execShell: "命令",
        loadSkill: "加载技能",
        listAgents: "列出助手",
        readAgent: "读取助手",
        startAgentRun: "启动子任务",
        startPreview: "启动预览",
        getPreviewStatus: "预览状态",
        stopPreview: "停止预览",
        releasePreview: "释放预览",
        captureVisualState: "截图检查",
        appDeploy: "部署应用",
        setTodoList: "更新计划",
        ask_user: "提问",
        runStreamingAgent: "转交",
      },
      gemini: {
        title: "图像生成",
        count: "生成了 {{count}} 张图片",
      },
      page: {
        untitled: "未命名页面",
      },
      table: {
        untitled: "未命名表格",
        export: "导出",
        convertToDataTable: "转为数据表",
        converting: "转换中...",
        convertSuccess: "已转换为数据表",
        convertFailed: "转换为数据表失败",
        defaultColumnLabel: "列 {{index}}",
        convertedTitle: "转换后的表格",
      },
      common: {
        open: "打开",
        retry: "重试",
        run: "运行",
      },
      imgOpts: {
        defaultName: "图片 {{index}}",
      },
      DialogSettings: "会话设置",
      collaborativeConversation: "协作会话",
      childRunObserver: {
        panelAriaLabel: "子任务运行",
        title: "子任务",
        subtitleLoading: "加载子运行…",
        subtitleCount: "{{count}} 个后台子运行",
        refreshAriaLabel: "刷新子任务列表",
        refreshTitle: "刷新",
        expandAriaLabel: "展开子任务",
        expandTitle: "展开",
        collapseAriaLabel: "收起子任务",
        collapseTitle: "收起",
        loading: "正在加载子任务…",
        loadFailed: "加载失败",
        retry: "重试",
        empty:
          "当前对话还没有后台子任务。通过 startAgentRun 异步派发启动后会出现在这里。",
        statusPending: "排队中",
        statusRunning: "运行中",
        statusDone: "已完成",
        statusFailed: "失败",
        statusCancelled: "已取消",
        statusUnknown: "未知",
        defaultTitle: "子任务",
        missingDialogId: "缺少子对话 ID",
        detailStatus: "状态",
        detailAgent: "智能体",
        detailThread: "线程",
        detailLoading: "加载子对话…",
        detailLoadFailed: "无法加载子对话",
        detailEmpty: "暂无消息或证据。",
        messagesAriaLabel: "子对话消息",
        emptyContent: "（空内容）",
        appendInstructionTitle: "追加指令",
        appendInstructionPlaceholder: "追加指令给运行中的任务…",
        appendInstructionButton: "追加",
        appendInstructionSending: "发送中…",
        appendInstructionSuccess: "已加入队列，将在下一轮消费",
        appendInstructionFailed: "发送指令失败",
        continueTaskTitle: "继续任务",
        continueTaskPlaceholder: "输入指令继续任务…",
        continueTaskButton: "继续任务",
        continueTaskSuccess: "已提交继续指令",
        queuedCount: "{{count}} 条排队中",
        queuedCountTitle: "当前排队中的指令数",
      },
      chat: {
        agentBehavior: {
          title: "Agent 自动化边界",
          selfUpdate: {
            title: "updateSelf 免询问字段",
            greeting: "欢迎语",
            greetingDesc: "调整开场文案或快捷菜单。",
            introduction: "简介",
            introductionDesc: "更新 Agent 对外展示的简介。",
            tags: "标签",
            tagsDesc: "更新标签和轻量分类信息。",
            prompt: "系统提示词",
            promptDesc: "高影响字段，默认建议保持询问。",
            references: "知识引用",
            referencesDesc: "会改变 Agent 默认参考的文档和页面。",
            tools: "工具",
            toolsDesc: "会改变 Agent 可调用的工具能力。",
            model: "模型",
            modelDesc: "更换底层模型。",
            provider: "提供商",
            providerDesc: "更换模型提供方。",
            public: "公开状态",
            publicDesc: "切换是否公开展示。",
            temperatureDesc: "调整生成随机性。",
            topPDesc: "调整 nucleus sampling。",
            frequencyPenaltyDesc: "调整重复惩罚。",
            presencePenaltyDesc: "调整新话题激励。",
            maxTokensDesc: "调整单次回复最大 token 数。",
            reasoningEffort: "推理强度",
            reasoningEffortDesc: "调整 reasoning effort。",
            name: "名称",
            nameDesc: "修改 Agent 名称。",
          },
          tone: {
            label: "偏好语气",
            default: "默认",
            direct: "直接",
            pragmatic: "务实",
            friendly: "友好",
            professional: "专业",
          },
          knowledge: {
            label: "知识沉淀",
            level1: "1: 不主动创建",
            level2: "2: 先问我再创建",
            level3: "3: 回答后建议创建",
            level4: "4: 高价值结果可自动创建",
          },
          space: {
            label: "当前空间读取",
            level1: "1: 不自动读取",
            level2: "2: 只看结构和标题",
            level3: "3: 轻量读取",
            level4: "4: 自适应读取",
          },
        },
        scrollButtons: {
          title: "快捷滚动按钮",
          top: "滚动到顶部",
          bottom: "滚动到底部",
        },
      },
    },
  },
  "zh-Hant": {
    translation: {
      AddAgent: "添加智能體",
      selectedAgentsCount: "已選擇 {{count}} 個智能體",
      addSelectedAgents: "添加選中",
      clearSelection: "清空",
      recommended: "推薦",
      favoriteAgent: "收藏",
      myAgent: "我的",
      capabilityMatch: "能力匹配",
      confirm: "確認",
      cancel: "取消",
      clickHere: "點擊查看",
      clearConversation: "清除對話記錄",
      compressionError: "圖片壓縮失敗，將發送原始圖片",
      compressingImages: "正在壓縮圖片...",
      copyContent: "複製內容",
      copyDiagnostics: "複製診斷資訊",
      finishReasonLengthNotice: "回答因達到模型輸出上限被截斷",
      retryProgress: "自動重試 {{attempt}}/{{maxAttempts}} · {{seconds}}s",
      sendErrorCard: {
        title: "發送失敗",
        retry: "重試",
        viewDetails: "查看詳情",
        hideDetails: "收起詳情",
        suggestion: "建議",
        verifyAccount: "驗證帳號",
      },
      copyFailed: "複製失敗",
      contextAutoCompressed: "已自動壓縮上下文，以保持在模型視窗內",
      contextAutoCompressFailed: "上下文自動壓縮失敗",
      copySuccess: "複製成功",
      createDialog: "創建對話",
      createDialogFromAI: "使用AI創建對話",
      createRobot: "創建機器人",
      customizeYourAI: "自定義AI助手",
      AgentAddFailed: "添加智能體失敗",
      AgentAddedSuccess: "智能體添加成功",
      MultipleAgentsAddedSuccess: "已添加 {{count}} 個智能體",
      AgentRemoveFailed: "移除智能體失敗",
      AgentRemovedSuccess: "智能體移除成功",
      Agents: "智能體",
      delete: "刪除",
      deleteContentConfirmation: "您確定要刪除此內容嗎？此操作無法撤銷。",
      deleteContentTitle: '刪除 "{{title}}"',
      deleteMovedToTrash: '"{{title}}" 已移至回收站',
      deleteFailed: "刪除失敗",
      deleteMessage: "刪除訊息",
      deleteSuccess: "刪除成功",
      confirmDelete: "再次點擊確認刪除",
      dropToUpload: "將檔案拖放到此處上傳",
      excelIsEmpty: "Excel 檔案內容為空",
      excelIsEmptyOrInvalid: "Excel 檔案為空或格式無效",
      excelParseError: "解析 Excel 檔案失敗",
      FailedToLoadAgents: "載入智能體失敗",
      fileReadError: "讀取檔案內容失敗",
      First: "首個",
      Info: "資訊",
      LoadingAgents: "正在載入智能體...",
      messageInput: "訊息輸入框",
      messageInputArea: "訊息輸入區，支持檔案上傳",
      messageOrFileHere: "輸入訊息或上傳檔案...",
      pastedTextChip: "已貼上文字 #{{id}} · {{lines}} 行 · {{size}}",
      removePastedText: "移除貼上文字",
      expandPastedText: "展開貼上文字到輸入框",
      Mode: "模式",
      ModeUpdateFailed: "模式更新失敗",
      ModeUpdatedSuccess: "模式更新成功",
      newchat: "新對話",
      NoAgents: "暫無可用智能體",
      NoDescription: "暫無描述資訊",
      Orchestrated: "編排模式",
      Parallel: "並行模式",
      playAudio: "播放音頻",
      resend: "重新發送",
      Retry: "重試",
      saveContent: "儲存內容",
      saveFailed: "儲存失敗",
      saveSuccess: "儲存成功",
      SelectAgentToAdd: "選擇要添加到對話的智能體",
      sendFail: "訊息發送失敗",
      Sequential: "順序模式",
      ShowDialogConfigInfo: "查看對話設定與資訊",
      Tokens: "令牌數",
      Unnamed: "未命名",
      uploadFile: "上傳檔案",
      useServerProxy: "使用中轉服務",
      viewDetails: "了解更多詳情",
      // 新增文件上传相关翻译
      fileReadErrorMessage: "讀取檔案時出錯，請重試。",
      excelEmptyMessage: "Excel 檔案為空或格式無效。",
      excelNoDataMessage: "Excel 檔案中沒有數據。",
      excelParseErrorMessage: "解析 Excel 檔案時出錯，請檢查檔案格式。",
      excelUploadSuccess: "Excel 檔案上傳並轉換成功！",
      docxUploadSuccess: "DOCX 檔案上傳並轉換成功！",
      docxUploadError: "上傳 DOCX 檔案時出錯，請檢查檔案格式。",
      pdfUploadSuccess: "PDF 檔案上傳並轉換成功！",
      pdfUploadError: "上傳 PDF 檔案時出錯，請檢查檔案格式。",
      compressingImagesMessage: "正在壓縮圖片，請稍候...",
      compressionErrorMessage: "圖片壓縮失敗，將使用原始圖片上傳。",
      sendFailMessage: "發送訊息失敗，請重試。",
      // 新增余额不足相关翻译
      insufficientBalanceDetailed:
        "餘額不足。當前模型：{{modelName}}，單次最多消耗：{{pricePerMessage}}積分。餘額：{{balance}}積分。",
      insufficientBalance: "餘額不足",
      insufficientBalanceShortfall: "還差 {{shortfall}} 積分。",
      agentConfigMissing: "智能體配置缺失",
      agentConfigLoadFailed: "智能體配置載入失敗，請再試一次。",
      modelPricingMissing: "模型定價信息缺失",
      chooseAnotherModelContinue: "選擇其他模型繼續",
      chooseAnotherModelDialogTitle: "選擇其他模型",
      useThisModelContinue: "用這個模型繼續",
      switchModelContinueSuccess: "已切換模型，並繼續當前對話",
      contextReprocessingNotice:
        "切換模型或編輯歷史訊息後，系統可能需要重新讀取部分對話內容。",
      editingMessageNotice:
        "正在編輯歷史訊息，發送後將丟棄其後的訊息。系統可能需要重新讀取部分對話內容。",
      switchModelContinueFailed: "切換模型失敗，請重試",
      noAlternativeModels: "沒有其他可用模型",
      noAvailableAgentMessage: "沒有可用的智能體消息",
      creditsUnit: "積分",
      dialogUsageTitle: "會話用量",
      usageInputLabel: "輸入",
      usageOutputLabel: "輸出",
      contextWindow: "上下文視窗",
      dialogUsageTokensSection: "本會話 token",
      dialogUsageBillingSection: "計費",
      recharge: "點此充值", // 新增单独的充值翻译
      // 新增中止功能相关翻译
      stopAllGeneration: "停止所有生成",
      allMessagesAborted: "所有訊息已中止",
      // 新增发送按钮翻译
      send: "發送",
      // Guide / queue / loop 相关
      guideFromMessage: "引用訊息",
      guideSeeded: "已填入輸入框",
      guideEmptyContent: "訊息沒有可引用的文本",
      editMessage: "編輯訊息",
      editSeeded: "已進入編輯模式",
      editEmptyContent: "訊息沒有可編輯的文字",
      cancelEditingMessage: "取消編輯訊息",
      branchMessage: "分支",
      branchCreated: "已建立分支對話",
      branchFailed: "建立分支對話失敗",
      stopBeforeDelete: "請等待生成停止後再刪除",
      messageQueued: "訊息已排隊，將在當前輪次結束後發送",
      cannotSendFileDuringLoop: "Agent 運行中，含附件訊息請等待完成後再發送",
      assistantReplyStarting: "正在處理…",
      queueBadgeLabel: "排隊訊息",
      queueBadgeQueued: "排隊中",
      queueBadgeTitle: "排隊訊息",
      queueBadgeClear: "清空",
      queueBadgeMore: "還有 {{n}} 條",
      composerPlaceholderQueuing: "正在回覆，輸入將排隊…",
      composerPlaceholderDrainPaused: "等待確認中…",
      composerPlaceholderError: "發送失敗，請在回覆結束後重試",
      MaxContext: "最大上下文",
      Compression: "上下文壓縮",
      CompressionTimes: "{{count}} 次",
      inheritedContextNotice: "此對話繼承自上一段對話的上下文",
      inheritedContextNoticeWithTitle: "此對話繼承自「{{title}}」的上下文",
      openSourceDialog: "查看原對話",
      loadError: "載入出錯，部分訊息可能不完整",
      selectADialog: "請選擇一個對話",
      privateDialogGuestTitle: "這是私有對話連結",
      privateDialogGuestHint:
        "目前 /dialog-… 路由只對已登入且有權限的帳號可見。若要把這段對話分享給別人，請改用 /share/<token> 連結。",
      thinkingProcess: "思考過程",
      thinkingActive: "思考中…",
      thinkingDone: "思考了 {{seconds}}s",
      thinkingContent: "思考內容",
      answerContent: "回答",
      deleteMessageTitle: "刪除訊息",
      delConfirm: "刪除此工具輸出？",
      delConfirmMessage: "刪除此訊息？",
      bash: {
        blockedTitle: "危險命令已被攔截",
        blockedDesc: "該命令被檢測為高危操作，預設未執行。如確有必要，請確認風險後繼續。",
        runUnsafe: "仍要執行（unsafe）",
        exitCode: "退出碼: {{code}}",
        noOutput: "(無輸出)",
      },
      codeEdit: {
        viewFinal: "最終結果",
        viewDiff: "差異預覽",
        conflictTitle: "寫入衝突：檔案已存在",
        conflictMsg: "目標檔案已存在，且本次調用未允許覆蓋，因此沒有進行寫入。",
        overwrite: "覆蓋寫入",
        confirmOverwritePrompt: "剛才嘗試寫入檔案 {{path}} 時發現檔案已存在，且未覆蓋。請確認覆蓋該檔案，並使用你剛才生成的最新內容。",
      },
      tool: {
        failed: "執行失敗",
        executing: "正在執行...",
        requiresApproval: "等待確認",
        resumePrompt: "請基於剛才工具執行的結果繼續完成你之前的計劃；如果任務已經完成，請用簡潔的方式總結結果。",
        preflightRepairing: "預檢發現 {{count}} 個問題，正在自動修復…",
        moreSteps: "還有 {{count}} 步…",
      },
      toolConfirm: {
        executing: "正在執行…",
        patchApplied: "修補已套用",
        executed: "已執行 {{name}}",
        retryApplyPatch: "重試套用修補",
        retryExec: "重試執行 {{name}}",
        confirmDeleteSpaces: "確認刪除這些 Space",
        applyPatchDanger: "套用這個修補（危險操作）",
        confirmExec: "執行 {{name}}（需要確認）",
        successPatch: "✅ 已成功將修補套用到 {{path}}。",
        successExec: "✅ 已成功執行 {{name}}。",
        failure: "❌ 執行失敗：{{error}}",
        unknownError: "未知錯誤",
        confirmDelete: "確認刪除{{entity}}",
        confirmExecGate: "確認執行 {{name}}",
        fallbackDialogs: "符合的對話",
        fallbackSpaces: "符合的空間",
        failureDialogs: "刪除對話執行失敗",
        failureSpaces: "刪除空間執行失敗",
        // executedSummary 仅写入 toolMessage 内部记录，不直接展示给用户，故保留英文标识符
        executedDialogs: "deleteDialogs executed",
        executedSpaces: "deleteSpaces executed",
        entityDialog: "對話",
        entitySpace: "空間",
        deleteLabelMultiple: "{{title}} 等 {{count}} 個{{entity}}",
        fallbackMatched: "符合項",
      },
      toolNames: {
        tool: "工具",
        exa_search: "搜尋",
        firecrawl_scrape: "Firecrawl 抓取",
        firecrawl_search: "Firecrawl 搜尋",
        read_file: "讀取檔案",
        readFile: "讀取檔案",
        writeFile: "寫入檔案",
        editFile: "修改檔案",
        globFiles: "查找檔案",
        searchWorkspace: "搜尋工作區",
        readWorkspaceFile: "讀取檔案",
        writeWorkspaceFile: "寫入檔案",
        replaceWorkspaceText: "替換文字",
        startPreview: "啟動預覽",
        getPreviewStatus: "預覽狀態",
        stopPreview: "停止預覽",
        releasePreview: "釋放預覽",
        captureVisualState: "截圖檢查",
        execShell: "執行命令",
        listAgents: "列出助手",
        readAgent: "讀取助手",
        loadSkill: "載入技能",
        startAgentRun: "啟動子任務",
      },
      toolVerbs: {
        readFile: "讀取",
        readWorkspaceFile: "讀取",
        writeFile: "寫入",
        writeWorkspaceFile: "寫入",
        editFile: "修改",
        replaceWorkspaceText: "修改",
        applyDiff: "修改",
        globFiles: "查找檔案",
        searchWorkspace: "查找檔案",
        codeSearch: "搜尋",
        exa_search: "網頁搜尋",
        firecrawl_search: "網頁搜尋",
        fetchWebpage: "網頁擷取",
        firecrawl_scrape: "網頁擷取",
        execShell: "命令",
        loadSkill: "載入技能",
        listAgents: "列出助手",
        readAgent: "讀取助手",
        startAgentRun: "啟動子任務",
        startPreview: "啟動預覽",
        getPreviewStatus: "預覽狀態",
        stopPreview: "停止預覽",
        releasePreview: "釋放預覽",
        captureVisualState: "截圖檢查",
        appDeploy: "部署應用",
        setTodoList: "更新計畫",
        ask_user: "提問",
        runStreamingAgent: "轉交",
      },
      gemini: {
        title: "圖像生成",
        count: "生成了 {{count}} 張圖片",
      },
      page: {
        untitled: "未命名頁面",
      },
      table: {
        untitled: "未命名表格",
        export: "匯出",
        convertToDataTable: "轉為資料表",
        converting: "轉換中...",
        convertSuccess: "已轉換為資料表",
        convertFailed: "轉換為資料表失敗",
        defaultColumnLabel: "欄 {{index}}",
        convertedTitle: "轉換後的表格",
      },
      common: {
        open: "打開",
        retry: "重試",
        run: "運行",
      },
      imgOpts: {
        defaultName: "圖片 {{index}}",
      },
      DialogSettings: "會話設定",
      collaborativeConversation: "協作會話",
      childRunObserver: {
        panelAriaLabel: "子任務運行",
        title: "子任務",
        subtitleLoading: "載入子運行…",
        subtitleCount: "{{count}} 個後台子運行",
        refreshAriaLabel: "重新整理子任務列表",
        refreshTitle: "重新整理",
        expandAriaLabel: "展開子任務",
        expandTitle: "展開",
        collapseAriaLabel: "收起子任務",
        collapseTitle: "收起",
        loading: "正在載入子任務…",
        loadFailed: "載入失敗",
        retry: "重試",
        empty:
          "目前對話還沒有後台子任務。透過 startAgentRun 非同步派發啟動後會出現在這裡。",
        statusPending: "排隊中",
        statusRunning: "運行中",
        statusDone: "已完成",
        statusFailed: "失敗",
        statusCancelled: "已取消",
        statusUnknown: "未知",
        defaultTitle: "子任務",
        missingDialogId: "缺少子對話 ID",
        detailStatus: "狀態",
        detailAgent: "智能體",
        detailThread: "執行緒",
        detailLoading: "載入子對話…",
        detailLoadFailed: "無法載入子對話",
        detailEmpty: "暫無訊息或證據。",
        messagesAriaLabel: "子對話訊息",
        emptyContent: "（空內容）",
        appendInstructionTitle: "追加指令",
        appendInstructionPlaceholder: "追加指令給運行中的任務…",
        appendInstructionButton: "追加",
        appendInstructionSending: "發送中…",
        appendInstructionSuccess: "已加入佇列，將在下一輪消費",
        appendInstructionFailed: "發送指令失敗",
        continueTaskTitle: "繼續任務",
        continueTaskPlaceholder: "輸入指令繼續任務…",
        continueTaskButton: "繼續任務",
        continueTaskSuccess: "已提交繼續指令",
        queuedCount: "{{count}} 條排隊中",
        queuedCountTitle: "目前排隊中的指令數",
      },
      chat: {
        agentBehavior: {
          title: "Agent 自動化邊界",
          selfUpdate: {
            title: "updateSelf 免詢問欄位",
            greeting: "歡迎語",
            greetingDesc: "調整開場文案或快捷選單。",
            introduction: "簡介",
            introductionDesc: "更新 Agent 對外展示的簡介。",
            tags: "標籤",
            tagsDesc: "更新標籤和輕量分類資訊。",
            prompt: "系統提示詞",
            promptDesc: "高影響欄位，預設建議保持詢問。",
            references: "知識引用",
            referencesDesc: "會改變 Agent 預設參考的文件和頁面。",
            tools: "工具",
            toolsDesc: "會改變 Agent 可呼叫的工具能力。",
            model: "模型",
            modelDesc: "更換底層模型。",
            provider: "提供商",
            providerDesc: "更換模型提供方。",
            public: "公開狀態",
            publicDesc: "切換是否公開展示。",
            temperatureDesc: "調整生成隨機性。",
            topPDesc: "調整 nucleus sampling。",
            frequencyPenaltyDesc: "調整重複懲罰。",
            presencePenaltyDesc: "調整新話題激勵。",
            maxTokensDesc: "調整單次回覆最大 token 數。",
            reasoningEffort: "推理強度",
            reasoningEffortDesc: "調整 reasoning effort。",
            name: "名稱",
            nameDesc: "修改 Agent 名稱。",
          },
          tone: {
            label: "偏好語氣",
            default: "預設",
            direct: "直接",
            pragmatic: "務實",
            friendly: "友善",
            professional: "專業",
          },
          knowledge: {
            label: "知識累積",
            level1: "1: 不主動建立",
            level2: "2: 先問我再建立",
            level3: "3: 回答後建議建立",
            level4: "4: 高價值結果可自動建立",
          },
          space: {
            label: "目前空間讀取",
            level1: "1: 不自動讀取",
            level2: "2: 只看結構和標題",
            level3: "3: 輕量讀取",
            level4: "4: 自適應讀取",
          },
        },
        scrollButtons: {
          title: "快捷捲動按鈕",
          top: "捲動到頂部",
          bottom: "捲動到底部",
        },
      },
    },
  },
  ja: {
    translation: {
      AddAgent: "エージェントを追加",
      selectedAgentsCount: "{{count}} 件のエージェントを選択中",
      addSelectedAgents: "選択項目を追加",
      clearSelection: "クリア",
      recommended: "おすすめ",
      favoriteAgent: "お気に入り",
      myAgent: "自分の",
      capabilityMatch: "機能一致",
      confirm: "確認",
      cancel: "キャンセル",
      clickHere: "こちらをクリック",
      clearConversation: "会話をクリア",
      compressionError: "画像圧縮に失敗しました。元の画像を送信します。",
      compressingImages: "画像を圧縮中...",
      copyContent: "コンテンツをコピー",
      copyDiagnostics: "診断情報をコピー",
      finishReasonLengthNotice: "モデルの出力上限に達したため、回答が途中で終了しました",
      retryProgress: "自動リトライ {{attempt}}/{{maxAttempts}} · {{seconds}}s",
      sendErrorCard: {
        title: "送信失敗",
        retry: "再試行",
        viewDetails: "詳細を表示",
        hideDetails: "詳細を折りたたむ",
        suggestion: "提案",
        verifyAccount: "アカウントを確認",
      },
      copyFailed: "コピーに失敗しました",
      contextAutoCompressed: "モデルウィンドウ内に収まるようコンテキストを自動圧縮しました",
      contextAutoCompressFailed: "コンテキストの自動圧縮に失敗しました",
      copySuccess: "コピーに成功しました",
      createDialog: "対話を作成",
      createDialogFromAI: "AIで対話を作成",
      createRobot: "ロボットを作成",
      customizeYourAI: "AIをカスタマイズ",
      AgentAddFailed: "エージェントの追加に失敗しました",
      AgentAddedSuccess: "エージェントを追加しました",
      MultipleAgentsAddedSuccess: "{{count}} 件のエージェントを追加しました",
      AgentRemoveFailed: "エージェントの削除に失敗しました",
      AgentRemovedSuccess: "エージェントを削除しました",
      Agents: "エージェント",
      delete: "削除",
      deleteContentConfirmation:
        "このコンテンツを削除しますか？この操作は元に戻せません。",
      deleteContentTitle: '「{{title}}」を削除',
      deleteMovedToTrash: '"{{title}}" をゴミ箱に移動しました',
      deleteFailed: "削除に失敗しました",
      deleteMessage: "メッセージを削除",
      deleteSuccess: "削除に成功しました",
      confirmDelete: "もう一度クリックして削除を確認",
      dropToUpload: "ファイルをここにドロップしてアップロード",
      excelIsEmpty: "Excelファイルが空です",
      excelIsEmptyOrInvalid: "Excelファイルが空か無効です",
      excelParseError: "Excelファイルの解析に失敗しました",
      FailedToLoadAgents: "エージェントの読み込みに失敗しました",
      fileReadError: "ファイルの読み込みに失敗しました",
      First: "最初",
      Info: "情報",
      LoadingAgents: "エージェントを読み込み中...",
      messageInput: "メッセージ入力欄",
      messageInputArea: "ファイルアップロード対応のメッセージ入力エリア",
      messageOrFileHere: "メッセージを入力またはファイルをアップロード...",
      pastedTextChip: "貼り付けテキスト #{{id}} · {{lines}} 行 · {{size}}",
      removePastedText: "貼り付けテキストを削除",
      expandPastedText: "貼り付けテキストを入力欄に展開",
      Mode: "モード",
      ModeUpdateFailed: "モードの更新に失敗しました",
      ModeUpdatedSuccess: "モードを更新しました",
      newchat: "新しいチャット",
      NoAgents: "エージェントはまだありません",
      NoDescription: "説明がありません",
      Orchestrated: "編成モード",
      Parallel: "並行モード",
      playAudio: "音声を再生",
      resend: "再送信",
      Retry: "再試行",
      saveContent: "コンテンツを保存",
      saveFailed: "保存に失敗しました",
      saveSuccess: "保存に成功しました",
      SelectAgentToAdd: "対話に追加するエージェントを選択",
      sendFail: "メッセージの送信に失敗しました",
      Sequential: "順次モード",
      ShowDialogConfigInfo: "対話の設定と情報を表示",
      Tokens: "トークン数",
      Unnamed: "名前なし",
      uploadFile: "ファイルをアップロード",
      useServerProxy: "中継サービスを使用",
      viewDetails: "詳細を確認",
      // 新增文件上传相关翻译
      fileReadErrorMessage:
        "ファイルの読み込みに失敗しました。もう一度お試しください。",
      excelEmptyMessage: "Excelファイルが空か無効です。",
      excelNoDataMessage: "Excelファイルにデータが見つかりません。",
      excelParseErrorMessage:
        "Excelファイルの解析に失敗しました。フォーマットを確認してください。",
      excelUploadSuccess: "Excelファイルのアップロードと変換が成功しました！",
      docxUploadSuccess: "DOCXファイルのアップロードと変換が成功しました！",
      docxUploadError:
        "DOCXファイルのアップロードに失敗しました。フォーマットを確認してください。",
      pdfUploadSuccess: "PDFファイルのアップロードと変換が成功しました！",
      pdfUploadError:
        "PDFファイルのアップロードに失敗しました。フォーマットを確認してください。",
      compressingImagesMessage: "画像を圧縮中です。お待ちください...",
      compressionErrorMessage: "画像圧縮に失敗しました。元の画像を送信します。",
      sendFailMessage:
        "メッセージの送信に失敗しました。もう一度お試しください。",
      // 新增余额不足相关翻译
      insufficientBalanceDetailed:
        "残高不足。モデル：{{modelName}}、1回の最大コスト：{{pricePerMessage}}ポイント。残高：{{balance}}ポイント。",
      insufficientBalance: "残高不足",
      insufficientBalanceShortfall: "あと {{shortfall}} ポイント不足しています。",
      agentConfigMissing: "エージェントの設定が見つかりません",
      agentConfigLoadFailed:
        "エージェント設定の読み込みに失敗しました。再試行してください。",
      modelPricingMissing: "モデルの価格情報が見つかりません",
      chooseAnotherModelContinue: "別のモデルを選んで続ける",
      chooseAnotherModelDialogTitle: "別のモデルを選択",
      useThisModelContinue: "このモデルで続ける",
      switchModelContinueSuccess: "モデルを切り替えて現在の会話を続けました",
      contextReprocessingNotice:
        "モデルの切り替えや過去のメッセージの編集後は、会話の一部を再読み込みする場合があります。",
      editingMessageNotice:
        "過去のメッセージを編集中です。送信すると、それ以降のメッセージは破棄されます。会話の一部を再読み込みする場合があります。",
      switchModelContinueFailed: "モデルの切り替えに失敗しました。再試行してください。",
      noAlternativeModels: "利用可能な他のモデルがありません",
      noAvailableAgentMessage: "利用可能なエージェントメッセージがありません",
      creditsUnit: "ポイント",
      dialogUsageTitle: "セッション使用量",
      usageInputLabel: "入力",
      usageOutputLabel: "出力",
      contextWindow: "コンテキストウィンドウ",
      dialogUsageTokensSection: "セッション token",
      dialogUsageBillingSection: "課金",
      recharge: "クリックしてチャージ", // 新增单独的充值翻译
      // 新增中止功能相关翻译
      stopAllGeneration: "すべての生成を停止",
      allMessagesAborted: "すべてのメッセージが中止されました",
      // 新增发送按钮翻译
      send: "送信",
      // Guide / queue / loop 関連新規訳
      guideFromMessage: "メッセージを引用",
      guideSeeded: "入力欄に挿入しました",
      guideEmptyContent: "このメッセージに引用できるテキストがありません",
      editMessage: "メッセージを編集",
      editSeeded: "編集モードに入りました",
      editEmptyContent: "このメッセージに編集できるテキストがありません",
      cancelEditingMessage: "メッセージの編集をキャンセル",
      branchMessage: "ブランチ",
      branchCreated: "ブランチ会話を作成しました",
      branchFailed: "ブランチ会話の作成に失敗しました",
      stopBeforeDelete: "生成が停止するまでお待ちください",
      messageQueued: "メッセージをキューに入れました。現在のターン後に送信されます",
      cannotSendFileDuringLoop: "Agent 実行中です。添付メッセージは完了後にお送りください",
      assistantReplyStarting: "処理中…",
      queueBadgeLabel: "キュー中メッセージ",
      queueBadgeQueued: "キュー中",
      queueBadgeTitle: "キュー中メッセージ",
      queueBadgeClear: "クリア",
      queueBadgeMore: "他 {{n}} 件",
      composerPlaceholderQueuing: "返信中…入力はキューに追加されます",
      composerPlaceholderDrainPaused: "確認待ち…",
      composerPlaceholderError: "送信失敗 — ターン終了後に再試行",
      MaxContext: "最大コンテキスト",
      Compression: "コンテキスト圧縮",
      CompressionTimes: "{{count}} 回",
      inheritedContextNotice: "この会話は前の会話のコンテキストを引き継いでいます",
      inheritedContextNoticeWithTitle: "この会話は「{{title}}」のコンテキストを引き継いでいます",
      openSourceDialog: "元のチャットを開く",
      loadError: "読み込みに失敗しました。一部のメッセージが不完全な可能性があります",
      selectADialog: "ダイアログを選択してください",
      privateDialogGuestTitle: "これは非公開ダイアログリンクです",
      privateDialogGuestHint:
        "/dialog-… ルートは、このダイアログに権限のあるログイン済みユーザーだけが開けます。他の人に共有したい場合は、/share/<token> リンクを使ってください。",
      thinkingProcess: "思考プロセス",
      thinkingActive: "考えています…",
      thinkingDone: "{{seconds}}秒考えた",
      thinkingContent: "思考内容",
      answerContent: "回答",
      // Mobile Upload
      imagePickerError: "画像選択エラー: {{error}}",
      shareAttachmentWarning: "共有コマンドでは添付ファイルは無視されます",
      uploading: "アップロード中...",
      uploadSuccess: "アップロード成功",
      uploadFailed: "アップロード失敗",
      deleteMessageTitle: "メッセージを削除",
      delConfirm: "この工具の出力を削除しますか？",
      delConfirmMessage: "このメッセージを削除しますか？",
      bash: {
        blockedTitle: "危険なコマンドをブロックしました",
        blockedDesc: "このコマンドは高リスクとして検出され、実行されませんでした。必要に応じてリスクを確認し、続行してください。",
        runUnsafe: "そのまま実行（アンセーフ）",
        exitCode: "終了コード: {{code}}",
        noOutput: "(出力なし)",
      },
      codeEdit: {
        viewFinal: "最終結果",
        viewDiff: "差分プレビュー",
        conflictTitle: "書き込み競合：ファイルが既に存在します",
        conflictMsg: "対象ファイルが既に存在し、上書きが許可されていないため、変更は行われませんでした。",
        overwrite: "上書き",
        confirmOverwritePrompt: "{{path}}への書き込み試行は、ファイルが存在し上書きが許可されていなかったため失敗しました。最新のコンテンツで上書きするか確認してください。",
      },
      tool: {
        failed: "実行失敗",
        executing: "実行中...",
        requiresApproval: "承認が必要",
        resumePrompt: "ツールの実行結果に基づいて、前の計画を続行してください。タスクが完了した場合は、簡潔にまとめてください。",
        preflightRepairing: "プリフライトで {{count}} 件の問題を検出、自動修復中…",
        moreSteps: "他 {{count}} ステップ…",
      },
      toolConfirm: {
        executing: "実行中…",
        patchApplied: "パッチを適用しました",
        executed: "{{name}} を実行しました",
        retryApplyPatch: "パッチ適用を再試行",
        retryExec: "{{name}} の実行を再試行",
        confirmDeleteSpaces: "これらの Space を削除しますか",
        applyPatchDanger: "このパッチを適用（危険な操作）",
        confirmExec: "{{name}} を実行（確認が必要）",
        successPatch: "✅ パッチを {{path}} に適用しました。",
        successExec: "✅ {{name}} を実行しました。",
        failure: "❌ 実行失敗：{{error}}",
        unknownError: "不明なエラー",
        confirmDelete: "{{entity}}の削除を確認",
        confirmExecGate: "{{name}} の実行を確認",
        fallbackDialogs: "一致する対話",
        fallbackSpaces: "一致するスペース",
        failureDialogs: "対話の削除に失敗しました",
        failureSpaces: "スペースの削除に失敗しました",
        // executedSummary 仅写入 toolMessage 内部记录，不直接展示给用户，故保留英文标识符
        executedDialogs: "deleteDialogs executed",
        executedSpaces: "deleteSpaces executed",
        entityDialog: "対話",
        entitySpace: "スペース",
        deleteLabelMultiple: "{{title}} など {{count}} 件の{{entity}}",
        fallbackMatched: "一致項目",
      },
      toolNames: {
        tool: "ツール",
        exa_search: "検索",
        firecrawl_scrape: "Firecrawl スクレイプ",
        firecrawl_search: "Firecrawl 検索",
        read_file: "ファイル読み取り",
        readFile: "ファイル読み取り",
        writeFile: "ファイル書き込み",
        editFile: "ファイル編集",
        globFiles: "ファイル検索",
        searchWorkspace: "ワークスペース検索",
        readWorkspaceFile: "ファイル読み取り",
        writeWorkspaceFile: "ファイル書き込み",
        replaceWorkspaceText: "テキスト置換",
        startPreview: "プレビュー開始",
        getPreviewStatus: "プレビュー状態",
        stopPreview: "プレビュー停止",
        releasePreview: "プレビュー解放",
        captureVisualState: "スクリーンショット確認",
        execShell: "コマンド実行",
        listAgents: "エージェント一覧",
        readAgent: "エージェント読み取り",
        loadSkill: "スキルを読み込み",
        startAgentRun: "エージェント実行",
      },
      toolVerbs: {
        readFile: "読み取り",
        readWorkspaceFile: "読み取り",
        writeFile: "書き込み",
        writeWorkspaceFile: "書き込み",
        editFile: "編集",
        replaceWorkspaceText: "編集",
        applyDiff: "編集",
        globFiles: "ファイル検索",
        searchWorkspace: "ファイル検索",
        codeSearch: "検索",
        exa_search: "Web検索",
        firecrawl_search: "Web検索",
        fetchWebpage: "ページ取得",
        firecrawl_scrape: "ページ取得",
        execShell: "実行",
        loadSkill: "スキル読み込み",
        listAgents: "エージェント一覧",
        readAgent: "エージェント読み取り",
        startAgentRun: "子タスク起動",
        startPreview: "プレビュー起動",
        getPreviewStatus: "プレビュー状態",
        stopPreview: "プレビュー停止",
        releasePreview: "プレビュー解放",
        captureVisualState: "スクリーンショット",
        appDeploy: "アプリデプロイ",
        setTodoList: "計画を更新",
        ask_user: "質問",
        runStreamingAgent: "引き継ぎ",
      },
      gemini: {
        title: "画像生成",
        count: "{{count}} 枚の画像を生成しました",
      },
      page: {
        untitled: "無題のページ",
      },
      table: {
        untitled: "無題のテーブル",
        export: "エクスポート",
        convertToDataTable: "データテーブルに変換",
        converting: "変換中...",
        convertSuccess: "データテーブルに変換しました",
        convertFailed: "データテーブルへの変換に失敗しました",
        defaultColumnLabel: "列 {{index}}",
        convertedTitle: "変換済みテーブル",
      },
      common: {
        open: "開く",
        retry: "再試行",
        run: "実行",
      },
      imgOpts: {
        defaultName: "画像 {{index}}",
      },
      DialogSettings: "対話設定",
      collaborativeConversation: "協調会話",
      childRunObserver: {
        panelAriaLabel: "子タスク実行",
        title: "子タスク",
        subtitleLoading: "子実行を読み込み中…",
        subtitleCount: "バックグラウンド子実行 {{count}} 件",
        refreshAriaLabel: "子タスク一覧を更新",
        refreshTitle: "更新",
        expandAriaLabel: "子タスクを展開",
        expandTitle: "展開",
        collapseAriaLabel: "子タスクを折りたたむ",
        collapseTitle: "折りたたむ",
        loading: "子タスクを読み込み中…",
        loadFailed: "読み込みに失敗しました",
        retry: "再試行",
        empty:
          "この会話にはまだバックグラウンド子タスクがありません。startAgentRun（非同期）で起動するとここに表示されます。",
        statusPending: "待機中",
        statusRunning: "実行中",
        statusDone: "完了",
        statusFailed: "失敗",
        statusCancelled: "キャンセル済み",
        statusUnknown: "不明",
        defaultTitle: "子タスク",
        missingDialogId: "子ダイアログ ID がありません",
        detailStatus: "状態",
        detailAgent: "エージェント",
        detailThread: "スレッド",
        detailLoading: "子ダイアログを読み込み中…",
        detailLoadFailed: "子ダイアログを読み込めません",
        detailEmpty: "メッセージまたは証拠はまだありません。",
        messagesAriaLabel: "子ダイアログのメッセージ",
        emptyContent: "（空）",
        appendInstructionTitle: "指示を追加",
        appendInstructionPlaceholder: "実行中のタスクに指示を追加…",
        appendInstructionButton: "追加",
        appendInstructionSending: "送信中…",
        appendInstructionSuccess: "キューに追加されました。次のターンで処理されます",
        appendInstructionFailed: "指示の送信に失敗しました",
        continueTaskTitle: "タスクを継続",
        continueTaskPlaceholder: "タスクを継続する指示を入力…",
        continueTaskButton: "タスクを継続",
        continueTaskSuccess: "継続指示を送信しました",
        queuedCount: "待機中 {{count}} 件",
        queuedCountTitle: "待機中の指示数",
      },
      chat: {
        agentBehavior: {
          title: "Agent 自動化の境界",
          selfUpdate: {
            title: "updateSelf の確認を省略できるフィールド",
            greeting: "挨拶文",
            greetingDesc: "冒頭の文言やショートカットメニューを調整します。",
            introduction: "紹介文",
            introductionDesc: "Agent が外部に表示する紹介文を更新します。",
            tags: "タグ",
            tagsDesc: "タグと軽量な分類情報を更新します。",
            prompt: "システムプロンプト",
            promptDesc: "影響の大きいフィールドのため、既定では確認を維持することを推奨します。",
            references: "知識参照",
            referencesDesc: "Agent が既定で参照するドキュメントやページを変更します。",
            tools: "ツール",
            toolsDesc: "Agent が呼び出せるツール能力を変更します。",
            model: "モデル",
            modelDesc: "基盤モデルを変更します。",
            provider: "プロバイダー",
            providerDesc: "モデルプロバイダーを変更します。",
            public: "公開状態",
            publicDesc: "公開表示するかどうかを切り替えます。",
            temperatureDesc: "生成のランダム性を調整します。",
            topPDesc: "nucleus sampling を調整します。",
            frequencyPenaltyDesc: "繰り返しペナルティを調整します。",
            presencePenaltyDesc: "新トピックの奨励を調整します。",
            maxTokensDesc: "1 回の返信の最大トークン数を調整します。",
            reasoningEffort: "推論強度",
            reasoningEffortDesc: "reasoning effort を調整します。",
            name: "名前",
            nameDesc: "Agent の名前を変更します。",
          },
          tone: {
            label: "好みのトーン",
            default: "デフォルト",
            direct: "率直",
            pragmatic: "実務的",
            friendly: "フレンドリー",
            professional: "プロフェッショナル",
          },
          knowledge: {
            label: "知識の蓄積",
            level1: "1: 主体的に作成しない",
            level2: "2: 作成前に確認",
            level3: "3: 回答後に作成を提案",
            level4: "4: 価値の高い結果は自動作成",
          },
          space: {
            label: "現在のスペースの読み取り",
            level1: "1: 自動読み取りしない",
            level2: "2: 構造とタイトルのみ",
            level3: "3: 軽量に読み取る",
            level4: "4: 適応的に読み取る",
          },
        },
        scrollButtons: {
          title: "クイックスクロールボタン",
          top: "トップへスクロール",
          bottom: "最下部へスクロール",
        },
      },
    },
  },
};
