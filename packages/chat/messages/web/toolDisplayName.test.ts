...
describe("tool display names", () => {
  it("uses translated tool labels when available", () => {
    ...
    expect(resolveToolDisplayName("globFiles", t)).toBe("查找文件");
  });
  ...
  it("uses Chinese defaults for common workspace tools when i18n misses keys", () => {
    ...
    expect(resolveToolDisplayName("codeSearch", passthrough)).toBe("代码搜索");
    expect(resolveToolDisplayName("globFiles", passthrough)).toBe("查找文件");
  ...
  it("never leaks raw API names when i18n returns the key path or API name", () => {
    ...
    expect(resolveToolDisplayName("codeSearch", broken)).toBe("代码搜索");
    expect(resolveToolDisplayName("globFiles", broken)).toBe("查找文件");
  ...
  it("normalizes functions. prefix and snake_case tool names", () => {
    expect(resolveToolDisplayName("functions.globFiles")).toBe("查找文件");
    expect(resolveToolDisplayName("glob_files")).toBe("查找文件");
    expect(resolveToolDisplayName("code_search")).toBe("代码搜索");
  });
...
describe("formatToolRowHeaderSummary", () => {
  ...
  it("builds query summary for codeSearch", () => {
    expect(
      formatToolRowHeaderSummary({
        toolName: "codeSearch",
    ...
    ).toBe("代码搜索 · groupConsecutiveToolEntries");
  });
...
describe("buildFallbackActivity", () => {
  ...
  it("returns fallback for codeSearch with query", () => {
    expect(buildFallbackActivity("codeSearch", { query: "TODO" })).toEqual({
      title: "在代码里找线索",
      detail: "TODO",
    });
  });
