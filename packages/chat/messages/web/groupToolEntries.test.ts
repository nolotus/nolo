...
describe("groupConsecutiveToolEntries", () => {
  it("recovers missing toolName from assistant tool_calls without overriding existing names", () => {
    ...
    const assistant = {
      ...
      tool_calls: [
        ...
        { id: "call-2", type: "function", function: { name: "codeSearch", arguments: "{}" } },
      ],
    };
  ...
  it("keeps a stable group key when a consecutive run grows", () => {
    ...
    const second = { id: "tool-2", role: "tool", toolName: "codeSearch" };
  ...
  it("preserves tool → narration → tool → final reply order", () => {
    ...
    const tool3 = { id: "tool-3", role: "tool", toolName: "codeSearch" };
    const finalAssistant = {
      id: "assistant-final",
      role: "assistant",
      content: "已完成，下面是结果。",
  ...
  it("stops coalescing at a user message", () => {
    ...
    const tool2 = { id: "tool-2", role: "tool", toolName: "codeSearch" };
