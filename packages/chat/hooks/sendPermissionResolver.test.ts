...
describe("resolveSendPermissionState", () => {
  ...
  it("allows send permission for legacy deepseek provider with balance", () => {
    const { getModelPricing } = require("ai/llm/getPricing");
    const serverPrices = getModelPricing("deepseek", "deepseek-v4-flash");
    expect(serverPrices).not.toBeNull();
    ...
    expect(result.sendPermission.allowed).toBe(true);
    expect(result.sendPermission.pricing?.modelName).toBe("deepseek-v4-flash");
    expect(result.sendPermission.pricing?.pricePerMessage).toBeGreaterThan(0);
  });
});
