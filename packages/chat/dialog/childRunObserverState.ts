...
} from "shared/contracts/appendInstruction";
...
/**
 * Route mode by child run status:
 * - "running" -> "enqueue" (running task: append to queue)
 * - "done" | "failed" -> "continue" (terminal task: continue task)
 * - "cancelled" / other / null -> null (no append entry)
 */
export function resolveAppendInstructionMode(
  status: string | null | undefined,
): AppendInstructionMode | null {
  const normalized = asOptionalTrimmedString(status)?.toLowerCase();
  ...
  // "cancelled", "orphaned", etc. -> no entry
  return null;
}

export function buildAppendInstructionControlUrl(
  serverOrigin: string | null | undefined,
): string {
  const origin = normalizeServerOrigin(serverOrigin);
  if (!origin) return "/api/agent/runs/control";
  return `${origin}/api/agent/runs/control`;
}

export function resolveAppendDialogKey(
  thread: Pick<ClientAgentThread, "dialogKey" | "dialogId" | "threadId">,
): string {
  return (
    asOptionalTrimmedString(thread.dialogKey) ??
...
export function buildAppendInstructionRequestBody(args: {
  dialogKey: string;
  userInput: string;
  mode: AppendInstructionMode;
  runtimeContext?: Record<string, unknown>;
}): AppendInstructionControlRequest {
  const dialogKey = asOptionalTrimmedString(args.dialogKey) ?? "";
  const userInput = asOptionalTrimmedString(args.userInput) ?? "";
  return {
    action: "append",
    dialogKey,
    userInput,
...
export function parseAppendInstructionError(
  payload: unknown,
  ...
  return fallbackStatus ? `HTTP ${fallbackStatus}` : "Failed to append instruction";
}

export function parseAppendInstructionResponse(
  payload: unknown,
  httpStatus: number,
):
  | { ok: true; queued?: number; status?: string; message?: string }
  | { ok: false; error: string } {
  if (httpStatus < 200 || httpStatus >= 300) {
    return {
      ok: false,
      error: parseAppendInstructionError(payload, httpStatus),
    };
  }
  if (!payload || typeof payload !== "object") {
    return { ok: false, error: "Invalid response" };
  ...
  const record = payload as {
    ok?: boolean;
    data?: { queued?: number; status?: string; message?: string };
    error?: unknown;
    queued?: number;
  };
  if (record.ok === false) {
  ...
  const queued =
    typeof record.data?.queued === "number"
      ? record.data.queued
  ...
  return {
    ...
    ...(record.data?.message ? { message: record.data.message } : {}),
  };
}
