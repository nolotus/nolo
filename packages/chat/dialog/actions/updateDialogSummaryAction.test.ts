...
async function loadUpdateDialogSummaryAction() {
  ...
  const planContextUsageMock = mock(() => ({
    historyBudget: 5,   // 极小预算确保短消息也触发压缩（P-1 后按 content 估算 token）
    rawMessageBudget: 0,
    minTailTokens: 0,
  }));
...
describe("updateDialogSummaryAction behavior", () => {
  it("uses the builtin summary model output and persists the new summary payload", async () => {
    ...
    const preFetchedMessages = [
      {
        id: "m1",
        role: "user",
        // P-1 修复后 token 从 content 估算；用 padding 让每条 ~15k token 触发压缩。
        // 断言用 toContain，padding 在后面不影响匹配。
        content: "先记录用户想把标题和摘要模型切到 DeepSeek。" + "x".repeat(60_000),
        usage: { completion_tokens: 100 },
      },
      {
        id: "m2",
        role: "assistant",
        content: "已经完成标题模型切换。" + "x".repeat(60_000),
        usage: { completion_tokens: 100 },
      },
      {
        id: "m3",
      ...
      },
      {
        id: "m5",
        role: "user",
        content: "还需要补长期可跑的行为测试。" + "x".repeat(60_000),
        usage: { completion_tokens: 100 },
      },
    ];
    ...
    // P0-3: 验证传入的 prompt 是三段式且含第一人称 handoff 要素
    expect(runLlmPayload.llmConfig.prompt).toContain("第一人称");
    expect(runLlmPayload.llmConfig.prompt).toContain("待验证");
    expect(runLlmPayload.llmConfig.prompt).toContain("文件操作清单");
  ...
  it("does not actively summarize completed tasks before context pressure", async () => {
    ...
    const preFetchedMessages = Array.from({ length: 12 }, (_, index) => ({
      ...
      // P-1 修复后从 content 估算；padding 到 ~1500 token/条。
      content: `第 ${index + 1} 条任务阶段消息` + "x".repeat(6000),
