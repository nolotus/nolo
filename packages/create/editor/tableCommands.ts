...
export const insertRow = (
  ...
  const [rowEntry] = Editor.nodes(editor, {
    match: (n) => isCustomElement(n) && n.type === "table-row",
  });
  ...
  if (rowEntry && tableEntry) {
    ...
    const [tableNode] = tableEntry as [Element, Path];
    const columns = (tableNode.children[0] as Element).children.length;
...
export const setColumnWidth = (
  ...
  Transforms.setNodes(editor, { columns: newColumns } as Partial<Element>, { at: tablePath });
};
...
const moveToAdjacentCell = (
  ...
  const [table, tablePath] = tableEntry as [Element, Path];
  const [row, rowPath] = rowEntry as [Element, Path];
