...
export const convertPdfToSlate = async (file: File): Promise<SlateNode[]> => {
  try {
        ...
        for (const item of items) {
          ...
          if (item.dest && typeof item.dest === "string") {
            ...
            if (
              ...
              pageNumber = (await pdf.getPageIndex(destRef[0] as any)) + 1; // 页面号从 1 开始
            }
          } else if (
            item.dest &&
            item.dest.length > 0 &&
            ...
            pageNumber = (await pdf.getPageIndex(item.dest[0] as any)) + 1;
          }
    ...
    for (let i = 1; i <= numPages; i++) {
      ...
      // 获取当前页面的超链接注解
      const pageLinks: LinkAnnotation[] = annotations.flatMap((annot) =>
        annot.subtype === "Link" && annot.url
          ? [
              {
                url: annot.url,
