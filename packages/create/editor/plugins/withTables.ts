...
import { isCustomElement, type Element, type TableElement } from "../types";
...
export const withTables = (editor: Editor) => {
  ...
  editor.normalizeNode = ([node, path]) => {
    ...
    if (isCustomElement(node) && node.type === "table") {
      const element = node as TableElement;
      let maxColumns = 0;
      ...
      if (columnMismatch && maxColumns > 0) {
        ...
        Transforms.setNodes(editor, { columns: newColumns } as Partial<Element>, { at: path });
