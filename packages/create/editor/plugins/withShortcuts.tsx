...
export const withShortcuts = (editor: Editor) => {
  ...
  const onKeyDown = (editor as any).onKeyDown;
  ...
  editor.insertText = (text: string) => {
    ...
    if (text.endsWith(" ") && selection && Range.isCollapsed(selection)) {
      ...
      // 当前实现是精确匹配，所以顺序不影响
      const shortcut = (SHORTCUTS as Record<string, string | { type: string; wrapper: string; ordered?: boolean; checked?: boolean }>)[beforeText];
        ...
        if (typeof shortcut === "string") {
          ...
          if (checked !== undefined) {
            ...
            if (checked) {
              Transforms.setNodes(
                editor,
                { checked: true } as Partial<Element>,
                {
                  match: (n) =>
                    isCustomElement(n) && n.type === "list-item",
                }
              );
            }
          } else if (ordered) {
  ...
  (editor as any).onKeyDown = (event: KeyboardEvent) => {
    for (const hotkey in HOTKEYS) {
      if (isHotkey(hotkey, event)) {
        ...
        const mark = (HOTKEYS as Record<string, string>)[hotkey];
