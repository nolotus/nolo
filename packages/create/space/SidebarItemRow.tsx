...
function SidebarItemRow({
  ...
  const dialogStatusSelector = useMemo(
    () => (dialogId ? selectDialogStatus(dialogId) : null),
    [dialogId]
  );
  const unreadDialogSelector = useMemo(
    () => (dialogId ? selectIsDialogUnread(dialogId) : null),
    [dialogId]
  );
  ...
  const sseDialogStatus = useAppSelector((state) =>
    dialogStatusSelector ? dialogStatusSelector(state) : undefined
  );
  const sseIsUnread = useAppSelector((state) =>
    unreadDialogSelector ? unreadDialogSelector(state) : false
  );
