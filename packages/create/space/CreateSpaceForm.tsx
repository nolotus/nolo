...
import { object, string, optional, picklist, forward, check, pipe, minLength } from "valibot";
...
export const CreateSpaceForm = ({ onClose }: { onClose: () => void }) => {
  ...
  const form = useForm<CreateSpaceRequest>({
    schema: pipe(
      object({
        name: string(),
        description: optional(string()),
        visibility: optional(picklist([SpaceVisibility.PRIVATE, SpaceVisibility.PUBLIC])),
        boundFolder: optional(string()),
      }),
      // name 跨字段校验：有 boundFolder 时 name 可为空，否则必填且至少 2 字符
      forward(
        check((data) => {
          const hasBound = (data.boundFolder || "").trim().length > 0;
          if (hasBound) return true;
          return (data.name || "").trim().length >= 2;
        }, t("space.nameRequired")),
        ["name"],
      ),
    ),
    defaultValues: {
  ...
  const boundFolder = (values.boundFolder || "") as string;
      ...
      >
          ...
          <div>
            ...
            {boundFolder ? (
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() => set("boundFolder", undefined)}
                  style={{
                    flexShrink: 0,
                    display: "flex",
                    alignItems: "center",
        ...
        <div>
          ...
          <Select
            id={visibilityId}
            selectedKey={
              values.visibility == null ? undefined : String(values.visibility)
            }
