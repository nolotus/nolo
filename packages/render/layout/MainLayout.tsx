...
import { selectViewMode, setViewMode } from "create/space/spaceSlice";
...
const MainLayout: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem("nolo-space-view-mode", viewMode);
    } catch {
      // ignore localStorage write failures
    }
  }, [viewMode]);
  // Restore viewMode from localStorage after mount to avoid hydration mismatch.
  // Initial state is always "all" (SSR-safe); this reads the persisted value
  // once on the client so user preference is respected.
  const hasRestoredViewMode = useRef(false);
  useEffect(() => {
    if (hasRestoredViewMode.current) return;
    hasRestoredViewMode.current = true;
    if (typeof window === "undefined") return;
    try {
      const stored = window.localStorage.getItem("nolo-space-view-mode");
      if (stored === "categories" && viewMode !== "categories") {
