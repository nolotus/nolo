      ...
      const adapter = createAdapter({
        ...
        fetchImpl: async (_url, init) => {
          ...
          if (completeCount === 1) {
            ...
            // ask_user 默认注入（CORE/CLI_DEFAULT_TOOLS）。headless（dry-run /
            // 无 requestUserChoice）下模型若调用，executor 走降级路径返回编号
            // 菜单（server 端同为该降级），而非硬错误。
            expect(sentToolNames).toContain("ask_user");
