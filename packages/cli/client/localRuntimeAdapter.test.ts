...
describe("CLI local runtime adapter", () => {
  ...
  const DEFAULT_PRIVATE_LOCAL_TOOL_NAMES = [
    // ask_user 默认注入（TUI 默认启用，headless 降级为编号菜单）
    "ask_user",
    ...DEFAULT_LOCAL_CODING_TOOL_NAMES,
  ...
  test("can expose only declared local workspace tools for tool ablations", async () => {
    ...
    expect(toolNamesFromRequest(requests[0])).toEqual(["readFile", "execShell"]);
  });
  ...
  test("does not auto-inject fetchWebpage/exa_search in declared-only mode", async () => {
    ...
    const toolNames = toolNamesFromRequest(requests[0]);
    // declared-only mode skips default tools (fetchWebpage/exa_search/ask_user).
  ...
  test("declared-only 模式不注入 ask_user（默认注入，ablation 排除）", async () => {
    const requests: Array<{ body: any }> = [];
    const adapter = createAdapter({
      env: {
        OPENAI_API_KEY: "sk-local",
...
describe("resolveCliRequestedToolNames — ask_user 默认注入门控", () => {
  test("默认（非 declared-only）注入 ask_user（TUI 默认启用）", () => {
    const agentConfig = { key: "agent-default", tools: [] } as any;
    const requested = resolveCliRequestedToolNames(agentConfig, {} as any);
    expect(requested).toContain("ask_user");
  });

  test("declared-only 模式不注入 ask_user（headless 无交互通道）", () => {
    const agentConfig = { key: "agent-declared", tools: [] } as any;
    const requested = resolveCliRequestedToolNames(
      agentConfig,
      { NOLO_LOCAL_WORKSPACE_TOOLSET: "declared-only" } as any,
    );
