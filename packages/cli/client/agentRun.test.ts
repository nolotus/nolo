...
describe("cli agent run client", () => {
  ...
  test("runs forced local turns through the injected runtime adapter without HTTP", async () => {
    ...
    expect(output.text()).not.toContain("--- 当前时间 ---");
  });

  test("returns dialogId when a local provider failure is persisted (e.g. 402)", async () => {
    // localLoop saveTurn()s on provider errors and hangs dialogId on the thrown
    // error. The CLI must surface that id so TUI keeps state.dialogId and the
    // next user message continues instead of opening a fresh dialog.
    const output = new CaptureOutput();
    const result = await runAgentTurn({
      agentName: "frontend",
      agentKey: "frontend-local",
      ...
      localRuntimeAdapter: {
        ...
        loadDialogHistory: async () => [],
        saveTurn: async () => ({ dialogId: "dialog-after-402" }),
        resolveProvider: async () => ({
          model: "fake-local",
          complete: async () => {
            throw new Error("Insufficient Balance (UPSTREAM_402)");
          },
        }),
        executeTool: async () => {
          throw new Error("no tools expected");
  ...
  test("auto mode surfaces dialogId after local 402 without falling back to server", async () => {
    ...
    const result = await runAgentTurn({
      ...
      localRuntimeAdapter: {
        ...
        loadDialogHistory: async () => [],
        saveTurn: async () => ({ dialogId: "dialog-auto-402" }),
        resolveProvider: async () => ({
          model: "local-model",
          complete: async () => {
            throw new Error("Insufficient Balance (UPSTREAM_402)");
    ...
    expect(result.exitCode).toBe(1);
    expect(result.dialogId).toBe("dialog-auto-402");
    expect(result.localError).toBeInstanceOf(Error);
    expect(String((result.localError as Error).message)).toContain("UPSTREAM_402");
