...
// controlAgentRun 支持 list / status / wait / stop / todo / append：wait 轮询本地 run 记录
// 直到终态（复用 checkStaleRun 惰性对账），超时返回 status="timeout"（非失败）。
// append 支持终态 continuation（--continue <dialogId> 启动新后台 run 续跑）。
//
  ...
  return async (call: any): Promise<{ content: string; metadata?: Record<string, unknown> }> => {
    ...
    const dodCommands = Array.isArray(args.dodCommands)
      ? args.dodCommands
          ...
          .map((c: string) => c.trim())
      : typeof args.dod === "string" && args.dod.trim().length > 0
        ? [args.dod.trim()]
        : Array.isArray(args.dod)
          ? args.dod
              .filter((c: unknown): c is string => typeof c === "string" && c.trim().length > 0)
              .map((c: string) => c.trim())
          : undefined;
