...
describe("cli memory commands", () => {
  ...
  test("memory delete posts filters to all candidate servers when confirmed", async () => {
    ...
    // 破坏性写 fan-out 不得静默扩大到本地服务器：共享 resolveServerCandidates
    // 不含本地 origin，删除目标集必须排除 127.0.0.1:38123。
    expect(requests.map((request) => request.url)).not.toContain("http://127.0.0.1:38123/api/memory/delete");
