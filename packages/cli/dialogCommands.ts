...
async function readDialogOverHttp(args: {
  ...
  /** When true, only fetch messages (skip the meta GET). Restoring history for
   *  display already knows the dialog title from the picker, and the meta hop is
   *  by far the slowest (~900ms), so it is redundant here. */
...
export async function readDialogSnapshot(args: {
  ...
  const candidateBases = [
    ...resolveServerCandidates({ NOLO_SERVER: args.base }, args.base),
...
function formatDialogReadFailure(error: unknown, fallbackDialogId?: string): string {
  ...
  if (attempts.length > 0) {
    ...
    const deduped: HttpAttempt[] = [];
    for (const attempt of attempts) {
      const key = `${attempt.base}|${attempt.status ?? ""}|${attempt.message ?? ""}`;
      if (seen.has(key)) continue;
      seen.add(key);
      deduped.push(attempt);
      if (deduped.length >= MAX_ATTEMPT_LINES) break;
    }
    lines.push("  attempts:");
    for (const attempt of deduped) {
      const status = attempt.status != null ? ` HTTP ${attempt.status}` : "";
      const detail = attempt.message ? ` — ${attempt.message}` : "";
      lines.push(`    - ${attempt.base}${status}${detail}`);
    }
    if (attempts.length > deduped.length) {
      lines.push(`    … and ${attempts.length - deduped.length} more`);
    }
  }
  ...
  const hasConnRefusedOrTimeout = attempts.some((attempt) => {
    const text = `${attempt.status ?? ""} ${attempt.message ?? ""}`.toLowerCase();
    return (
      ...
      // bun fetch 对连接失败的真实文案（无 refused/timeout 关键词）。
  ...
  if (allNotFound) {
    ...
    lines.push(
      "  next-step: 存在连接拒绝/超时，请检查 NOLO_SERVER/--server 指向的 server 是否可达、是否在运行。",
    );
  }
