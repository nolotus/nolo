...
describe("cli tui workspace process", () => {
  ...
  test("Ctrl+O opens the latest reply in native copy view and restores the TUI", async () => {
    const { input, output } = createFakeTtyPair({ rows: 10, columns: 50 });
    const chunks: Uint8Array[] = [];
    output.on("data", (chunk) => chunks.push(toPlainUint8Array(chunk)));

    setTimeout(() => input.write("show reply\r"), 10);
    ...
    const result = await Promise.race([
      startTuiWorkspace({
        ...
        env: { NOLO_LANG: "en" },
        agentRunner: async (options) => {
          options.output.write("first copyable line\nsecond copyable line");
          setTimeout(() => input.write("\u000f"), 10);
          setTimeout(() => input.write("\x1b"), 40);
          setTimeout(() => input.write("/copy view\r"), 70);
          setTimeout(() => input.write("\r"), 100);
          setTimeout(() => input.write("/exit\r"), 130);
          return { exitCode: 0, dialogId: "dialog-copy-view" };
        },
      }).then(() => "done"),
      new Promise<"timeout">((resolve) => setTimeout(() => resolve("timeout"), 3000)),
    ]);
    ...
    expect(result).toBe("done");
    expect(stdout).toContain("first copyable line\nsecond copyable line");
    expect(stdout).toContain("Copy view");
    expect(stdout.match(/Copy view/g)).toHaveLength(2);
    expect(stdout).toContain("Press Esc or Enter to return");
    const copyViewTitleIndex = stdout.indexOf("Copy view");
    const mouseDisabledIndex = stdout.lastIndexOf(
      "\x1b[?1000l\x1b[?1006l",
      copyViewTitleIndex
    );
    const mouseRestoredIndex = stdout.indexOf(
      "\x1b[?1006h\x1b[?1000h",
      copyViewTitleIndex
    );
    expect(mouseDisabledIndex).toBeGreaterThan(-1);
    expect(mouseRestoredIndex).toBeGreaterThan(copyViewTitleIndex);
  });
