...
/** Add concise, renderer-agnostic output-format guidance to an agent prompt. */
export function prependOutputFormatGuidance(message: string): string {
  // Only recognize the exact block we own at the prompt prefix. A user's
  // mention of the heading must not suppress guidance injection.
  if (message.startsWith(OUTPUT_FORMAT_GUIDANCE)) return message;
  return `${OUTPUT_FORMAT_GUIDANCE}${message}`;
}
