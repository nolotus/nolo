...
describe("cli tui session", () => {
  ...
  test("starts in an out-of-the-box nolo workspace", () => {
    ...
    // auto-route is on by default, so the status line shows "auto" not "nolo"
    expect(renderStatusLine(state)).toContain("auto");
    expect(renderStatusLine(state)).not.toContain("🏔 nolo");
  ...
  test("NOLO_AUTO_ROUTE=0 keeps default agent at 256k with measured estimate", () => {
    ...
    expect(state.estimatedContextTokens).toBeGreaterThan(2_000);
    expect(renderStatusLine(state)).toMatch(/context: \d+(\.\d+)?% \([\d.]+k\/256k\)/);
    expect(renderStatusLine(state)).not.toContain("(0/");
  });

  test("NOLO_AUTO_ROUTE=0 /switch to default agent stays on 256k", () => {
    ...
    try {
      ...
      // Pretend custom agent had a 512k window already; switching back to
      // the default key must not silently upgrade to flash 1M.
      ...
      expect(switched.nextState.contextWindow).toBe(256_000);
    } finally {
  ...
  test("supports lightweight slash commands for switching agents and attaching docs", () => {
    ...
    // 自动路由开启时平台列表首项是 auto（与 nolo 默认 agent 同 key）
    expect(agents.output).toContain("1  auto");
    expect(agents.output).not.toContain("app-builder");
    expect(agents.output).toContain("auto-route each new dialog");

    const switched = handleTuiInput("/switch auto", state);
    expect(switched.nextState.agentName).toBe("auto");
    expect(switched.nextState.agentKey).toBe("agent-pub-01NOLOAPPBLD000000019KCKT0");
    expect(switched.output).toContain("Switched to auto");
  ...
  test("/switch auto returns to the default agent (auto-route)", () => {
    ...
    expect(switched.nextState.agentKey).toBe(DEFAULT_TUI_AGENT_KEY);
    expect(switched.output).toContain("Switched to auto");
    // 回到默认 key 后状态行显示 auto，而不是所选 agent 名
    expect(renderStatusLine(switched.nextState)).toContain("🏔 auto");
  ...
  test("shows auto on the status line for the default agent while auto-route is on", () => {
    const state = createInitialTuiState({
      NOLO_CLI_GIT_STATUS: "0",
    ...
    // 默认 agent（未显式选择）→ 显示 auto，agent 段不显示默认 agent 名
    expect(status).toContain("🏔 auto");
    expect(status).not.toContain("🏔 nolo");
    // runtime mode 标签保留（非重复的 auto）
    expect(status).toContain("server");
  });

  test("hides the duplicate runtime-mode auto label next to the auto-route label", () => {
    const state = createInitialTuiState({ NOLO_CLI_GIT_STATUS: "0" });
    const status = renderStatusLine(state);
    expect(status).toContain("🏔 auto");
  ...
  test("supports numeric switch targets from the built-in agent list", () => {
    ...
    expect(result.nextState.agentName).toBe("auto");
    expect(result.output).toContain("Switched to auto");
  });
