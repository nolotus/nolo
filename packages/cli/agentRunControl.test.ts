...
describe("agent run control plane", () => {
  ...
  test("runAgentStopCommand sends SIGTERM and marks killed", async () => {
    ...
    expect(lines.join("")).toContain("Sent SIGTERM");
  });

  test("runAgentKillCommand sends SIGKILL and marks killed", async () => {
    const kills: { pid: number; signal: string | number }[] = [];
    const deps = createDeps({
      kill: (pid, signal) => {
        kills.push({ pid, signal });
  ...
  test("signal commands handle already exited process", async () => {
    ...
    expect(code).toBe(0);
    expect(readRunRecord("run-1", deps)?.status).toBe("killed");
    expect(lines.join("")).toContain("already exited");
  });
...
describe("run activity tracker", () => {
  test("tracks inFlight and counters from loop events", () => {
    ...
    tracker.onLoopEvent({ kind: "tool-start", name: "execShell", atMs: Date.now() });
    expect(tracker.getActivity().inFlight?.kind).toBe("tool");

    tracker.onLoopEvent({ kind: "tool-end", name: "execShell", atMs: Date.now(), ok: true });
    expect(tracker.getActivity().inFlight).toBeNull();
    expect(tracker.getActivity().counters.toolCalls).toBe(1);

    tracker.onLoopEvent({ kind: "tool-end", name: "writeFile", atMs: Date.now(), ok: true });
