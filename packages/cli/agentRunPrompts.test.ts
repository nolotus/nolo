...
describe("agentRunPrompts - output format guidance", () => {
  test("prepends each format guideline while preserving the user message", () => {
    const message = "Explain the release process.";
    const guided = prependOutputFormatGuidance(message);

    expect(guided).toContain("Output format guidance:");
    expect(guided).toContain("Use semantic Markdown by default.");
    expect(guided).toContain("Use tables for genuinely multi-column structured data.");
    expect(guided).toContain("Use $...$ or $$...$$ for mathematical expressions.");
    expect(guided).toContain("Use ```mermaid fenced blocks only when diagrams materially clarify a process, dependency, architecture, or state transition.");
    expect(guided).toContain("Use language-tagged fenced blocks for code.");
    expect(guided).toContain("Do not add tables or Mermaid for decoration; when complex syntax is uncertain, prefer plain Markdown or text.");
    expect(guided).toContain(message);
    expect(guided.indexOf(message)).toBeGreaterThan(guided.indexOf("Output format guidance:"));
  });

  test("does not inject the guidance more than once", () => {
    const guided = prependOutputFormatGuidance("Existing task");
    expect(guided.match(/Output format guidance:/g)).toHaveLength(1);
    expect(prependOutputFormatGuidance(guided)).toBe(guided);
  });

  test("does not mistake a user mention of the heading for the full guidance", () => {
    const message = 'Search for the literal text "Output format guidance:".';
    const guided = prependOutputFormatGuidance(message);
    expect(guided.match(/Output format guidance:/g)).toHaveLength(2);
    expect(guided).toContain("Use semantic Markdown by default.");
    expect(guided).toContain(message);
  });
});
