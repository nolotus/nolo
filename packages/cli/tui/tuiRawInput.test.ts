...
describe("createFixedInput composer diffing & decoupled repaint", () => {
  test("re-rendering identical buffer and cursor skips full redraw but ensures cursor positioning (CUP)", () => {
    const tty = mockTty();
    const input = createFixedInput(tty.output, {
    ...
    // avoids wiping the screen (\x1b[J) but reliably emits cursor positioning (CUP)
    // so hardware cursor is returned to the input area after message streaming/rendering.
    input.repaint("hello", 5);
  ...
  test("exitOutputMode always repositions cursor to composer line", () => {
    const tty = mockTty();
    const input = createFixedInput(tty.output, {
      getStatusLine: () => "status: ready",
    });
    ...
    // Turn finishes and exitOutputMode runs with empty/unchanged draft
    input.exitOutputMode("", 0);
    expect(tty.chunks.length).toBeGreaterThan(0);
    const lastChunk = tty.chunks[tty.chunks.length - 1];
    expect(lastChunk).toMatch(/\x1b\[\d+;\d+H/);
  });
});
...
describe("createRawInputDecoder modal 关闭丢弃延迟 Enter", () => {
  test("Enter 走 debounce 期间 destroy() 丢弃它，不发射成 submit token", async () => {
    // 回归：confirm/action-gate 弹窗打开时，composer 的 decoder 与 dialog 的
    // key reader 是同一 stdin 上的两个 data 监听。Enter(\r) 因含换行在 decoder
    // 里走 unmarkedPasteDebounceMs(40ms) 延迟；dialog 同步关闭并把
    // modalOwnsKeyboard 翻回 false 后，这个延迟 Enter 才落地——若不清缓冲就会
    // 落进 handleInputToken 的提交路径，把当前草稿 enqueue 成 chat 消息。
    // 修复：modal 关闭时调用 decoder.destroy()（clearTimer + 清 pendingBuffer），
    // 让延迟的确认 Enter 被整体丢弃，而不是 40ms 后漏进提交路径。
    const tokens: string[] = [];
    const decode = createRawInputDecoder((token) => tokens.push(token), {
      unmarkedPasteDebounceMs: 40,
    });
    // 用户敲 Enter 确认弹窗：此刻 dialog 端已收到、即将关闭，composer 端
    // 这个 Enter 还在 debounce 缓冲里。
    decode("\r");
    expect(tokens).toEqual([]);
    // 弹窗关闭瞬间调用 destroy() 丢弃缓冲（等价于 releaseKeyboardToComposer）。
    decode.destroy();
    // 超过 debounce 时间：Enter 必须被丢弃，绝不作为 submit token 发出。
    await Bun.sleep(80);
    expect(tokens).toEqual([]);
    // 后续正常输入不受影响（destroy 只清缓冲不摘监听）。
    decode("a");
    expect(tokens).toEqual(["a"]);
  });

  test("destroy() 丢弃半截 CSI 尾，不泄漏进 composer", async () => {
    const tokens: string[] = [];
    const decode = createRawInputDecoder((token) => tokens.push(token), {
      escTimeoutMs: 15,
    });
