...
describe("applyTuiInputKey", () => {
  ...
  test("ignores the retired Ctrl+O shortcut without changing the draft", () => {
...
describe("handleTuiInput - copy", () => {
  test("keeps /copy as direct copy", () => {
    const state = createInitialTuiState({});
    expect(handleTuiInput("/copy", state).action).toEqual({ type: "copy-last" });
  });

  test.each(["/copy last", "/copy latest"])("maps %s to copy-last", (input) => {
    const result = handleTuiInput(input, createInitialTuiState({}));
    expect(result.action).toEqual({ type: "copy-last" });
  });

  test("maps /copy all to copy-all", () => {
    const result = handleTuiInput("/copy all", createInitialTuiState({}));
    expect(result.action).toEqual({ type: "copy-all" });
  });

  test.each(["/copy view", "/copy something", "/copy foo"])("rejects unsupported arguments: %s", (input) => {
    const result = handleTuiInput(input, createInitialTuiState({}));
    expect(result.action).toBeUndefined();
    expect(result.output).toBe("Usage: /copy");
  });
});
...
describe("handleTuiInput - /altscreen dispatch", () => {
  ...
  test("/math on returns set-math enabled true", () => {
    const state = createInitialTuiState({});
    const res = handleTuiInput("/math on", state);
    expect(res.action).toEqual({ type: "set-math", enabled: true });
    setCliLocale("en");
    expect(res.output).toBe(t("mathOn"));
  });

  test("/math off returns set-math enabled false", () => {
    const state = createInitialTuiState({});
    const res = handleTuiInput("/math off", state);
    expect(res.action).toEqual({ type: "set-math", enabled: false });
    setCliLocale("en");
    expect(res.output).toBe(t("mathOff"));
  });

  test("/math with no/bad arg shows usage", () => {
    const state = createInitialTuiState({});
    setCliLocale("en");
...
describe("handleTuiInput - path-vs-slash disambiguation", () => {
  ...
  test("/theme terminal and /theme auto select terminal-native mode", () => {
    const state = createInitialTuiState({});
    setActiveBrightness("dark");
    expect(handleTuiInput("/theme terminal", state).output).toContain("follows terminal colors");
    expect(getActiveBrightness()).toBeNull();
    setActiveBrightness("light");
    expect(handleTuiInput("/theme auto", state).output).toContain("follows terminal colors");
    expect(getActiveBrightness()).toBeNull();
  });
