...
describe("selectDialog", () => {
  ...
  test("re-anchors above the composer when the terminal resizes", async () => {
    ...
    const resultPromise = runSelectDialog<SelectDialogItem>({
      items: [{ label: "nolo" }],
      readKey: async () => keys.shift() ?? STREAM_CLOSED,
  ...
  test("drains buffered keys before reporting EOF", async () => {
    // Regression (review MEDIUM, same class as wheel-cancel): a single chunk
    // holding multiple legal keys followed immediately by stream end must
    // deliver every key in order, and only THEN report STREAM_CLOSED. The
    // old code reported EOF unconditionally on close, cancelling the dialog
    // while ArrowDown + Enter were still unread in the buffer.
    const input = new PassThrough() as unknown as NodeJS.ReadStream;
    (input as { isTTY?: boolean }).isTTY = true;
    const readKey = createRawKeyReader(input);

    // The reader is pull-based: each readKey() yields the next buffered key.
    const first = readKey();
    // A single chunk with two keys arrives, then immediately EOF.
    input.emit("data", "\x1b[B\r");
    input.emit("end");

    expect(await first).toBe("\x1b[B"); // ArrowDown (from the buffer, pre-EOF)
    expect(await readKey()).toBe("\r"); // Enter still buffered after EOF
    expect(await readKey()).toBe(STREAM_CLOSED);
  });

  test("drops incomplete trailing bytes at EOF without hanging", async () => {
    // Regression: after the stream closes, a buffer that holds only the start
    // of an SGR mouse report (e.g. `\x1b[<64;10`) can never complete. It must
    // be discarded and STREAM_CLOSED returned immediately — never a hang
    // waiting for bytes EOF will not deliver.
    const input = new PassThrough() as unknown as NodeJS.ReadStream;
    (input as { isTTY?: boolean }).isTTY = true;
    const readKey = createRawKeyReader(input);

    const readPromise = readKey();
    input.emit("data", "\x1b[<64;10");
    input.emit("end");

    // Timeout guard: fail the test (rather than hang) if STREAM_CLOSED never
    // arrives. A generous bound so a slow CI box won't flake on the guard.
    const result = await Promise.race([
      readPromise,
      new Promise((resolve) =>
        setTimeout(() => resolve("TIMEOUT_HANG"), 5000),
      ),
    ]);
    expect(result).toBe(STREAM_CLOSED);
  });

  test("pipe input ending in \x1b[B\r selects instead of cancelling", async () => {
    // Dialog-level regression: piping `\x1b[B\r` then EOF into a non-interactive
    // stream (script/CI) must consume ArrowDown + Enter and return the selected
    // item — not cancel because EOF raced the buffered keys.
    const input = new PassThrough() as unknown as NodeJS.ReadStream;
    (input as { isTTY?: boolean }).isTTY = true;
    const readKey = createRawKeyReader(input);
    const resultPromise = runSelectDialog<SelectDialogItem>({
