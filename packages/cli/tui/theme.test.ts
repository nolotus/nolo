...
describe("tui theme", () => {
  beforeEach(() => {
    ...
    // Most legacy cases below assert fixed-palette values. New terminal-mode
    // cases opt into the default explicitly and verify its ANSI-only contract.
    setActiveBrightness("dark");
  });

  test("terminal mode uses ANSI indexed colors even on truecolor terminals", () => {
    setActiveBrightness(null);
    expect(getActiveThemeMode()).toBe("terminal");
    expect(themeColorSequence("accent", { COLORTERM: "truecolor" })).toBe("\x1b[34m");
  });

  test("terminal mode disables surface and diff truecolor backgrounds", () => {
    setActiveBrightness(null);
    const env = { COLORTERM: "truecolor", TERM_PROGRAM: "ghostty" };
    expect(surfaceBackgroundSequence(env)).toBe("");
    expect(userSurfaceBackgroundSequence(env)).toBe("");
    expect(diffLineSequences(env)).toBeNull();
    expect(renderDiffLine({ kind: "added", text: "+ ok", env, colorEnabled: true }))
      .toBe("\x1b[32m+ ok\x1b[39m");
  });

  test("explicit light and dark modes retain truecolor palettes", () => {
    const env = { COLORTERM: "truecolor" };
    setActiveBrightness("light");
    expect(themeColorSequence("accent", env)).toMatch(/^\x1b\[38;2;/);
    expect(surfaceBackgroundSequence(env)).toMatch(/^\x1b\[48;2;/);
    setActiveBrightness("dark");
    expect(themeColorSequence("accent", env)).toMatch(/^\x1b\[38;2;/);
    expect(surfaceBackgroundSequence(env)).toMatch(/^\x1b\[48;2;/);
  });

  test("NOLO_TUI_THEME overrides the process-wide mode consistently", () => {
    setActiveBrightness("dark");
    expect(resolveTuiThemeMode({ NOLO_TUI_THEME: "light" })).toBe("light");
    expect(themeColorSequence("accent", {
      COLORTERM: "truecolor",
      NOLO_TUI_THEME: "light",
    })).toBe("\x1b[38;2;9;105;218m");

    setActiveBrightness("light");
    expect(resolveTuiThemeMode({ NOLO_TUI_THEME: "terminal" })).toBe("terminal");
    expect(themeColorSequence("accent", {
      COLORTERM: "truecolor",
      NOLO_TUI_THEME: "terminal",
    })).toBe("\x1b[34m");
  });
