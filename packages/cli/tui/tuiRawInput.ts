...
/** Soft-highlight collapsed paste chips and image paths so they read as chips, not typed text. */
function styleCollapsedPastePlaceholders(
  ...
  let result = text;
  if (result.includes("[paste #")) {
    COLLAPSED_PASTE_PLACEHOLDER_RE.lastIndex = 0;
    result = result.replace(COLLAPSED_PASTE_PLACEHOLDER_RE, (match) =>
      themeText(match, "accent", true),
    );
  }
  // Soft highlight image path tokens (e.g. .png, .jpg, .webp) in composer without shifting column width
  result = result.replace(
    /(?:^|\s)(["']?(?:~|\/|[a-zA-Z]:|\.\.?[\\/])[^\s"']+\.(?:png|jpg|jpeg|gif|webp)["']?)/gi,
    (match, p1) => {
      const prefix = match.slice(0, match.indexOf(p1));
      return `${prefix}${themeText(p1, "accent", true)}`;
    },
  );
  return result;
}
...
export function createFixedInput(
  ...
  const repaintAt = (buffer: string, cursorPos?: number, force = false) => {
    ...
    //
    // Always position the hardware cursor to the input composer row/column.
    // Even when composer text/dimensions did not change, external writes
    // (transcript history streaming, status messages, completed turns) leave
    // the hardware cursor in the message scroll region. Sending CUP here
    // guarantees the cursor is reliably returned to the input area without
    // redrawing or wiping (\x1b[J) the composer body.
