...
const withTruecolor = (fn: () => void) => {
  ...
  const prevBrightness = getActiveBrightness();
  process.env.NOLO_TUI_TRUECOLOR = "1";
  setActiveBrightness("dark");
  try {
    withColor(fn);
  } finally {
    setActiveBrightness(prevBrightness);
...
describe("buildHistoryLines — per-turn memoization", () => {
  ...
  test("default spacious layout inserts blank separators between turns", () => {
    noColor(() => {
...
describe("terminal-mode theme cache fingerprint", () => {
  test("palette switch invalidates finalized and streaming ANSI rows", () => {
    const previousTheme = getActiveThemeName();
    const previousBrightness = getActiveBrightness();
    try {
      setActiveBrightness(null);
      setActiveThemeName("catppuccin");
      resetStreamingTurnCache();
      const history = createTurnHistory();
      history.turns.push({ role: "user", content: "finalized row" });
      startTurn(history, "user");
      appendToCurrentTurn(history, "streaming row");

      withColor(() => {
        const blue = buildHistoryLines(history, 80);
        expect(blue.find((line) => line.includes("finalized row"))).toContain("\x1b[34m");
        expect(blue.find((line) => line.includes("streaming row"))).toContain("\x1b[34m");

        setActiveThemeName("rose");
        const magenta = buildHistoryLines(history, 80);
        expect(magenta.find((line) => line.includes("finalized row"))).toContain("\x1b[35m");
        expect(magenta.find((line) => line.includes("streaming row"))).toContain("\x1b[35m");
      });
    } finally {
      setActiveThemeName(previousTheme);
      setActiveBrightness(previousBrightness);
      resetStreamingTurnCache();
    }
  });
});
...
describe("virtualized renderHistory — windowed painting matches the full render path", () => {
  ...
  test("renders the tail of a long session without rebuilding the whole transcript", () => {
    ...
    noColor(() => {
      ...
      const lastAssistant = history.turns[199]!;
      const fullLines = buildHistoryLines(history, 99);
      // At width 99, the table makes turn 99's assistant block 23 rows. The
      // 22-row tail therefore starts on its second row (the heading), not on
      // the preceding user turn or the assistant's first line. Keep the
      // offset/render invariant explicit so this is not a string-only test.
      expect(countTurnLines(lastAssistant.role, lastAssistant.content, 99)).toBe(23);
      expect(renderTurnBlock(lastAssistant.role, lastAssistant.content, 99, false)).toHaveLength(23);
      expect(history.scrollTop).toBe(fullLines.length - 22);
      expect(writtenData).toContain("标题 99");
      expect(writtenData).toContain("结束段落 99");
    });
  });
});
...
describe("buildTurnOffsets & incremental streaming verification", () => {
  ...
  test("buildTurnOffsets calculates line offset indexes correctly across multiple turns, empty content, and single lines", () => {
    noColor(() => {
      ...
      // User turn at index 0 gets separator 1; every subsequent turn gets one too.
      let expectedRow = 0;
      for (let i = 0; i < offsets.entries.length; i++) {
        ...
        expect(entry.separatorAbove).toBe(1);
  ...
  test("buildTurnOffsets inserts unconditional separator gaps above every turn", () => {
    noColor(() => {
      const history = createTurnHistory();
      ...
      // Every turn gets separator 1 (user-first or after another turn).
      expect(offsets.entries[0]!.separatorAbove).toBe(1);
      expect(offsets.entries[1]!.separatorAbove).toBe(1);
      expect(offsets.entries[2]!.separatorAbove).toBe(1);

      expect(offsets.entries[0]!.startRow).toBe(1);
      expect(offsets.entries[1]!.startRow).toBe(1 + offsets.entries[0]!.lineCount + 1);
    });
  });
    ...
    test("strictly matches renderTurnBlock output line count across diverse markdown content", () => {
      const testCases: { name: string; role: "user" | "assistant"; content: string }[] = [
        ...
        },
        {
          ...
          role: "assistant",
          content: "[nolo] Executing tool: globFiles...\nResult output line.",
        },
        {
  ...
  describe("renderHistory — double buffering & line-level frame diffing", () => {
    ...
    test("buildTurnOffsets caches line counts across multiple widths without re-wrapping", () => {
      const history = createTurnHistory();
      startTurn(history, "assistant");
      appendToCurrentTurn(history, "A very long sentence that wraps across multiple terminal columns when the terminal is narrow.");
      finalizeCurrentTurn(history);

      const offset80 = buildTurnOffsets(history, 80);
      const offset40 = buildTurnOffsets(history, 40);
      const offset80Again = buildTurnOffsets(history, 80);
