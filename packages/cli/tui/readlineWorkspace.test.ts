...
describe("renderHistory", () => {
  test("renders user and assistant turns in the main area", () => {
    ...
    // Per-row clear (positioning + padding) only — full-screen ED would wipe the docked composer.
...
describe("Ctrl+C 防误退", () => {
  ...
  test("Idle + 空草稿：单次 Ctrl+C 不退出，输出提示；1000ms 内第二次才退出", async () => {
    ...
    // 第一次 Ctrl+C：不退出，输出双击提示。
    input.write("\x03");
    await Bun.sleep(80);
    expect(stripAnsi(stdout())).toContain(t("ctrlCExitHint"));

    // 第二次（1000ms 内）：退出。
    input.write("\x03");
    const exited = await Promise.race([wp.then(() => true), Bun.sleep(3000)]);
    expect(exited).toBe(true);
  });

  test("Idle + 草稿非空：Ctrl+C 只清空草稿，绝不退出", async () => {
    const { input, output, stdout } = makeStreams();
    const wp = startTuiWorkspace({
      scriptDir: "",
      input,
    ...
    input.write("\x03");
    await Bun.sleep(80);
    expect(stripAnsi(stdout())).toContain(t("ctrlCClearedDraft"));
  ...
  test("Idle + 有鼠标选区：Ctrl+C 提取选区并复制，清除高亮，绝不退出", async () => {
    const input = new PassThrough() as PassThrough & {
      isTTY?: boolean;
      setRawMode?: (mode: boolean) => void;
    };
    ...
    output.isTTY = true;
    output.rows = TERM_ROWS;
    output.columns = 21; // narrow so the reply wraps across many rows
    input.setRawMode = () => {};
    const chunks: string[] = [];
    output.on("data", (chunk) => chunks.push(String(chunk)));
    const stdout = () => chunks.join("");
    ...
    const wp = startTuiWorkspace({
      ...
      agentRunner: async (opt) => {
        opt.output.write("abcdefghijklmnopqrstuvwxyz");
    ...
    input.write("q\r");
    let waited = 0;
    // 等待回复渲染完成（stdout 里出现回复内容前缀）。
    while (!stripAnsi(stdout()).includes("abcdefghijklmnopqr") && waited < 1000) {
      await Bun.sleep(20);
      waited += 20;
    }
    await Bun.sleep(80);
    ...
    // 输出提示（可能被换行/空格包裹）。
    expect(stripAnsi(stdout())).toContain("已复制选区");
  ...
  test("Busy：Ctrl+C 保持中止 Turn，绝不退出", async () => {
    ...
    input.write("start\r");
    let waited = 0;
    while (turnCount < 1 && waited < 1000) { await Bun.sleep(20); waited += 20; }
    await Bun.sleep(40);

    // Busy 期间 Ctrl+C：中止 turn，不退出。
    input.write("\x03");
    await Bun.sleep(80);
    // 进程仍存活。
    release!();
  ...
  test("/copy all 提取完整对话 Markdown 到剪贴板并输出提示", async () => {
    const { input, output, stdout } = makeStreams();
    const copied: string[] = [];
    const wp = startTuiWorkspace({
      scriptDir: "",
      ...
      clipboardWriter: async (text) => { copied.push(text); },
      agentRunner: async (opt) => {
        opt.output.write("assistant reply");
        return { exitCode: 0, dialogId: "d" };
      },
    });
    await Bun.sleep(40);
    // 一轮对话：user "q" + assistant reply。
    input.write("q\r");
    let waited = 0;
    while (!stripAnsi(stdout()).includes("assistant reply") && waited < 1000) {
      await Bun.sleep(20);
      waited += 20;
    }
    await Bun.sleep(60);

    input.write("/copy all\r");
    await Bun.sleep(100);
    expect(copied.length).toBe(1);
    // Markdown 里同时含 User 与 Assistant 两节。
    expect(copied[0]).toContain("## User");
    expect(copied[0]).toContain("## Assistant");
    expect(copied[0]).toContain("assistant reply");
    expect(stripAnsi(stdout())).toContain("已复制完整对话");
  ...
  test("/copy 复制最近一条 assistant 回复（含 OSC 52 已由默认路径覆盖）", async () => {
    const { input, output, stdout } = makeStreams();
    const copied: string[] = [];
    const wp = startTuiWorkspace({
      scriptDir: "",
      ...
      clipboardWriter: async (text) => { copied.push(text); },
      agentRunner: async (opt) => {
        opt.output.write("latest assistant text");
        return { exitCode: 0, dialogId: "d" };
      },
    });
    await Bun.sleep(40);
    input.write("q\r");
    let waited = 0;
    while (!stripAnsi(stdout()).includes("latest assistant text") && waited < 1000) {
      await Bun.sleep(20);
      waited += 20;
    }
    await Bun.sleep(60);
    ...
    await Bun.sleep(100);
    expect(copied.length).toBe(1);
    expect(copied[0]).toContain("latest assistant text");
...
describe("scroll-aware history", () => {
  ...
  test("applyScrollAction scrolls by wheel lines and refollows at bottom", () => {
    ...
    expect(history.scrollTop).toBe(5);
  ...
  test("executes /theme locally during busy turn without enqueuing, while normal text remains queued", async () => {
    ...
    const themeMessage = t("themeBrightnessSwitched", "light");
    expect(outputTextAfterTheme).toContain(themeMessage);
    // The busy-path notice is followed by a composer cursor repaint, proving
    // the newly selected mode is applied now rather than waiting for another
    // assistant stream chunk. renderHistory is frame-diffed and may emit no
    // bytes here because ANSI indexed light/dark foregrounds are identical.
    expect(outputTextAfterTheme.slice(outputTextAfterTheme.indexOf(themeMessage)))
      .toMatch(/\x1b\[\d+;\d+H/);
...
// Regression: 修复 ask_user 选择框关闭后 TUI 无法再输入。
// 根因：runOneAgentTurn 的 requestUserChoice finally 里调用 `onData.destroy()`，
// 但 `onData` 只在交互式 composer 的 `isInteractiveInput` 块内声明，runOneAgentTurn
// 作用域不可见 —— 运行时报 `ReferenceError: onData is not defined`，finally 在
// `modalOwnsKeyboard = false` 之前中断，导致键盘锁永久卡住，之后所有按键都被丢弃。
// 修复：把 composer 解码器的 drain 提升到 workspace 作用域的 composerDecoderDrain
// 钩子，ask_user 关闭时调用它清掉延迟 Enter，并复位 modalOwnsKeyboard。
describe("ask_user choice dialog resumes composer input after closing (modalOwnsKeyboard regression)", () => {
  type FakeInput = PassThrough & {
    isTTY?: boolean;
    isRaw?: boolean;
    setRawMode?: (mode: boolean) => void;
  ...
  test("feed a follow-up turn after answering an ask_user dialog (input not frozen)", async () => {
    const input = new PassThrough() as FakeInput;
    const output = new PassThrough() as FakeOutput;
    input.isTTY = true;
    output.isTTY = true;
    ...
    const workspacePromise = startTuiWorkspace({
      ...
      agentRunner: async (opt: any) => {
        turnsProcessed.push(opt.message);
        if (turnsProcessed.length === 1) {
          // 模型通过 ask_user 向用户提问：渲染选择框并等待用户作答。
          ...
          opt.output.write(`CHOICE_ACCEPTED:${String((choice as any).label)}`);
        }
        return { exitCode: 0, dialogId: "test-dialog" };
      },
    });
    ...
    while (Date.now() < d1) {
      if (Buffer.concat(chunks).toString("utf8").includes("Option A")) {
        sawChoice = true;
        break;
      }
      await new Promise((r) => setTimeout(r, 20));
    ...
    // 用 Enter 作答：单选中选中第一项并自动提交，关闭对话框。
    input.write("\r");

    // 等待首轮完成（agentRunner 已拿到 choice 并回写）。
    let turn1Done = false;
    const d2 = Date.now() + 5000;
    while (Date.now() < d2) {
      if (Buffer.concat(chunks).toString("utf8").includes("CHOICE_ACCEPTED")) {
        turn1Done = true;
        break;
      }
      await new Promise((r) => setTimeout(r, 20));
    ...
    // 留出 turn 收尾 / composer 恢复的余量。
    await new Promise((r) => setTimeout(r, 80));

    // 关键回归断言：ask_user 之后 composer 必须能再次接收输入。
    // 修复前 modalOwnsKeyboard 因 onData ReferenceError 卡住，这条 follow-up
    // 会被丢弃，agentRunner 不会第二次被调用。
    input.write("follow up\r");
    const d3 = Date.now() + 5000;
    while (Date.now() < d3) {
      if (turnsProcessed.includes("follow up")) break;
      await new Promise((r) => setTimeout(r, 20));
    }
    expect(turnsProcessed).toContain("follow up");
...
describe("/resume renders restored assistant turns through the full renderer", () => {
  ...
  test("table is rendered as box lines (no raw separator)", async () => {
    const tableMarkdown =
      "| 水果 | 颜色 |\n| --- | --- |\n| 苹果 | 红 |";
    const out = await runResume([
      { role: "assistant", content: tableMarkdown },
    ]);
    // 原断言（bullet）已被有意废弃：commit e3407368c 把 markdown 表格从
    // "• 苹果 — 红" bullet 改成真实框线表。保留原意图——恢复的历史消息经过
    // 完整渲染器，不泄漏裸 markdown 分隔符——但断言改为匹配新渲染。
...
describe("alternate screen isolation (startTuiWorkspace)", () => {
  test("TTY 启动写 ?1049h 且启动序列不再含 \\x1b[3J", async () => {
    ...
    expect(out).not.toContain("\x1b]11;?\x07");
    input.write("/exit\r");
    input.end();
    await Promise.race([workspacePromise, new Promise((r) => setTimeout(r, 3000))]);
  });
...
describe("readlineWorkspace paste store clearance contract", () => {
  ...
  test("附件消费制：直接 chat 提交在发出消息前清空 attachedImages", () => {
    // HIGH review 修复的源码契约 pin：直接提交路径里 imageUrls 组装完成后、
    ...
    const chatBlockStart = code.indexOf('result.action?.type === "chat"');
    // runTuiWorkspace 里有两处 `if (isInteractiveInput(input))`，终点取 chat
    // 块之后的第一处（runSubmittedLine 收尾），否则切片为空。
    const chatBlock = code.slice(
      chatBlockStart,
      code.indexOf("if (isInteractiveInput(input))", chatBlockStart),
    );
    expect(chatBlock).toContain("attachedImages: []");
    // 清空必须发生在消息发出之前。
    expect(chatBlock.indexOf("attachedImages: []")).toBeLessThan(
      chatBlock.indexOf("runOneAgentTurn("),
    );
...
describe("terminal-native theme polling contract", () => {
  test("does not create an OSC 11 auto-theme interval", () => {
    const code = readFileSync(join(import.meta.dir, "readlineWorkspace.ts"), "utf8");
    expect(code).not.toContain("AUTO_THEME_POLL_MS");
    expect(code).not.toContain("maybeAutoRefreshTheme");
    expect(code).not.toContain("autoThemeTimer");
  });

  test("a workspace env override does not leak into the next workspace", async () => {
    setActiveThemeMode("terminal");

    const first = makeTtyIo();
    const firstPromise = startTuiWorkspace({
      scriptDir: "",
      input: first.input,
      output: first.output,
      env: { NOLO_TUI_THEME: "light" },
      agentRunner: async () => ({ exitCode: 0, dialogId: "d" }),
    });
    // startTuiWorkspace applies its env mode synchronously before awaiting the
    // fixed-mode OSC probe.
    expect(getActiveThemeMode()).toBe("light");
    first.input.write("\x1b]11;rgb:ffff/ffff/ffff\x07");
    await new Promise((resolve) => setTimeout(resolve, 10));
    first.input.write("/exit\r");
    first.input.end();
    await firstPromise;
    expect(getActiveThemeMode()).toBe("terminal");

    const second = makeTtyIo();
    const secondPromise = startTuiWorkspace({
      scriptDir: "",
      input: second.input,
    ...
    expect(getActiveThemeMode()).toBe("terminal");
    second.input.write("/exit\r");
    second.input.end();
    await secondPromise;
  });

  test("older overlapping workspace cleanup cannot reset the newer owner", async () => {
    ...
    secondInput.end("/exit\n");
    await secondPromise;
    expect(getActiveThemeMode()).toBe("terminal");
  });

  test("non-interactive workspace cleanup restores terminal mode", async () => {
    setActiveThemeMode("terminal");
    const input = new PassThrough();
    const output = new PassThrough();
    const workspacePromise = startTuiWorkspace({
  ...
  test("startup render failure still releases workspace theme state", async () => {
    const input = new PassThrough();
    const output = {
      isTTY: false,
      write: () => {
        throw new Error("early paint failed");
      },
    } as unknown as NodeJS.WritableStream;
    const workspacePromise = startTuiWorkspace({
      scriptDir: "",
    ...
    expect(getActiveThemeMode()).toBe("light");
    await expect(workspacePromise).rejects.toThrow("early paint failed");
    expect(getActiveThemeMode()).toBe("terminal");
    expect(getActiveTerminalBaseHex()).toBeNull();
  });

  test("workspace cleanup clears the probed terminal base before fixed dark fallback", async () => {
    const truecolorDarkEnv = {
      COLORTERM: "truecolor",
      NOLO_TUI_THEME: "dark",
    };
    setActiveThemeMode("dark");
    setActiveTerminalBaseHex(null);
    const expectedFallbackBg = diffLineSequences(truecolorDarkEnv, "dark")!.added.bg;
    setActiveThemeMode("terminal");
    ...
    // Fake TTY streams cannot answer the production terminal-device guard, so
    // record the same white base an OSC reply would have produced.
    setActiveTerminalBaseHex("FFFFFF");
    expect(getActiveTerminalBaseHex()).toBe("FFFFFF");
    first.input.write("/exit\r");
    first.input.end();
    await firstPromise;
    expect(getActiveTerminalBaseHex()).toBeNull();
    ...
    const secondPromise = startTuiWorkspace({
      ...
      input: second.input,
      output: second.output,
      env: { NOLO_TUI_THEME: "dark" },
      agentRunner: async () => ({ exitCode: 0, dialogId: "d" }),
    });
    // Let the silent OSC probe and system fallback settle. An empty fallback
    // hex must leave the terminal base null rather than reviving A's white.
    await new Promise((resolve) => setTimeout(resolve, 350));
    expect(getActiveTerminalBaseHex()).toBeNull();
    expect(diffLineSequences(truecolorDarkEnv, "dark")!.added.bg)
      .toBe(expectedFallbackBg);
    second.input.write("/exit\r");
    second.input.end();
    await secondPromise;
    expect(getActiveTerminalBaseHex()).toBeNull();
...
describe("pinned agent notice", () => {
  ...
  test("paste shortcut (\\x16) triggers clipboard image read and downgrades without crashing", async () => {
    const input = new PassThrough() as PassThrough & {
      isTTY?: boolean;
      setRawMode?: (mode: boolean) => void;
    };
    ...
    const workspacePromise = startTuiWorkspace({
      ...
      output,
      env: {
        SSH_CONNECTION: "1.2.3.4 5678 5.6.7.8 22", // simulate remote session for deterministic downgrade
      },
      agentRunner: async () => {
        agentTurnRan = true;
        return { exitCode: 0, dialogId: "test" };
      },
    });

    await tick(50);

    // Send paste shortcut \x16 (Ctrl+V)
    input.write("\x16");
    await tick(100);

    const outText = Buffer.concat(chunks).toString("utf8");
    // Verify downgrade message was emitted
    expect(outText).toContain("远程会话无法读取本地剪贴板，请拖入图片文件或直接输入图片路径");

    // Verify input continues to work normally
    input.write("hello after paste\r");
    await tick(100);
    expect(agentTurnRan).toBe(true);
