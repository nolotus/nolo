...
import { createRawKeyReader, STREAM_CLOSED } from "./selectDialog";
...
describe("runAskChoiceDialog", () => {
  ...
  function makeKeyReader(keys: Array<string | typeof STREAM_CLOSED>) {
    let index = 0;
    const readKey = async () => {
      const next = keys[index++];
      return next === undefined ? STREAM_CLOSED : next;
    };
  ...
  test("a single chunk of 50 wheel reports does not cancel and keeps the cursor reasonable", async () => {
    // Regression: one wheel/trackpad gesture sends dozens of SGR reports in a
    // single stdin chunk. They must be parsed one-by-one (no cancel from a
    // "closed stream" misfire) AND the cursor must not fly off — the wheel
    // throttle caps how far a single gesture can move it.
    setCliLocale("en");
    const input = new PassThrough() as unknown as NodeJS.ReadStream;
    (input as { isTTY?: boolean }).isTTY = true;
    const readKey = createRawKeyReader(input);
    ...
    const resultPromise = runAskChoiceDialog({
      request: {
        ...
        choices: [
          { id: "a", label: "A", userMessage: "chose A" },
          { id: "b", label: "B", userMessage: "chose B" },
          { id: "c", label: "C", userMessage: "chose C" },
          { id: "d", label: "D", userMessage: "chose D" },
          { id: "e", label: "E", userMessage: "chose E" },
        ],
        blocking: true,
      },
      output: {
    ...
    // 50 wheel-down reports (code 65 = wheel down), all in one chunk.
    const reports = Array.from({ length: 50 }, (_, i) => `\x1b[<65;3;${i + 1}M`).join("");
    input.emit("data", reports);
    await new Promise((r) => setTimeout(r, 30));

    // The dialog must still be open (not cancelled) and the cursor must have
    // moved only a bounded amount (throttle caps a single gesture), so it must
    // NOT have reached the last row E.
    expect(writes.join("")).toContain("❯");
    expect(writes.join("")).not.toContain("❯ [5] E");
    // A later genuine cancel still works after the burst.
    input.emit("data", "\x1b");
    const result = await resultPromise;
    expect(result).toEqual({ kind: "cancelled" });
  });

  test("pipe input ending in arrow-down + enter selects instead of cancelling", async () => {
    // Dialog-level regression: piping `\x1b[B\r` then EOF through the real raw
    // key reader (non-interactive script/CI path) must consume the buffered
    // ArrowDown + Enter and return the selected choice — NOT cancel because EOF
    // raced the unread keys (the wheel-cancel class of bug).
    setCliLocale("en");
    const input = new PassThrough() as unknown as NodeJS.ReadStream;
    (input as { isTTY?: boolean }).isTTY = true;
    const readKey = createRawKeyReader(input);
