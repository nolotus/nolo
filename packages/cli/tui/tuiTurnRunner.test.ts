...
/**
 * `/auto on` escape hatch (docs/plans/2026-08-31-write-gate-escape-hatch.md).
 * `readlineWorkspace.ts`'s actionGateHandler short-circuits with exactly
 * `if (state.autoConfirm && isAutoConfirmableFileWriteGate(gate)) return
 * buildGateConfirmedResult(gate);` — these tests cover that predicate's
 * contract directly, since the handler itself is a closure inside
 * `runTuiWorkspace` and not independently exported.
 */
describe("isAutoConfirmableFileWriteGate — /auto on safety boundary", () => {
  const fileWriteGate: LocalAgentActionGate = {
    id: "permission-file-write-session-call-1",
    kind: "confirm",
    title: "确认写入文件",
    body: "本会话首次写入文件：a.ts。",
    toolName: "writeFile",
    toolCallId: "call-1",
    // `action` is carried on the gate via the PermissionRequest spread at
    // localLoop.ts:1697 (`{ ...policy.permissionRequest, id, kind: "confirm",
    // toolName, toolCallId }`); the base `ActionGate` type doesn't declare
    // it, so it's attached here the same way runtime construction does.
    ...({ action: "file_write" } as Record<string, unknown>),
  } as LocalAgentActionGate;

  test("a file_write confirm gate is auto-confirmable", () => {
    expect(isAutoConfirmableFileWriteGate(fileWriteGate)).toBe(true);
  });

  test("buildGateConfirmedResult(gate) is the exact shape /auto returns for it", () => {
    expect(buildGateConfirmedResult(fileWriteGate)).toEqual({
      content: `action gate completed: ${fileWriteGate.title}`,
      metadata: {
        actionGateResult: {
          gateId: fileWriteGate.id,
          status: "completed",
          output: "confirmed",
        },
      },
  ...
  test("a confirm gate for a different action is NOT auto-confirmable", () => {
    const otherConfirmGate = {
      ...fileWriteGate,
      action: "some_other_capability",
    } as LocalAgentActionGate;
    expect(isAutoConfirmableFileWriteGate(otherConfirmGate)).toBe(false);
  });

  test("a confirm gate carrying no action at all is NOT auto-confirmable", () => {
    const bareConfirmGate: LocalAgentActionGate = {
      id: "gate-bare",
      kind: "confirm",
      title: "确认某操作",
      toolName: "writeFile",
    ...
    expect(isAutoConfirmableFileWriteGate(bareConfirmGate)).toBe(false);
  });

  test("safety boundary: a handoff gate is never auto-confirmable, even with action: file_write", () => {
    const handoffGate = {
      id: "gate-handoff",
      kind: "handoff",
      title: "run an interactive command",
      toolName: "execShell",
      toolCallId: "call-handoff",
      action: "file_write",
    } as unknown as LocalAgentActionGate;
    expect(isAutoConfirmableFileWriteGate(handoffGate)).toBe(false);
  });

  test("safety boundary: an input gate is never auto-confirmable, even with action: file_write", () => {
    const inputGate = {
      id: "gate-input",
      kind: "input",
    ...
    expect(isAutoConfirmableFileWriteGate(inputGate)).toBe(false);
  });
});
