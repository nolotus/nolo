...
export function handleTuiInput(input: string, state: TuiState): TuiInputResult {
  ...
  switch (command) {
    ...
    case "/theme": {
      ...
      if (!sub) {
        const brightness = getActiveThemeMode();
        return {
          nextState: state,
          output: [
      ...
      if (sub === "auto" || sub === "terminal") {
        setActiveBrightness(null);
        return {
          nextState: state,
          output: t("themeBrightnessAuto"),
        };
      }
    ...
    case "/cd": {
      if (!argText) {
        // 无参数：只显示当前 cwd + 用法，不改状态、不产 action。
        return {
          nextState: state,
      ...
      try {
        if (!statSync(target).isDirectory()) {
          return { nextState: state, output: t("cdNotDir", target) };
        }
      } catch {
        return { nextState: state, output: t("cdNotFound", target) };
      }
    ...
    case "/math": {
      if (argText !== "on" && argText !== "off") {
        return { nextState: state, output: t("mathUsage") };
      }
      return {
        nextState: state,
        output: argText === "on" ? t("mathOn") : t("mathOff"),
        action: { type: "set-math", enabled: argText === "on" },
      };
    }
    case "/altscreen": {
      if (argText !== "on" && argText !== "off") {
