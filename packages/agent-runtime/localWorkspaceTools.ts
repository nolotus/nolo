...
/** Upper bound on internalSearchWorkspace tool output (matches legacy searchFiles). */
...
async function runWorkspaceCommandLimitedLines(args: {
  ...
  const stdoutPromise = new Promise<void>((resolveRead, rejectRead) => {
    proc.on("error", rejectRead);
    ...
    proc.stdout?.on("close", () => resolveRead());
    if (!proc.stdout) {
  ...
  try {
    await Promise.all([stdoutPromise, exitSettled]);
  } catch (error) {
...
function defensivelyTruncateReadFile(args: {
  ...
  if (lines.length <= 10) {
    ...
    return {
      content: `${head}\n\n[... ${omitted} characters omitted. Content exceeds limit (${args.content.length} chars). Use lines slice or codeSearch to inspect specific parts. ...]\n\n${tail}`,
  ...
  return {
    content: `${head}\n\n[... Omitted ${Math.max(1, omittedLines)} lines (${omittedChars} chars). File exceeds safety limit (${args.content.length} chars, ${args.totalLines} lines). Use lines="start-end" (e.g. lines="151-300") or codeSearch to read specific sections. ...]\n\n${tail}`,
    truncatedByCharLimit: true,
  };
}
...
// ───────────────────────────────────────────────────────────────────────────
// Internal workspace primitives (Cursor exec bridge only)
//
// These are the read-only bridges the Cursor provider (`grepArgs` / `lsArgs`)
// routes through `ctx.executeTool` → policy → executor. They are NOT exposed to
// any model schema (not in WORKSPACE_TOOL_NAMES, no executor registration in
// buildLocalWorkspaceOpenAiTools / TOOL_PACKS / aliases). They deliberately
// restore the internal behavior that the legacy listFiles/searchFiles executors
// provided (readdir+stat with trailing "/" dirs; rg → grep → JS search), which
// globFiles (files-only) and codeSearch (not in the local executor set) cannot
// reproduce.
// ───────────────────────────────────────────────────────────────────────────
...
async function listWorkspaceEntries(args: {
  ...
  const visit = async (dirPath: string, depth: number) => {
    ...
    for (const name of names) {
      ...
      // Emit paths relative to the requested directory (the Cursor ls bridge
      // feeds these to buildLsResultFromToolResult, which prepends the requested
      // path to build absPath — so workspace-root-relative would double-prefix).
      const relativePath = relative(args.dirPath, absolutePath) || ".";
  ...
  return { entries, truncated, limitedByMaxDepth };
}

export interface InternalSearchWorkspaceArgs {
  workspaceRoot: string;
  path?: string;
  query: string;
  caseSensitive?: boolean;
...
export interface InternalListWorkspaceEntriesArgs {
  workspaceRoot: string;
  path?: string;
  maxDepth?: number;
  maxResults?: number;
...
/**
 * Internal search primitive (Cursor grep bridge). Replicates the legacy
 * searchFilesTool internals: rg → grep → JS fallback, default case-sensitive,
 * `literal:false` treats the query as a regex. Output lines are
 * `path:line:content` (+ context lines with `-` separators) and metadata
 * reports the search engine used. Respects resolveLocalWorkspaceToolPath path
 * safety and ALWAYS_EXCLUDED_GLOBS exclusion semantics.
 */
 ...
 * recursion bounded by maxDepth (default 1 → direct children only). Cursor's
 * buildLsResultFromToolResult parses "/"-suffixed lines into childrenDirs and
 * the rest into childrenFiles, so maxDepth=1 yields only direct children
 * (e.g. under `src`, `a.ts` and `components/` but not `components/Button.tsx`).
 */
export async function internalListWorkspaceEntries(args: InternalListWorkspaceEntriesArgs): Promise<AgentRuntimeToolResult> {
  const requestedPath = args.path ?? ".";
