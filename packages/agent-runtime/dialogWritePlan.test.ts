...
describe("agent runtime dialog write plan", () => {
  ...
  test("FINDING-1: existing checkpoint's availableToolNames is pruned even when result has no runtimeToolSurface", () => {
    const now = 1710000000000;
    const plan = buildAgentRuntimeDialogWritePlan({
      input: {
        agentKey: "agent-user-1-frontend",
        continueDialogId: "legacy-dialog",
        messages: [{ role: "user", content: "continue" }],
        result: { content: "done", model: "m" },
      },
      userId: "user-1",
    ...
    const checkpoint = plan.ops[0]?.value?.runtimeCheckpoint;
    expect(checkpoint).toBeDefined();
    expect(checkpoint.availableToolNames).toBeUndefined();
    expect(checkpoint.lastToolNames).toEqual(["readFile"]);
    expect(checkpoint.status).toBe("done");
  });

  test("FINDING-1: runtimeToolSurface is still written as toolSurface when present", () => {
    const now = 1710000000000;
    const plan = buildAgentRuntimeDialogWritePlan({
      input: {
        agentKey: "build_agent",
        continueDialogId: "d",
        messages: [{ role: "user", content: "continue" }],
        result: {
          content: "done",
          model: "m",
          runtimeToolSurface: { finalToolNames: ["readFile", "writeFile"] },
        },
      },
      userId: "user-1",
      now,
    ...
    expect(checkpoint).toBeDefined();
    expect(checkpoint.availableToolNames).toBeUndefined();
    expect(checkpoint.toolSurface).toEqual({
      finalToolNames: ["readFile", "writeFile"],
    });
    expect(checkpoint.status).toBe("running");
  });

  test("FINDING-HIGH: legacy-only checkpoint (availableToolNames only) is pruned to an empty checkpoint, not re-spread", () => {
    const now = 1710000000000;
    const plan = buildAgentRuntimeDialogWritePlan({
      input: {
        agentKey: "build_agent",
    ...
    const checkpoint = plan.ops[0]?.value?.runtimeCheckpoint;
    // Must exist (to override the stale checkpoint spread from existingDialog)...
    expect(checkpoint).toBeDefined();
    // ...and must no longer carry the pruned legacy field.
    expect(checkpoint.availableToolNames).toBeUndefined();
  });

  test("FINDING-HIGH: no existing runtimeCheckpoint and no runtimeToolSurface yields no runtimeCheckpoint field", () => {
    const now = 1710000000000;
    const plan = buildAgentRuntimeDialogWritePlan({
      input: {
        agentKey: "build_agent",
        continueDialogId: "fresh-dialog",
        messages: [{ role: "user", content: "continue" }],
        result: { content: "done", model: "m" },
      },
      userId: "user-1",
