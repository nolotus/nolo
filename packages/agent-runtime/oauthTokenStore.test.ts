...
describe("credential location follows NOLO_HOME", () => {
  // Regression: these paths used to resolve to the developer's real home even
  // in an isolated test process, so tests could only exercise OAuth wiring by
  // overwriting the developer's actual tokens and restoring them afterwards —
  // one crash away from leaving a fake token in place of a real login.
  ...
  test("NOLO_HOME redirects the credentials dir and every provider file", () => {
    process.env.NOLO_HOME = "/tmp/nolo-home-probe";
    expect(getCredentialsDir()).toBe("/tmp/nolo-home-probe/credentials");
    expect(getCredentialPath("xai")).toBe(
      "/tmp/nolo-home-probe/credentials/xai.json",
    );
  });

  test("an explicit homeDir still wins over the ambient NOLO_HOME", () => {
    process.env.NOLO_HOME = "/tmp/nolo-home-probe";
    expect(getCredentialPath("xai", "/tmp/explicit-home")).toBe(
      "/tmp/explicit-home/.nolo/credentials/xai.json",
    );
  });
});
