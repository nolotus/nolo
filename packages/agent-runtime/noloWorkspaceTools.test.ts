...
describe("in-process readDialog", () => {
  test("reads dialog meta and messages in-process when store is provided", async () => {
    const db = new Map<string, any>();
    db.set("dialog-user1-01ARZ3NDEKTSV4RRFFQ69G5FAV", {
      title: "Test Dialog",
      status: "done",
      runtimeCheckpoint: { status: "done" },
    });
    db.set("dialog-01ARZ3NDEKTSV4RRFFQ69G5FAV-msg-01A", {
      role: "user",
      content: "hello",
    });
    db.set("dialog-01ARZ3NDEKTSV4RRFFQ69G5FAV-msg-01B", {
      role: "assistant",
      content: "hi there",
    });

    const store = {
      read: async (key: string) => db.get(key) ?? null,
      iterator: async function* (options: { gte: string; lte?: string }) {
        const keys = Array.from(db.keys()).sort();
        for (const k of keys) {
          if (k >= options.gte && (!options.lte || k <= options.lte)) {
            yield [k, db.get(k)] as [string, unknown];
          }
        }
      },
    };

    const executors = buildNoloWorkspaceCliToolExecutors({
      store,
      env: { NOLO_USER_ID: "user1" },
    });

    const result = await executors.readDialog({
      name: "readDialog",
      arguments: JSON.stringify({ dialog: "01ARZ3NDEKTSV4RRFFQ69G5FAV" }),
    });

    expect(result.metadata).toEqual({
      noloWorkspaceTool: true,
      readDialog: true,
      inProcess: true,
    });

    const parsed = JSON.parse(result.content);
    expect(parsed.dialogId).toBe("01ARZ3NDEKTSV4RRFFQ69G5FAV");
    expect(parsed.dialogKey).toBe("dialog-user1-01ARZ3NDEKTSV4RRFFQ69G5FAV");
    expect(parsed.source).toBe("in-process");
    expect(parsed.meta.title).toBe("Test Dialog");
    expect(parsed.messagesCount).toBe(2);
    expect(parsed.messages).toHaveLength(2);
    expect(parsed.messages[0]).toMatchObject({ role: "user", content: "hello" });
    expect(parsed.messages[1]).toMatchObject({ role: "assistant", content: "hi there" });
  });

  test("supports mode=status and limit truncation", async () => {
    const db = new Map<string, any>();
    db.set("dialog-user1-01ARZ3NDEKTSV4RRFFQ69G5FAV", {
      status: "running",
      runtimeCheckpoint: { status: "running" },
    });
    db.set("dialog-01ARZ3NDEKTSV4RRFFQ69G5FAV-msg-01A", { role: "user", content: "msg1" });
    db.set("dialog-01ARZ3NDEKTSV4RRFFQ69G5FAV-msg-01B", { role: "assistant", content: "msg2" });
    db.set("dialog-01ARZ3NDEKTSV4RRFFQ69G5FAV-msg-01C", { role: "user", content: "msg3" });
    ...
    // mode=status
    const statusRes = await executors.readDialog({
      name: "readDialog",
      arguments: JSON.stringify({ dialog: "01ARZ3NDEKTSV4RRFFQ69G5FAV", mode: "status" }),
    });
    expect(statusRes.metadata).toEqual({
      noloWorkspaceTool: true,
      readDialog: true,
      inProcess: true,
      mode: "status",
    });
    const statusParsed = JSON.parse(statusRes.content);
    expect(statusParsed.status).toBe("running");
    expect(statusParsed.messages).toBeUndefined();

    // limit=2
    const limitRes = await executors.readDialog({
      name: "readDialog",
      arguments: JSON.stringify({ dialog: "01ARZ3NDEKTSV4RRFFQ69G5FAV", limit: 2 }),
    });
    const limitParsed = JSON.parse(limitRes.content);
    expect(limitParsed.messagesCount).toBe(2);
    expect(limitParsed.messages[0]).toMatchObject({ content: "msg2" });
    expect(limitParsed.messages[1]).toMatchObject({ content: "msg3" });
  });

  test("falls back to CLI child process when store is not provided", async () => {
    ...
    await executors.readDialog({
      name: "readDialog",
      arguments: JSON.stringify({ dialog: "dialog-user1-01ARZ3NDEKTSV4RRFFQ69G5FAV" }),
    });

    expect(spawnedCmd).toContain("dialog");
    expect(spawnedCmd).toContain("read");
    expect(spawnedCmd).toContain("dialog-user1-01ARZ3NDEKTSV4RRFFQ69G5FAV");
  });
});
