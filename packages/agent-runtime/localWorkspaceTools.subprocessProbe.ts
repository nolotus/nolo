  ...
  try {
    {
      ...
      const search = await executors.searchFiles({
        id: "call-search",
        name: "searchFiles",
        arguments: JSON.stringify({ query: "search" }),
      });
      assertContains(search.content, "README.md:1:hello search", "searchFiles content");
      assert(search.metadata?.exitCode === 0, "searchFiles exitCode should be 0");
...

    {
      ...
      const executors = createLocalWorkspaceToolExecutors({ workspaceRoot: root });
      const defaultSearch = await executors.searchFiles({
        id: "call-search-default",
        name: "searchFiles",
        arguments: JSON.stringify({ query: "SEARCH_IGNORED_TOKEN" }),
      });
      assert(!defaultSearch.content.includes("SEARCH_IGNORED_TOKEN"), "default search should respect .gitignore");
      assert(defaultSearch.metadata?.exitCode === 1, "default ignored search exitCode should be 1");
      const ignoredSearch = await executors.searchFiles({
        id: "call-search-ignored",
        name: "searchFiles",
        arguments: JSON.stringify({ query: "SEARCH_IGNORED_TOKEN", includeIgnored: true }),
      });
      assertContains(ignoredSearch.content, ".tmp", "includeIgnored search content");
      assert(ignoredSearch.metadata?.exitCode === 0, "includeIgnored search exitCode should be 0");
    }
