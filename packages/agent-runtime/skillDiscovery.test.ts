...
describe("parseSkillFrontmatter", () => {
  ...
  it("returns {} and warns when YAML frontmatter fails to parse (bad indentation)", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "nolo-parse-badindent-"));
    try {
      const filePath = join(tempDir, "SKILL.md");
      // Folded scalar where one continuation line is indented 1 space while the
      // rest are 2 spaces — js-yaml throws "bad indentation of a mapping entry".
      writeFileSync(
        filePath,
        [
          "---",
          "name: bad-indent-skill",
          "description: >-",
          "  properly indented line",
          " misindented line (1 space)",
          "  another properly indented line",
          "---",
          "body",
        ].join("\n"),
        "utf8",
      ...
      try {
        const parsed = parseSkillFrontmatter(filePath);
        // Silent-fail path: return shape is unchanged (empty object), never throw.
        expect(parsed).toEqual({});
        expect(parsed.description).toBeUndefined();
        expect(warnSpy).toHaveBeenCalled();
        const warnArgs = warnSpy.mock.calls.flat();
        expect(warnArgs.join(" ")).toContain(
          "[skillDiscovery] failed to parse frontmatter",
        );
        expect(warnArgs.join(" ")).toContain(filePath);
      } finally {
        warnSpy.mockRestore();
      }
    } finally {
