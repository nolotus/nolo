...
describe("cursorProvider", () => {
  ...
  test("handleExecServerMessage bridges grepArgs to searchFiles tool call", async () => {
    const mock = makeMockH2Request();
    const toolCalls: any[] = [];
    const executeTool = async (call: any) => {
      expect(call.name).toBe("searchFiles");
    ...
    expect(toolCalls[0].function.name).toBe("searchFiles");
    const execClient = mock.lastExecClientMessage();
  ...
  test("onToolEvent defaults round to 0 when toolEventRound omitted", async () => {
    ...
    await handleExecServerMessage(
      ...
      mock.stream as any,
      makeExecCtx(executeTool, [], [], (e: any) => events.push(e)),
    );
