...
describe("runLocalAgentTurn", () => {
  ...
  test("surfaces listFiles metadata in the tool message shown to the model", async () => {
    const savedTurns: AgentRuntimeSaveTurnInput[] = [];
    let completeCalls = 0;
    const adapter: AgentRuntimeHostAdapter = {
      host: "cli",
          ...
          if (completeCalls === 1) {
            return {
              ...
              tool_calls: [{
                id: "call-list",
                type: "function",
                function: { name: "listFiles", arguments: "{\"path\":\"docs\"}" },
              }],
              trace: messages,
            } as any;
          }
  ...
  test("surfaces searchFiles metadata in the tool message shown to the model", async () => {
    const savedTurns: AgentRuntimeSaveTurnInput[] = [];
    let completeCalls = 0;
    const adapter: AgentRuntimeHostAdapter = {
      host: "cli",
          ...
          if (completeCalls === 1) {
            return {
              ...
              tool_calls: [{
                id: "call-search",
                type: "function",
                function: { name: "searchFiles", arguments: "{\"query\":\"TODO\",\"path\":\"src\"}" },
              }],
              trace: messages,
            } as any;
          }
  ...
  test("continues tool loops until the provider returns final text", async () => {
    ...
    const adapter: AgentRuntimeHostAdapter = {
      ...
      loadAgentConfig: async (agentRef) => ({
        ...
        model: "fake-local",
        toolNames: ["readFile", "writeFile", "editFile", "searchFiles"],
      }),
      loadDialogHistory: async () => [],
      saveTurn: async (input) => {
        savedTurns.push(input);
  ...
  test("emits llm/tool loop events in order for a single-tool turn", async () => {
    ...
    expect(loopEvents[0]).toMatchObject({ kind: "llm-start", round: 0 });
    expect(loopEvents[1]).toMatchObject({ kind: "llm-end", round: 0, ok: true });
    expect(loopEvents[2]).toMatchObject({ kind: "tool-start", name: "execShell" });
    expect(loopEvents[3]).toMatchObject({ kind: "tool-end", name: "execShell", ok: true });
  ...
  test("repair 后仍 reasoning-only → 最终 content 是 EMPTY_ASSISTANT_FALLBACK_MESSAGE，不是空串", async () => {
    ...
    // 第一轮 reasoning-only → repair；第二轮仍 reasoning-only → fallback。
    expect(completeCalls).toBe(2);
  ...
  test("模型不支持图片时，vision 预处理把 image_url 替换为文字描述", async () => {
    let providerMessages: AgentRuntimeChatMessage[] = [];
    const loopEvents: any[] = [];
    let resolveProviderCalls: { model?: string; provider?: string }[] = [];
    const adapter: AgentRuntimeHostAdapter = {
      host: "cli",
      capabilities: ["local-provider", "local-persistence"],
      loadAgentConfig: async (agentRef) => ({
      ...
      resolveProvider: async (agentConfig) => {
        resolveProviderCalls.push({ model: agentConfig.model, provider: agentConfig.provider });
        // 对 vision 预处理 agent（Qwen 3.7 Flash）返回描述文本，对主 agent 正常返回
        const isVisionPreprocess = agentConfig.model === "qwen3.7-flash";
        return {
          model: agentConfig.model ?? "glm-5.2",
          complete: async (messages) => {
            if (isVisionPreprocess) {
              return {
                content: "A photo of a sunset over mountains",
                model: agentConfig.model ?? "qwen3.7-flash",
                trace: messages,
              };
            }
            providerMessages = messages as AgentRuntimeChatMessage[];
    ...
    // vision 预处理成功时收到 image-preprocessed 事件（而非 image-downgraded）。
    const preprocessedEvents = loopEvents.filter((e) => e.kind === "image-preprocessed");
    expect(preprocessedEvents.length).toBe(1);
    expect(preprocessedEvents[0]?.imageCount).toBe(1);
    // 不应收到 image-downgraded（预处理成功了）
    const downgradeEvents = loopEvents.filter((e) => e.kind === "image-downgraded");
    expect(downgradeEvents.length).toBe(0);
    // 主 agent 和 vision 预处理 agent 各 resolve 了一次 provider
    expect(resolveProviderCalls.length).toBeGreaterThanOrEqual(2);
  });

  test("模型不支持图片且 vision 预处理失败时，fallback 到剥离占位符", async () => {
    let providerMessages: AgentRuntimeChatMessage[] = [];
    const loopEvents: any[] = [];
    const adapter: AgentRuntimeHostAdapter = {
      host: "cli",
      ...
      loadDialogHistory: async () => [],
      saveTurn: async () => ({ dialogId: "dialog-no-vision-fallback" }),
      resolveProvider: async (agentConfig) => {
        const isVisionPreprocess = agentConfig.model === "qwen3.7-flash";
        return {
          model: agentConfig.model ?? "glm-5.2",
    ...
    expect(result.content).toBe("ok without image");
    // fallback：provider 收到的消息里没有 image_url part（被剥离了）
    for (const msg of providerMessages) {
      if (Array.isArray(msg.content)) {
        for (const part of msg.content) {
          expect(part?.type).not.toBe("image_url");
  ...
  test("injects runtime guidance and current-time blocks into the system message", async () => {
    ...
    // execShell + fetchWebpage 触发 web-research-tool-policy guidance。
    expect(systemContent).toContain("--- 生产环境网页研究工具策略 ---");
  });
  ...
  test("does not inject tool-gated guidance blocks when the agent has no relevant tools", async () => {
    ...
    expect(systemContent).not.toContain("--- 生产环境网页研究工具策略 ---");
  });
